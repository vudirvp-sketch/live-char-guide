...
- Edit files in `src/master/` for content changes (`part_*.html`, `appendix_*.html`)
- Edit files in `src/shell/` for client-side shell (HTML, CSS, JS — `lazy-loader.js`, `event-bus.js`, `widgets/`)
- Edit files in `src/assets/` for static assets (favicon, fonts, `vs-styles.css`)
- Edit files in `data/` for widget data (JSON files)
- Edit files in `src/scripts/` and `scripts/` for build logic and validation
- **Note:** top-level `index.html`, `assets/`, `widgets/`, `parts/`, `event-bus.js`, `data/`, `build.hash` are generated root fallbacks (regenerated by `pnpm run build`) — do NOT edit them directly
