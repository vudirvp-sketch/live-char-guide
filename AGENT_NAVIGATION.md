...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 3 Appendix ✅ MIGRATED (iter 18) + KI#13 ✅ **CLOSED** (iter 24, 123/123 = 100%) + Phase 4 SVG integration: E18 Greeting Algorithm ✅ INTEGRATED (iter 25) + Deployed Guide Audit (DGA) ✅ **COMPLETE** (Phase 1 iter 26, Phase 2 iter 28-31, **KI#18 ✅ CLOSED 9/9 resolved**: 7 fixed A+B+C+D+E+I+F, 2 keep-by-design G+H; KI#19 ✅ CLOSED incidental iter 30) + KI#14/#16/#17 ✅ CLOSED. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, техническая архитектура — в `docs/architecture.md`.
...
38. **Chinese chars in master HTML (KI#19, found + fixed iter 30 — incidental during KI#18-E analysis)** — `src/master/part_05.html` L269 содержал смешение русского с китайскими иероглифами: «следует за**线索**ми, которые другие игнорируют» (вместо «следует за **зацепками**»). Chinese «线索» (xiànsuǒ) = «clue» — копипаст из Chinese-language tool/prompt случайно попал в master HTML при past edit. Canon part_05.md L56 уже имел корректное русское «за зацепками» — master HTML был corrupted. **Fix (iter 30, DONE):** replaced «за线索ми» → «за зацепками» per canon L56. Single-token replacement, no structural change. **Правило:** при edit master HTML — проверять, что текст не содержит non-Cyrillic/non-Latin символов кроме allowed English terms (см. pitfall #5) и markdown punctuation. Если найден non-Cyrillic symbol в Russian prose — сравнить с canon (если canon корректен, master HTML был corrupted — fix per canon). Регрессионная проверка: `rg "[\\u4e00-\\u9fff]" src/master/` (поиск CJK Unified Ideographs).
...
**iter 20 (KI#13 Part 1+2 + KI#17 closure + SVG audit):** 57/123 inline styles → 28 CSS classes (`vs-ki13-*` prefix в vs-styles.css SECTION 6). part_01: 48→0, part_02: 9→0. **KI#17 CLOSED.** SVG extracts audit: 0 orphans, все 17 elements embedded, все styles в vs-styles.css. Build hash fd3d96d3. 10 docs/files updated.

**iter 21 (KI#13 Part 3+4 + Phase 4 SVG integration analysis):** 23/123 inline styles → 18 новых CSS селекторов (`vs-ki13-inset-text`, `vs-ki13-spine-chain-*`, `vs-ki13-ring-delay-*`, `vs-ki13-ring-title--*`). part_03: 2→0, part_04: 21→0. Phase 4 SVG integration analysis complete (Canon migration уже удалил major duplicates; 3 retained infographics intentional; Part 7B candidate для iter 23+ optional). Build hash fd3d96d3 unchanged. 10 docs/files updated.

**iter 22 (KI#13 Part 5+6):** 6/123 inline styles → 5 новых CSS селекторов (`vs-ki13-context-limits-note`, `vs-ki13-cot-panel/heading/text/strong`). part_05: 1→0, part_06: 5→0. Total KI#13: 86/123 fixed (70%). `src/assets/vs-styles.css` +30 строк (SECTION 6). `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged. 7 docs + 4 regenerated fallbacks updated.

**iter 23 (KI#13 Part 7A):** 19/123 inline styles → 9 новых CSS селекторов с `vs-ki13-p7a-*` sub-namespace (semantic grouping: 4 color overrides + 3 badge sizing/spacing + 1 template-hidden state + 1 border-cyan). part_07a: 19→0 (E16: 4→0, E17: 8→0, E02: 7→0). Total KI#13: 105/123 fixed (85%). `src/assets/vs-styles.css` +33 строки (SECTION 6). All validation gates PASS. Build hash fd3d96d3 unchanged.

**iter 24 (KI#13 Part 9+10 — COMPLETE, KI#13 CLOSED):** 18/123 inline styles → 19 новых CSS селекторов с `vs-ki13-p9-*` и `vs-ki13-p10-*` sub-namespaces (semantic grouping: 1 indicator position + 4 zone-title color modifiers + 1 quick-checks panel + 1 shared callout-pos base + 11 per-instance top modifiers + 1 card-block-accent). part_09: 6→0 (E14 Quality Scale), part_10: 12→0 (E15 Annotated Blueprint — DRY pattern for 11 positional callouts: shared `.vs-ki13-p10-callout-pos` base + `.vs-ki13-p10-callout-pos--top-N` modifiers). Total KI#13: **123/123 fixed (100%) — CLOSED.** `src/assets/vs-styles.css` +52 строки (SECTION 6, total 60 селекторов). `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged. 7 docs + 4 regenerated fallbacks updated.

**iter 25 (Phase 4 SVG integration — E18 Greeting Algorithm, COMPLETE):** New VS element E18 created — 4-step pipeline (Sensory Anchor → Тело FLAW → Реплика → Крючок) с SVG arrows, reuses E02 `.pipeline-*` classes + new `.pipeline-node__code` for technique sequence line. Step 2 (Тело FLAW) uses `.pipeline-node__box--spine` (violet — SPINE connection). Standalone prototype: `visual-system/elements/E18-greeting-algorithm.html`. Component extracts: 3 new files in `visual-system/integration/component-extracts/`. E18 styles (+12 строк) appended to `src/assets/vs-styles.css` SECTION 5 (header E01–E17 → E01–E18). Master HTML `src/master/part_07b.html`: textual `infographic inf-pipeline` block (lines 33–61 pre-iter-25) → VS-EMBED E18 (lines 28–110 iter 25). Migration principle «viz > dry text» applied. part_07b: 371 → 424 строк (+53). Canon `docs/canon/part_07b.md` updated (front-matter, §7B.2 row 2 retention note, +iter 25 update section + validation gates). INTEGRATION-MAP.md +E18 row. `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged. **Phase 4 SVG integration — DONE.** VS elements registry: 18 (E01–E18).

**iter 26 (Deployed Guide Audit Phase 1 — STARTED, 1/8 fixed):** Новый цикл работы по запросу user: пошаговая проверка собранного при деплое гайда на дублирование смысловой и функциональной нагрузки между текстом и визуализациями. Принцип: `viz > dry text` (iter 8+). Полный аудит 14 master HTML файлов выполнен — выявлено 8 duplication/inconsistency кейсов (KI#18 sub-items A–H). Применён 1 safe fix: **KI#18-A** (Part 9 Quality Scale) — убран дублирующий столбец «Признаки» (повторял E14 zone criteria), добавлен недостающий tier «Отличный» (85–100%), исправлена naming inconsistency «Плохой» → «Слабый» (соответствие E14 viz tier label), добавлен intro paragraph linking to E14. `src/master/part_09.html`: 582 → 583 строк (+1). Canon `docs/canon/part_09.md` updated (§9.1 table + front-matter + migration history row 4). `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged. **KI#18 🟡 ACTIVE — 1/8 fixed (A), 7 pending (B–H).**

**iter 27 (STATUS CHECK — без правок кода):** User запросил краткий отчёт о стадии работ. Никаких изменений в master HTML / canon / build artifacts / widgets / data. Только актуализация документации: дата → 2026-07-08, iter 27 запись добавлена в STATUS/worklog/CHANGELOG/PLAN/AGENT_NAVIGATION. Build hash fd3d96d3 unchanged (validation gates не запускались — нет source изменений).

**iter 28 (DGA Phase 2 — KI#18-B + KI#18-C FIXED, KI#18-I NEW documented):** Применены 2 safe fix: KI#18-B (Part 1 p1_card_overview drop «Функция» col duplicating E01) + KI#18-C (Part 2 p2_basic_anchors drop «Описание» col duplicating E03). KI#18-I NEW documented (Part 2 p2_embodiment — same pattern as C, E04 depth-label duplication), pending iter 29. All validation gates PASS. Build hash fd3d96d3 unchanged. KI#18 🟡 ACTIVE — 3/9 fixed (A+B+C), 6 pending.

**iter 29 (DGA Phase 2 continued — KI#18-I + KI#18-F FIXED partial):** Применены 2 safe fix: **KI#18-I** (Part 2 p2_embodiment — drop duplicate «Описание» column, повторявшую E04 `depth-label` Внутренний/Физический/Перцептивный/Вывод; expand intro paragraph linking to E04 viz). Table: 4 cols → 3 cols (# / Слой / Пример — typological category examples unique). part_02: 415 → 415 (0 net — intro line expanded, 4 rows shortened by 1 cell each). Same pattern as KI#18-C (applied iter 28). **KI#18-F** (Part 6 p6_cot_tiers — drop duplicate «Формат» column, повторявшую E11 `stair-step__name` Без CoT/Эмоциональная метка/GHOST-link/Полный XML; add intro paragraph linking to E11 viz). Table: 4 cols → 3 cols (Tier / Для моделей / Пример). part_06: 259 → 261 (+2 — intro paragraph added, 4 rows shortened by 1 cell each). **KI#18-F partial:** «Для моделей» + «Пример» cols partial duplication (≈ viz `model-pill` + `stair-step__format` Russian translations) DEFERRED for accessibility — kept for Russian readers, full deduplication needs рус/англ overlap decision. Canon `docs/canon/part_02.md` + `docs/canon/part_06.md` updated (front-matter + table rewritten + DGA Phase 2 fix section + validation gates). `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged. **KI#18 🟡 ACTIVE — 5/9 fixed (A+B+C+I+F), 4 pending (D, E, G, H) iter 30+.**

**iter 30 (DGA Phase 2 continued — KI#18-D + KI#18-E FIXED, KI#19 incidental FIXED):** Применены 3 fix: **KI#18-D** (Part 4 p4_spine_overview intro L143 — trimmed partial re-explanation «фреймворк из 5 элементов» shown in E05 viz hexagons → «SPINE связивает прошлое персонажа с его поведением (структура показана в VS-EMBED E05 выше).» Single-line edit. part_04: 670 → 670 (0 net). Панель «Причинно-следственная цепь» внутри VS-EMBED L122-136 НЕ редактировалась — часть canonical E05 viz source, добавляет unique контент (русские переводы + causality verbs + dynamics insight). По принципу «better to underdo than to break» — panel оставлена). **KI#18-E** (Part 5 p5_elena_profile — semantic bug fixed: aligned OCEAN extreme-pole rule к strict <30 или >70, most prevalent across codebase. Cross-viz/text inconsistency resolved: E09 viz L122+L138 says «1 экстремум» (only O=72), но text L272-273, L279 говорил «3 экстремальных полюса» (broad rule <40 или >60). L272: «Экстремальный полюс.» → «Cautious zone (30–40) — на границе с экстремальной зоной (<30), напрямую связана с FLAW.» L273: аналогично для N=68 (cautious 60–70). L279: «3 экстремальных полюса» → «1 экстремальный полюс (O=72 > 70) + 2 значения в cautious zone (A=38, N=68)». part_05: 615 → 615 (0 net). Canon part_05.md L59+L60+L62 синхронизированы). **KI#19** (incidental — Chinese chars «线索» в `src/master/part_05.html` L269 заменены на русское «зацепками» per canon L56. Master HTML был corrupted при past edit). Canon `docs/canon/part_04.md` + `docs/canon/part_05.md` updated (front-matter + DGA Phase 2 fix section + validation gates). `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged. **KI#18 🟡 ACTIVE — 7/9 fixed (A+B+C+D+E+I+F), 2 pending (G, H) iter 31+ likely keep-by-design.**

**iter 31 (DGA Phase 2 final — KI#18-G + KI#18-H ✅ CLOSED keep-by-design → KI#18 ✅ CLOSED):** Документированы rationale для последних 2 pending sub-items KI#18. Оба sub-item'а — keep-by-design с documented rationale, **без master HTML edit**. **KI#18-G** (Part 8 per-AP sections vs E12 Antipattern Catalog viz — catalog vs detail pattern): rationale documented в `docs/canon/part_08.md` (7-dimension comparison table Purpose/Symptom/Cause/Fix/Examples/Callouts/Reader intent; viz = quick scan 15 APs на one screen с severity dots, per-AP sections = deep-dive with concrete thresholds + multi-step solutions + cross-ref links + diff examples + RULE callouts; different reader intents — scan/lookup vs deep-dive, не pure re-explanation; same pattern as iter 12 item #5 navigation table). **KI#18-H** (Part 10 E15 Annotated Blueprint callouts vs E01 Card Anatomy in Part 1 — annotation layer pattern): rationale documented в `docs/canon/part_10.md` (6-dimension comparison table Purpose/Visualization/Annotation layers/Token budget display/Reader intent/Position in flow; E01 = pure block anatomy vertical card-stack, E15 = central card template с 4 ANNOTATION LAYERS structure/anchors/spine/directives — different visualization patterns; E15 callouts ≠ pure budget tables — annotation labels combining budget range с annotation context; removing callouts = strips E15 of core function 4-layer annotation approach; Token Budget subsections per character card ≠ E15 callouts — concrete total per card vs generic per-block ranges). `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged (no source code change). **KI#18 ✅ CLOSED — 9/9 resolved** (7 fixed: A iter 26, B+C iter 28, I+F iter 29, D+E iter 30; 2 keep-by-design: G+H iter 31). **DGA COMPLETE.**

**iter 32+ — что осталось:**
- **DGA COMPLETE — KI#18 ✅ CLOSED.** Все 9 sub-items resolved (7 fixed + 2 keep-by-design). iter 32+ roadmap: **none planned** — DGA Phase 1+2 COMPLETE, all KIs CLOSED.
- **Все previous KI (KI#1..KI#19) ✅ CLOSED.** Новые баги — сначала документировать в `STATUS.md` как Known Issue (KI#N), потом фиксить.
- **Принцип `viz > dry text` (iter 8+):** сохраняется. Unique контент не удаляется даже при дублировании. **Catalog vs Detail / Annotation Layer patterns** — NOT pure re-explanation, keep-by-design с documented rationale (см. KI#18-G + KI#18-H).
...
**Подсказка следующему агенту:** Перед стартом iter 32+ прочитай `STATUS.md` (iter 31 DGA Phase 2 final COMPLETE — KI#18 ✅ CLOSED 9/9 resolved: 7 fixed A+B+C+D+E+I+F, 2 keep-by-design G+H; все previous KI#1..#17 + KI#19 ✅ CLOSED; iter 32+ roadmap — none planned, DGA COMPLETE), `worklog.md` (iter 31 record — DGA Phase 2 final; iter 30 one-liner; iter 29 one-liner; iter 28 one-liner; iter 26 record — DGA Phase 1), этот файл (AGENT_NAVIGATION §6 pitfall #37 KI#18 ✅ CLOSED iter 31, §6 pitfall #38 KI#19 ✅ CLOSED, §8 iter 32+ roadmap — DGA COMPLETE, none planned), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 31 → DGA Phase 2 final, §8 iter 31 stop point — DGA COMPLETE). Принцип `viz > dry text` (iter 8+) — сохраняется. Unique контент не удаляется даже при дублировании. **Catalog vs Detail / Annotation Layer patterns** — NOT pure re-explanation, keep-by-design с documented rationale. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий.
