...
| `docs/research/v2_architecture.md` | **The consolidated v2 architecture specification (iter 146, P-a per DEC-23) — RATIFIED by DEC-24 (iter 147).** Layer model L1–L11 (post-extraction) · reader-mode spec (F1: Learn/Build/Debug/Reference — entries, visibility, disclosure; overlays, not four sites) · presentation policy (F2 dispositions · F3 checklist triage · F4 example tiering · F5 Part micro-template · F6 `<details class="interactive">`) · rule-strength (DEC-20 + iter-145 OBSERVATION) · 7/7 cluster+gate inventory · v2 build plan (per-Part slices in guide order) · audit & switch plan. The v2 build phase is OPEN — the Part-slice stage COMPLETE (Parts 1–10 executed iters 147–157, maps §5.8–§5.18, + the owner-called KI#84 canonical-audit reconciliation iter 157); the appendix-slice stage OPEN (Appendix A + the KI#70 WIRE executed iter 158, map §5.19; Appendix B iter 159, map §5.20; Appendix C iter 160, map §5.21 — the R01/R02 glossary compressions executed — the appendices render at runtime since iter 158; D follows). |
