...
**iter 15+ — что осталось:**
- **iter 15:** (reserved — не нужен, iter 14 покрыл Part 1+2+3).
...
**Подсказка следующему агенту:** Перед стартом iter 16 прочитай `STATUS.md` (iter 14 COMPLETE, Part 1+2+3+4+7A+8+9 ✅ MIGRATED, KI#13+KI#14+KI#16+KI#17 ACTIVE), `worklog.md` (iter 14 record), этот файл (AGENT_NAVIGATION §8 iter 15+ roadmap), `docs/canon/_README.md` (§5 migration status — Part 1+2+3+4+7A+8+9 ✅, остальные ❌), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 16 priorities). iter 16 priorities: Canon creation + migrate для Part 5+6+7B+10 (Psychology, CoT, Lorebook, Examples) — см. `_README.md` §5 planned iters. Перед стартом: `rg "VS-EMBED:" src/master/part_05.html src/master/part_06.html src/master/part_07b.html src/master/part_10.html` для inventory. Принцип «viz > dry text» (iter 8) — сохраняется. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue, потом фиксий.
