...
**Подсказка следующему агенту:** Перед стартом iter 14 прочитай `STATUS.md` (iter 13 COMPLETE, Part 4+7A+8+9 ✅ MIGRATED, KI#13+KI#14+KI#16+KI#17 ACTIVE), `worklog.md` (iter 13 record), этот файл (AGENT_NAVIGATION §8 iter 14+ roadmap), `docs/canon/_README.md` (§5 migration status — Part 4+7A+8+9 ✅, остальные ❌), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 14 priorities). iter 14 priorities: Canon creation + migrate для Part 1+2+3 (Foundations, Anchors, Voice) — см. `_README.md` §5 planned iters. Перед стартом: `rg "VS-EMBED:" src/master/part_01.html src/master/part_02.html src/master/part_03.html` для inventory. Принцип «viz > dry text» (iter 8) — сохраняется. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue, потом фиксий.
