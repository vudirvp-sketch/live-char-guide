...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 3 Appendix ✅ MIGRATED (iter 18) + KI#13/#18/#20 ✅ CLOSED + iter 34 (CSS/CSP) + **iter 35 — KI#21 P0 ✅ CLOSED (16/57 canon правок, build hash 69d9b813 unchanged)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, **аудит канона + fix plan (iter 33+) — в `docs/AUDIT_VERIFICATION.md` (P0 ✅ CLOSED iter 35, P1-P3 pending iter 36-38)**, техническая архитектура — в `docs/architecture.md`.
...
**iter 36+ — что осталось:**
- **KI#21 🟡 P0 ✅ CLOSED — Content Audit contradictions.** Осталось 41 правок (P1 11 + P2 18 + P3 12) на iter 36-38 по плану в `docs/AUDIT_VERIFICATION.md` §4: **iter 36 (P1, 11 правок — пример vs правило B1 LIE/B2/B5/B6 + dead code D1/D2/D4 + A5/A7/A8) → iter 37 (P2, 18 правок — терминология C1-C2/C5 + структурный cleanup E1-E7/F1/F4-F10/B4, ~1500 строк удалений) → iter 38 (P3, 12 правок + 3 новые секции — локальные D3/D5-D7/F2-F3/F8/F10 + G1-G5).**
