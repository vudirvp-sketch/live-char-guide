...
**iter 10+ — что осталось:**
- **iter 10:** Canon Part 7A — создать `docs/canon/part_07a.md` (13 H2 секций, 4 VS-маркера для E07/E08/E16/E17, Migration Notes таблица для iter 11). **НЕ править master HTML.**
- **iter 11:** Migrate Part 7A — `src/master/part_07a.html` (1168 строк — самый большой файл). При риске — разбить на 2 под-итерации (§7A.1-7 + §7A.8-13).
...
**Подсказка следующему агенту:** Перед стартом iter 10 прочитай `STATUS.md` (iter 9 COMPLETE, Part 4 ✅ MIGRATED+VALIDATED, KI#13+KI#14+KI#16 ACTIVE), `worklog.md` (iter 9 record — этот), этот файл (AGENT_NAVIGATION §6 pitfall #34 KI#16 added, §8 iter 10+ roadmap), `docs/canon/_README.md` (§5 Part 7A ❌ NOT MIGRATED — iter 10 задача, §4 workflow Canon creation), `docs/canon/part_04.md` (reference pilot — структура Canon-файла, front-matter, VS-маркеры, Migration Notes таблица), `src/master/part_07a.html` (1168 строк, 13 секций, 4 VS-EMBED: E07 voice hierarchy, E08 core directives, E16 author note, E17 sampling params). iter 10 priorities: (1) Создать `docs/canon/part_07a.md` (Canon §7A — 13 H2 секций, все `data-section` покрыть, 4 VS-маркера для E07/E08/E16/E17, Migration Notes таблица для iter 11); (2) НЕ править master HTML — это iter 11 задача; (3) После Canon creation — `pnpm run validate:master` (should be 0 errors) + git commit; (4) Решить — разбивать Part 7A на 2 под-итерации (Canon §7A.1-7 + Canon §7A.8-13) или делать одним проходом. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue, потом фиксий. Принцип «viz > dry text» (iter 8) — сохраняется.
