...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 3 Appendix ✅ MIGRATED (iter 18) + KI#13/#18/#20 ✅ CLOSED + iter 34 (CSS/CSP) + iter 35 (KI#21 P0) + iter 36 (KI#21 P1) + **iter 37 — KI#21 P2 ✅ CLOSED (45/57 canon правок, ~1130 строк удалено, build hash 69d9b813 unchanged)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, **аудит канона + fix plan (iter 33+) — в `docs/AUDIT_VERIFICATION.md` (P0+P1+P2 ✅ CLOSED iter 35-37, P3 pending iter 38)**, техническая архитектура — в `docs/architecture.md`.
...
**iter 38+ — что осталось:**
- **KI#21 🟡 P0+P1+P2 ✅ CLOSED — Content Audit contradictions.** Осталось 12 правок + 3 новые секции (P3) на iter 38 по плану в `docs/AUDIT_VERIFICATION.md` §4.4: **iter 38 (P3, 12 правок — локальные D3/D5-D7/F2-F3/F8/F10 + 3 новые секции G1-G5: «Как читать», TL;DR, Character map, Pre-build checklist).**
...
- **Callout labels English (iter 37+):** метки `RULE`, `RECOMMENDATION`, `EXAMPLE`, `ILLUSTRATION`, `TEMPLATE`, `Bridge`, `Synthesis`, `Cross-ref` — на английском как semantic anchors. Тело callouts — на русском. См. `docs/canon/_README.md` §3.9.
...
**Подсказка следующему агенту:** Перед стартом iter 38+ прочитай `STATUS.md` (iter 37 — KI#21 P0+P1+P2 ✅ CLOSED, 45/57 правок, ~1130 строк удалено, build hash `69d9b813`; iter 36 one-paragraph — canon P1; iter 35 one-paragraph — canon P0; iter 34 one-paragraph — CSS/CSP fix; iter 33 one-liner — audit verified; все previous KI#1..#24 ✅ CLOSED/VERIFIED), `worklog.md` (iter 37 record — самый подробный; iter 36 one-liner), этот файл (AGENT_NAVIGATION §6 pitfall #40 KI#21 P0+P1+P2 ✅ CLOSED, §8 OP-1 iter 37 row + iter 38+ roadmap с KI#21 P3 plan), **`docs/AUDIT_VERIFICATION.md` §4.4 (P3 fix plan — 12 правок + 3 новые секции: P3-1 D3 Greeting Елены 2 сцены, P3-2 D5 HTML-комментарии в карточках Омнис, P3-3 D6 Йоуёма контекст, P3-4 D7 cross-refs на Уолтера, P3-5 F2 Price table примеры, P3-6 F3 Voice Isolation % methodology, P3-7 F8 (covered by P0-2), P3-8 G1 «Как читать», P3-9 G2 TL;DR, P3-10 G3 (covered by P2-1), P3-11 G4 Character map, P3-12 G5 Pre-build checklist)**, `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE; §3.9 callout labels policy — English by design). Принцип `viz > dry text` (iter 8+) — сохраняется. VS scroll-animation invariant — `python3 scripts/audit_vs_embeds.py` (0 regressions expected). CSS scoping invariant (iter 34+) — VS-EMBED selectors scoped к element-specific parent (не generic). YAML front-matter (iter 37+) — все canon-файлы используют YAML front-matter, markdown quote-block deprecated. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 таблицу с sub-ID (A11, B7, и т.д.) и пометкой P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID, аудит переоценил, examples в пределах лимита 120 токенов).
