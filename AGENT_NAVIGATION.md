...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 3 Appendix ✅ MIGRATED (iter 18) + KI#13/#18/#20 ✅ CLOSED + iter 33 audit verified + **iter 34 — KI#22 ✅ CLOSED + KI#23 ✅ CLOSED + KI#24 ✅ VERIFIED (build hash fd3d96d3 → 69d9b813)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, **аудит канона + fix plan (iter 33+) — в `docs/AUDIT_VERIFICATION.md` (fix plan перенесён на iter 35-38)**, техническая архитектура — в `docs/architecture.md`.
...
40. **Content Audit contradictions (KI#21, found iter 33, 🟡 DOCUMENTED — fix plan ready в `docs/AUDIT_VERIFICATION.md` для iter 35-38)** — полный аудит канона `docs/canon/` (14 файлов, 5 008 строк) выявил ~50 противоречий/проблем: 10 критических (A1-A10 — T→A→P «Price» vs «Pattern» в Glossary, Елена OCEAN 3 vs 1+2 в §7A.13, Счётчик вырезаний 2 vs 3 уровня в 4 местах, NEED Выщербленного 3 формулировки, AP-9 conflates absent GHOST/LIE с WANT/NEED conflict, AP-15 «замолкает на час» = deferred Price, «Счётчик вырезаний» секция AN не описана, §8.1 15 APs + 1 без номера, §9.11 resume 3-level vs 4-zones, §9.11 два разных Quick Check набора), 6 «пример vs правило» (B1-B6, из них **B3 ❌ INVALID** — Examples Омнис в пределах лимита 120 токенов), 8 категорий терминологии (C1-C8 — англицизмы, метки callouts, кавычки), 7 проблем примеров (D1-D7 — dual-Elena GHOST, variant'ы Выщербленного, 2 Greeting Елены, Lorebook secondary GHOST, English `<!-- Demonstrates: -->`, Йоуёма без контекста, Уолтер изолирован), 7 структурных (E1-E7 — front-matter, Migration Notes ~1500 строк, Cross-refs ending, resume, §1.3 orphan, Pattern Matcher 3 раза, копипаста), 10 локальных (F1-F10 — 30+ «Canon planned iter 13/14/16» заглушек, типы Price без примеров, % без источника, cautious zone без определения, `<br/>` в markdown, Keirsey vs MBTI, и т.д.), 5 концептуальных упущений (G1-G5 — нет «Как читать», нет TL;DR, нет glossary-on-first-use, нет карты персонажей, нет pre-build checklist). **3 NEW-пункта найдены при верификации:** NEW-1 (китайский «待» в `part_04.md` L366, не покрыт KI#19), NEW-2 (Lorebook walkthrough Елены в `part_07a.md` L667 = secondary GHOST, усиливает D4), NEW-3 (§5.1 vs §5.3 vs §5.3 context-table — 3 формулировки правила OCEAN). **Fix plan (iter 35-38 — сдвинут с iter 34-37 из-за iter 34 CSS/CSP fix):** P0 (16 правок, критические противоречия) → P1 (11 правок, пример vs правило + dead code) → P2 (18 правок, терминология + структурный cleanup ~1500 строк удалений) → P3 (12 правок + 3 новые секции, локальные правки + G1-G5). Полный план с exact file+line+before/after для каждой правки — в `docs/AUDIT_VERIFICATION.md` §4. После каждой итерации — validation gates + `audit_vs_embeds.py` + git commit/push. Build hash `69d9b813` (после iter 34) expected unchanged (только canon-контент правки). **Правило (iter 33+):** при обнаружении новых противоречий в каноне — добавлять в `docs/AUDIT_VERIFICATION.md` §2 таблицу + присваивать sub-ID (A11, B7, и т.д.) с пометкой P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (аудит переоценил, они в пределах лимита 120 токенов).
...
| `docs/AUDIT_VERIFICATION.md` | При обнаружении новых противоречий в каноне — добавлять в §2 таблицу с sub-ID. Фикс-план iter 34-37 (KI#21 P0-P3) — canonical source. |
...
**iter 35+ — что осталось:**
- **KI#21 🟡 DOCUMENTED — Content Audit contradictions.** Fix plan ready в `docs/AUDIT_VERIFICATION.md` §4: **iter 35 (P0, 16 правок — критические противоречия A1-A10 + NEW-1 + NEW-3) → iter 36 (P1, 11 правок — пример vs правило B1/B2/B5/B6 + dead code D1/D2/D4 + A5/A7/A8) → iter 37 (P2, 18 правок — терминология C1-C2/C5 + структурный cleanup E1-E7/F1/F4-F10/B4, ~1500 строк удалений) → iter 38 (P3, 12 правок + 3 новые секции — локальные D3/D5-D7/F2-F3/F8/F10 + G1-G5).**
...
**Подсказка следующему агенту:** Перед стартом iter 35+ прочитай `STATUS.md` (iter 34 — KI#22 ✅ CLOSED + KI#23 ✅ CLOSED + KI#24 ✅ VERIFIED, build hash `69d9b813`; iter 33 one-paragraph — Content Audit Verified без правок кода; KI#21 🟡 DOCUMENTED — Content Audit contradictions, fix plan ready для iter 35-38; iter 32 one-liner — KI#20 ✅ CLOSED; все previous KI#1..#23 ✅ CLOSED/VERIFIED), `worklog.md` (iter 34 record — самый подробный; iter 33 one-liner), этот файл (AGENT_NAVIGATION §6 pitfalls #41 KI#22 + #42 KI#23 ✅ CLOSED, §8 OP-1 iter 34 row + iter 35+ roadmap с KI#21 fix plan), **`docs/AUDIT_VERIFICATION.md` (главный deliverable iter 33 — §4 фикс-план: 16 P0 + 11 P1 + 18 P2 + 12 P3 = 57 правок, exact file+line+before/after; iter numbering shifted 34→35, 35→36, 36→37, 37→38)**, `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 32 → KI#20 CLOSED, §8 iter 32 stop point). Принцип `viz > dry text` (iter 8+) — сохраняется. VS scroll-animation invariant — `python3 scripts/audit_vs_embeds.py` (0 regressions expected). CSS scoping invariant (iter 34+) — VS-EMBED selectors scoped к element-specific parent (не generic). Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 таблицу с sub-ID (A11, B7, и т.д.) и пометкой P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID, аудит переоценил, examples в пределах лимита 120 токенов).
