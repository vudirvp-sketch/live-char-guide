...
**iter 23 (KI#13 Part 7A):** 19/123 inline styles → 9 новых CSS селекторов с `vs-ki13-p7a-*` sub-namespace (semantic grouping: 4 color overrides `.vs-ki13-p7a-text-{violet/muted/danger/success}` × 10 применений, 3 badge sizing/spacing `.vs-ki13-p7a-badge-meta` + `.vs-ki13-p7a-badge-indent` + `.vs-ki13-p7a-badge-indent-top` × 7 применений, 1 state `.vs-ki13-p7a-template-hidden` для template-b initial display:none, 1 border `.vs-ki13-p7a-border-cyan`). part_07a: 19→0 (E16: 4→0, E17: 8→0, E02: 7→0). JS toggle compatibility verified для `.vs-ki13-p7a-template-hidden` — `vs-e16-author-note.js` line 37-38 использует `element.style.display = 'block'/'none'` (inline assignment overrides class, initial state via class preserved). Total KI#13: 105/123 fixed (85%). `src/assets/vs-styles.css` +33 строки (SECTION 6). `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged. 7 docs + 3 regenerated fallbacks updated.

**iter 24+ — что осталось:**
- **iter 24:** KI#13 Part 9 (6) + Part 10 (12) = 18 inline styles. Part 9: 1 positional `bottom:50%`, 4 color overrides, 1 panel max-width. Part 10: 11 positional `top:Npx; right:20px;` styles — кандидат на 1 generic class + per-instance modifier или single selector per position, 1 border-left+violet.
- **iter 25 (optional, low priority):** Phase 4 SVG integration — Part 7B Greeting algorithm infographic → new VS element E18 (requires `visual-system/elements/` prototyping first).
...
**Подсказка следующему агенту:** Перед стартом iter 24+ прочитай `STATUS.md` (iter 23 COMPLETE, KI#13 105/123 fixed = 85%, 18 remaining), `worklog.md` (iter 23 record), этот файл (AGENT_NAVIGATION §6 pitfall #36 KI#13 ACTIVE, §8 iter 24+ roadmap), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 24 priorities + §8 Phase 4 analysis). iter 24 priorities: KI#13 Part 9 (6 inline styles) + Part 10 (12 inline styles) = 18 inline styles. Part 10 имеет 11 positional `top:Npx; right:20px;` styles — кандидат на generic class + per-instance modifier. Принцип «viz > dry text» (iter 8) — сохраняется. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue, потом фиксий.
