...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 4 Appendix + Part 0 (concept) ✅ MIGRATED/ADDED (iter 18 + iter 38) + KI#13/#18/#20/#21/#22/#23/#32 ✅ CLOSED + iter 34-44 + **iter 45 — KI#33 🔵 PARTIAL (canon→master HTML sync Phase 2: 24 fixes applied; cumulatively 33/57; contentHash `665cede7` 2nd change since iter 34; regression test extended to 34 checks)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 0 (concept) → Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, **аудит канона + fix plan (iter 33+) — в `docs/AUDIT_VERIFICATION.md` (P0+P1+P2+P3 ✅ CLOSED iter 35-38, KI#21 ✅ CLOSED canon; 33/57 master sync iter 44+45)**, техническая архитектура — в `docs/architecture.md`.
...
| 45 | **CANON→MASTER HTML SYNC PHASE 2 ✅ PARTIAL — KI#33 🔵 PARTIAL (33/57 cumulatively, 24 fixes this iter).** 24 content fixes применены к `src/master/*.html` в 10 файлах: P0-1 (glossary T→A→P Pattern→Price), P0-7/8/9 (part_04 Выщербленный NEED text + variant row delete), P0-10 (part_08 AP-15 immediate Price), P0-11 (part_09 3-level→4-zone scale), P0-12 (part_09 Vysh Quick Check rename), P0-13/14 (part_10 Omnis GHOST+FLAW rewrites — concrete event/behavior вместо abstract fear/adjective), P0-16 (part_05 §5.1 RULE expansion), P1-1 (part_08 AP-9 broken SPINE criterion), P1-3 (part_08 OCEAN Overload orphan row delete + footnote), P1-4 (part_10 Walter GHOST rewrite — concrete observation, no label «Унижение»), P1-5 (part_04 3 Anchor type definitions), P1-6 (part_06 Tier 0 → 12B+), P1-7 (part_04 secondary GHOST row delete + Примечание), P1-10 (part_07a Lorebook example пожар→предательство), P1-11 (part_07b Пример 1 added + Пример 2/3/4 renumbered), P3-1 ×2 (part_07b + part_10 Greeting scene notes), P3-3 (part_03 Йоуёма context), P3-5 (part_02 Price table 4th column), P3-6 (part_03 Voice Isolation methodology note). P0-15 (NEW-1) SKIP — `待` char и Cross-references секция уже удалены. **contentHash: `34c34a7d9839c11d` → `665cede798c34fc0` (2nd change since iter 34).** Shell hash `69d9b813` unchanged. Regression test extended с 11 до 34 checks. **NEW invariant (iter 45+): Callout class policy** — `.callout.rule/.rec/.ex` и plain `.callout` ONLY; `.callout.note/.info/.warn/.tip/.box/.sidebar/.custom/.important` ПРОИБИЩЕНы (`scripts/validate-master.mjs` L452-463). Все validation gates PASS + `audit_vs_embeds.py` 0 regressions + `audit_canon_master_sync.py` 34/34 PASS. Осталось 24 fixes для iter 46+. | KI#33 🔵 PARTIAL |

**iter 46+ — что осталось:**
- **KI#33 🔵 PARTIAL (iter 45) — canon→master HTML sync Phase 3 (iter 46+, MEDIUM).** 24 audit правок remain. Категории: P1-8/9 (part_04 secondary-LIE Елена + variant-LIE Выщербленный row delete, 2 fixes low risk), P2-* content fixes (terminology cleanup — skip P2-4 YAML, P2-15 `<br/>` metadata), P3-2 (D5 part_10 HTML comments → visible callouts, structural high risk), P3-4 (D7 part_01/04/09 Уолтер cross-refs). После каждого Part: `pnpm run build` + `validate:master` + `audit_canon_master_sync.py`. Расширить `audit_canon_master_sync.py` до general-purpose drift detector.
...
**Подсказка следующему агенту:** Перед стартом iter 46+ прочитай `STATUS.md` (iter 45 — KI#33 🔵 PARTIAL 33/57 fixes, contentHash `665cede7` 2nd change since iter 34; iter 44 one-paragraph; iter 34-43 one-liners; все previous KI#1..#32 ✅ CLOSED/VERIFIED), `worklog.md` (iter 45 record — самый подробный; iter 44 one-liner), этот файл (AGENT_NAVIGATION §2a Deployment Pipeline iter 43+, §8 OP-1 iter 45 row + iter 46+ roadmap — KI#33 fix Phase 3 MEDIUM priority + minor задачи LOW), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 4 Appendix + Part 0 concept ✅, Canon COMPLETE; §3.9 callout labels policy). **Приоритет iter 46: KI#33 fix Phase 3 — canon→master HTML sync (24 fixes remain, MEDIUM).** **Invariants (iter 45+):** (1) `viz > dry text` (iter 8+). (2) VS scroll-animation — `python3 scripts/audit_vs_embeds.py` (0 regressions expected). (3) Component extracts drift (iter 42+). (4) CSS scoping (iter 34+). (5) YAML front-matter (iter 37+). (6) **Guide's role as example takes priority over character canon (iter 39+).** (7) **OCEAN labeling consistency (iter 40+, расширен iter 41; master sync ✅ iter 44).** (8) **Bible ↔ canon cross-ref symmetry (iter 41+; master sync ✅ iter 44).** (9) **Canon → master HTML sync (iter 43+ invariant, iter 45 PARTIAL 33/57):** `docs/canon/*.md` = source of truth для content. `src/master/*.html` = production HTML, деплоится на сайт. Regression test `python3 scripts/audit_canon_master_sync.py` (34/34 PASS — 11 iter 44 + 23 iter 45). Расширить до general-purpose drift detector — iter 46+. (10) **Build hash vs contentHash (iter 44+):** Shell hash `69d9b813` = SHA-256 of `src/shell/index.html` ONLY. contentHash = SHA-256 of combined `src/master/*.html`: `58f4daa85c05e070` → `34c34a7d9839c11d` (iter 44) → `665cede798c34fc0` (iter 45). **Shell hash unchanged ≠ master HTML unchanged.** (11) **Callout class policy (iter 45+ NEW):** разрешены только `.callout.rule/.rec/.ex` и plain `.callout`. Запрещены `.callout.note/.info/.warn/.tip/.box/.sidebar/.custom/.important` (см. `scripts/validate-master.mjs` L452-463). Для «Примечание»/«Методология»/etc — используй plain `<p><strong>Примечание:</strong>...</p>` без callout wrapper (см. precedent `part_07a.html` L754). Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 таблицу с sub-ID (A11, B7, и т.д.) и пометкой P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID, аудит переоценил, examples в пределах лимита 120 токенов).
