...
| `visual-system/` | Visual system prototypes: `PLAN.md`, `DESIGN-TOKENS.css`, `shared/`, `elements/` (E02–E07, E09, E11–E14, E16–E18 — E01/E08/E10/E15 → `REMOVED_WITH_REASON` iter 138, DEC-19), `hero/` → `REMOVED_WITH_REASON` iter 140 (DEC-19 §6.4 disposal; archive = git history), `integration/` (component-extracts). `tokens.json` feeds `qa:contrast`. | **Frozen design artifacts (DEC-19, iter 138):** master embed (L2) = the canonical visual markup; archive = git history; remaining families ride the freeze at their slice time. |
