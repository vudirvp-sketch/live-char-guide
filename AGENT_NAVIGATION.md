...
**iter 19+ — что осталось:**
- **iter 19+:** KI#13 (123 inline styles) + KI#16 (qa:csp FAIL: 2 inline scripts в `src/shell/index.html`) + Phase 4 actual SVG integration — после Canon complete (iter 18 done).
...
**Подсказка следующему агенту:** Перед стартом iter 19+ прочитай `STATUS.md` (iter 18 COMPLETE, все 10 Parts + 3 Appendix ✅ MIGRATED, Canon COMPLETE, KI#13+KI#16+KI#17 ACTIVE — KI#14 closed iter 16), `worklog.md` (iter 18 record), этот файл (AGENT_NAVIGATION §8 iter 19+ roadmap), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 19 priorities). iter 19+ priorities: KI#13 (123 inline styles → external CSS classes) + KI#16 (qa:csp FAIL: 2 inline scripts в `src/shell/index.html` → external widget) + Phase 4 actual SVG integration — см. `_README.md` §5. Принцип «viz > dry text» (iter 8) — сохраняется. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue, потом фиксий.
