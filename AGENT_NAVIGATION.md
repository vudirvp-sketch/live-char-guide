...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 3 Appendix ✅ MIGRATED (iter 18) + KI#13 ✅ **CLOSED** (iter 24, 123/123 = 100%) + Phase 4 SVG integration: E18 Greeting Algorithm ✅ INTEGRATED (iter 25) + **Deployed Guide Audit (DGA) Phase 1 — STARTED (iter 26, KI#18 🟡 ACTIVE: 1/8 fixed, 7 pending iter 27+)** + KI#14/#16/#17 ✅ CLOSED. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, техническая архитектура — в `docs/architecture.md`.
...
**Подсказка следующему агенту:** Перед стартом iter 27+ прочитай `STATUS.md` (iter 26 COMPLETE — DGA Phase 1 STARTED, KI#18 🟡 ACTIVE 1/8 fixed A, 7 pending B–H, все previous KI#1..#17 ✅ CLOSED), `worklog.md` (iter 26 record — самый подробный), этот файл (AGENT_NAVIGATION §6 pitfall #37 KI#18 🟡 ACTIVE, §8 iter 27+ roadmap — DGA Phase 2: fix KI#18-B Part 1 + KI#18-C Part 2 first, then E + F, G+H likely keep-by-design), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 26 → DGA Phase 1 STARTED, §8 iter 26 stop point). Принцип `viz > dry text` (iter 8+) — сохраняется. Unique контент не удаляется даже при дублировании. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий.
