...
| `docs/research/v2_architecture.md` | **The consolidated v2 architecture specification (iter 146, P-a per DEC-23) — PROPOSED for owner ratification (STATUS Next-step row 1 / PLAN row v2-ratify).** Layer model L1–L11 (post-extraction) · reader-mode spec (F1: Learn/Build/Debug/Reference — entries, visibility, disclosure; overlays, not four sites) · presentation policy (F2 dispositions · F3 checklist triage · F4 example tiering · F5 Part micro-template · F6 `<details class="interactive">`) · rule-strength (DEC-20 + iter-145 OBSERVATION) · 7/7 cluster+gate inventory · v2 build plan (per-Part slices, Part 1 first proposed) · audit & switch plan · 9 ratification questions Q1–Q9. Nothing DECIDED inside; no v2 build slice before ratification. |
