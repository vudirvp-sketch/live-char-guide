...
> **Entry document.** Read this first. Текущая версия: **9.2.0** + все 10 Parts + 4 Appendix + Part 0 ✅ MIGRATED + **iter 63: A59-1 Neuroticism→stress type taxonomy (§5.1 subsection, §5.2 Elena profile, §5.5 validation table, ocean.json) + A59-3 Personality sub-budget (§7A.12 subsection, character_schema.json). 96 sections unchanged. All validation gates PASS (96/96 sync, 96 sections build, terms ✅, duplicates ✅, 24 English leaks baseline)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, Canon (источник правды) — в `docs/canon/`.
