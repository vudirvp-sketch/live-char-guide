...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 3 Appendix ✅ MIGRATED (iter 18) + KI#13 ✅ **CLOSED** (iter 24, 123/123 = 100%) + Phase 4 SVG integration: E18 Greeting Algorithm ✅ INTEGRATED (iter 25) + Deployed Guide Audit (DGA) Phase 1 — STARTED (iter 26, KI#18-A ✅) + DGA Phase 2 (iter 28, KI#18-B ✅ + KI#18-C ✅, KI#18-I NEW) + KI#14/#16/#17 ✅ CLOSED. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, техническая архитектура — в `docs/architecture.md`.
...
**iter 28 (DGA Phase 2 — 2 fixes applied, 1 new bug documented):** Применены 2 safe fix из 7 pending sub-items KI#18: **KI#18-B** (Part 1 p1_card_overview table) — drop duplicate «Функция» column (повторял E01 `.block-content`), add intro paragraph linking to E01 viz (pattern KI#18-A from Part 9). Table: 4 cols → 3 cols (Блок / Влияние на модель / Доля бюджета — both unique). part_01: 365 → 367 (+1). **KI#18-C** (Part 2 p2_basic_anchors table) — drop duplicate «Описание» column (повторял E03 `flow-node__desc`), expand intro paragraph linking to E03. Table: 4 cols → 3 cols (# / Этап / Пример — examples unique). part_02: 415 → 415 (0 net — intro line replaced, 3 rows shortened by 1 cell each). Canon `docs/canon/part_01.md` + `docs/canon/part_02.md` updated (front-matter + table rewritten + migration history rows + DGA Phase 2 fix section + validation gates). **KI#18-I NEW** (Part 2 p2_embodiment table «Описание» col duplicates E04 viz — same pattern as C) — found during KI#18-C analysis, documented in STATUS.md §Known Issues, NOT fixed in iter 28 (pending iter 29+). Per user instruction «Если найден новый баг — сначала документируй в STATUS.md как Known Issue, потом фиксий». `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged. **KI#18 🟡 ACTIVE — 3/9 fixed (A+B+C), 6 pending (D, E, F, G, H, I) iter 29+.**
...
**Подсказка следующему агенту:** Перед стартом iter 29+ прочитай `STATUS.md` (iter 28 DGA Phase 2 COMPLETE — KI#18-B + KI#18-C ✅ FIXED, KI#18-I NEW documented; iter 27 status check; iter 26 COMPLETE — DGA Phase 1 STARTED, KI#18 🟡 ACTIVE 3/9 fixed A+B+C, 6 pending D+E+F+G+H+I, все previous KI#1..#17 ✅ CLOSED), `worklog.md` (iter 28 record — DGA Phase 2; iter 27 record — status check; iter 26 record — DGA Phase 1), этот файл (AGENT_NAVIGATION §6 pitfall #37 KI#18 🟡 ACTIVE 3/9 fixed, §8 iter 29+ roadmap — DGA Phase 2 continued: priority I + F, then D + E, G+H likely keep-by-design), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 26 → DGA Phase 1 STARTED). Принцип `viz > dry text` (iter 8+) — сохраняется. Unique контент не удаляется даже при дублировании. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий.
