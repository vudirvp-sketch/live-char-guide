...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 4 Appendix + Part 0 (concept) ✅ MIGRATED/ADDED (iter 18 + iter 38) + KI#13/#18/#20/#21/#22/#23/#32/#33/#34/#35 ✅ CLOSED + iter 34-49 + **iter 50 — KI#34 ✅ CLOSED (§1.8 Pre-build checklist section added to `src/master/part_01.html` + 3 regression checks) + KI#35 ✅ CLOSED (`p4_spine_overview` canon metadata added); все validation gates PASS (build hash `69d9b813` + contentHash `cc130a527480e61b` 5th change since iter 34); sectionCount 98 → 99; audit_canon_master_sync.py 92/92 PASS (was 89/89); drift detector: 0 master-only sections (was 1), 3 canon-only by design (was 4); 10 unmerged dependabot branches (informational)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 0 (concept) → Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, **аудит канона + fix plan (iter 33+) — в `docs/AUDIT_VERIFICATION.md` (P0+P1+P2+P3 ✅ CLOSED iter 35-38, KI#21 ✅ CLOSED canon; 57/57 master sync iter 44+45+46+47 + 1 new section iter 50)**, техническая архитектура — в `docs/architecture.md`.
...
| 50 | **KI#34 + KI#35 ✅ CLOSED — все MEDIUM/HIGH priority KI закрыты. Проект STABLE.** MEDIUM-priority KI#34 fix: добавлен `<section data-section="p1_prebuild_checklist" data-toc-nav>` block в `src/master/part_01.html` (L368-395, +28 строк) после p1_top3_problems с контентом из canon `docs/canon/part_01.md` L128-145 (6-row table: Размер модели/Контекстное окно/Сложность персонажа/GHOST/CoT/Lorebook + RECOMMENDATION callout + Cross-ref). 9 anchor refs resolve к existing section IDs (`#appendix_model_table`, `#p7a_token_budget`, `#p10_walter/elena/omnis/vysherblenny`, `#p4_ghost_layers`, `#p6_cot_basics`, `#p7b_lorebook_basics`); canon-only refs (`part_00 §0.2`, `appendix_character_map.md`) — как `<code>...</code>` plain text per KI#31-7a precedent. LOW-priority KI#35 fix: `` `data-section: p4_spine_overview` `` line added в `docs/canon/part_04.md` после `## 4.1 SPINE Overview` (cosmetic canon metadata). Regression test `audit_canon_master_sync.py` extended с 89 → **92 checks** (3 new positive checks: `KI#34-section`, `KI#34-table`, `KI#34-callout`). **contentHash: `84d69ecffca28cbf` → `cc130a527480e61b` (5th change since iter 34).** sectionCount: 98 → 99. Shell hash `69d9b813` unchanged. Все validation gates PASS + `audit_canon_master_sync.py` 92/92 PASS + `audit_canon_master_drift.py` informational (Master-only sections 1→0, canon-only actionable 1→0; heading mismatches 14→15 expected, content hash diffs 96→98 expected). Документация актуализирована: `STATUS.md`, `worklog.md`, `AGENT_NAVIGATION.md`, `ITER49_README.md` → `ITER50_README.md`, stale `_ITER49_MERGE_INSTRUCTIONS.txt` удалён. | KI#34 ✅, KI#35 ✅ |

**iter 51+ — что осталось (LOW priority only, проект STABLE):**
...
**Подсказка следующему агенту:** Перед стартом iter 51+ прочитай `STATUS.md` (iter 50 — KI#34 + KI#35 ✅ CLOSED, все MEDIUM/HIGH priority KI закрыты, проект STABLE; iter 49 one-paragraph — RECONNAISSANCE ONLY; iter 48 one-paragraph — drift detector added; iter 47 one-paragraph — KI#33 ✅ CLOSED 57/57; iter 34-46 one-liners; все previous KI#1..#35 ✅ CLOSED/VERIFIED), `worklog.md` (iter 50 record — самый подробный; iter 49 one-liner), этот файл (AGENT_NAVIGATION §2a Deployment Pipeline iter 43+, §8 OP-1 iter 50 row + iter 51+ roadmap — только LOW priority задачи), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 4 Appendix + Part 0 concept ✅, Canon COMPLETE; §3.9 callout labels policy). **Приоритет iter 51+: только LOW priority** — semantic paragraph-level drift detection (extension of drift detector), Glossary double-render (by design), Component extracts regeneration (опционально), Dependabot merges (GitHub-level). Если новых багов нет — проект STABLE. **Invariants (iter 50+):** (1) `viz > dry text` (iter 8+). (2) VS scroll-animation — `python3 scripts/audit_vs_embeds.py` (0 regressions expected; pre-existing path issue — symlink workaround `ln -sfn /path/to/repo /home/z/my-project/work/live-char-guide`). (3) Component extracts drift (iter 42+). (4) CSS scoping (iter 34+). (5) YAML front-matter (iter 37+). (6) **Guide's role as example takes priority over character canon (iter 39+).** (7) **OCEAN labeling consistency (iter 40+, расширен iter 41; master sync ✅ iter 44).** (8) **Bible ↔ canon cross-ref symmetry (iter 41+; master sync ✅ iter 44).** (9) **Canon → master HTML sync (iter 43+ invariant, iter 50 ✅ COMPLETE 58/58):** `docs/canon/*.md` = source of truth для content. `src/master/*.html` = production HTML, деплоится на сайт. Regression test `python3 scripts/audit_canon_master_sync.py` (92/92 PASS — 74 positive + 18 negative). (10) **Drift detector (iter 48+ invariant, iter 50 ✅ actionable clean):** `python3 scripts/audit_canon_master_drift.py` — informational structural comparison canon ↔ master HTML (section IDs + headings + content hashes). Exit 0 всегда. JSON baseline: `build/drift-report-iter48.json`. iter 50: 0 master-only sections, 3 canon-only by design. Heading mismatches (15, by design) + content hash diffs (98, informational) — expected. (11) **Build hash vs contentHash (iter 44+):** Shell hash `69d9b813` = SHA-256 of `src/shell/index.html` ONLY. contentHash = SHA-256 of combined `src/master/*.html`: `58f4daa85c05e070` → `34c34a7d9839c11d` (iter 44) → `665cede798c34fc0` (iter 45) → `d2fdafeaf093dd80` (iter 46) → `84d69ecffca28cbf` (iter 47-49) → `cc130a527480e61b` (iter 50, 5th change). **Shell hash unchanged ≠ master HTML unchanged.** (12) **Callout class policy (iter 45+ invariant):** разрешены только `.callout.rule/.rec/.ex` и plain `.callout`. Запрещены `.callout.note/.info/.warn/.tip/.box/.sidebar/.custom/.important` (см. `scripts/validate-master.mjs` L452-463). Для «Примечание»/«Методология»/«Annotation»/«Demonstrates»/«Synthesis»/etc — используй plain `<p><strong>Label:</strong>...</p>` без callout wrapper (см. precedent `part_07a.html` L754 для «Примечание», `part_10.html` для «Annotation»/«Demonstrates»). Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 таблицу с sub-ID (A11, B7, и т.д.) и пометкой P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID, аудит переоценил, examples в пределах лимита 120 токенов).
