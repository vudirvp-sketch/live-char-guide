...
| `docs/research/editorial_matrix.md` | Editorial matrix (ed-matrix Phases A+B — full guide coverage): block-level audit of Parts 0–10 + 4 appendices — 99 canon-declared section IDs, **504 rows** (the iter-126 audit's 500 + 4 post-audit row additions: iter 139/142/147's rows + the iter-159 Appendix B НАБЛЮДЕНИЕ row) with machine-checkable `<data-section>::<NN>` IDs, repeat registry R01–R29 (§24 classes). Evidence base for ed-1…ed-8 + dupes-1; KI#71/KI#72 recorded from it. The F2 `presentation` column (DEC-24) is seeded per Part/appendix at its build slice (Parts 1–10 + Appendices A/B: iters 147–159). |
