...
- **iter 23:** KI#13 Part 7A (19 inline styles → CSS classes). Largest remaining batch — Part 7A имеет 13 секций + 4 VS-EMBED (E08/E16/E17/E02).
- **iter 24:** KI#13 Part 9 (6) + Part 10 (12) = 18 inline styles. Part 10 имеет 11 positional `top:Npx; right:20px;` styles — кандидат на 1 generic class + per-instance modifier или single selector per position.
...
**Подсказка следующему агенту:** Перед стартом iter 23+ прочитай `STATUS.md` (iter 22 COMPLETE, KI#13 86/123 fixed = 70%, 37 remaining), `worklog.md` (iter 22 record), этот файл (AGENT_NAVIGATION §6 pitfall #36 KI#13 ACTIVE, §8 iter 23+ roadmap), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 23 priorities + §8 Phase 4 analysis). iter 23+ priorities: KI#13 Part 7A (19 inline styles → external CSS classes) — largest remaining batch, Part 7A имеет 13 секций + 4 VS-EMBED (E08/E16/E17/E02). Принцип «viz > dry text» (iter 8) — сохраняется. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue, потом фиксий.
