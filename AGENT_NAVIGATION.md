...
> **Entry document.** Read this first. Текущая версия: **9.2.0** (docs + code + source-side fully synced) + все 10 Parts + 4 Appendix + Part 0 ✅ MIGRATED + **iter 72: KI#49 ✅ CLOSED (`AGENT_NAVIGATION.md:11` "97 секций" → "96 секций" off-by-one после iter 62 MBTI stub merge) + Scenario labels §9.9/§9.10 extension (iter 55/57 pattern → §9.9 test_requirements + §9.10 12b_issues — последние чек-листы Part 9 без scenario-меток; 7/11 секций Part 9 покрыты, остальные 4 reference/overview by design без меток)**. Все validation gates PASS (version-sync ✅, 96/96 sync, 24 English leaks baseline, terms ✅, duplicates ✅, audit_vs_embeds ✅, validate:master 12/12 ✅, drift 170/131 baseline, build hash `4074bac5` unchanged)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, Canon (источник правды) — в `docs/canon/`.
...
**iter 73+ — что осталось (LOW priority only, проект STABLE):**
...
- **DONE (iter 57) — Annotation blocks §10.2-§10.4:** Walter, Omnis, Vysherblenny — все 3 карточки имеют `<strong>Annotation:</strong>` блок с `<ul>` списком Demonstrates-ссылок.
- **DONE (iter 55/57/72) — Scenario labels Part 9:** iter 55 (§9.3 pattern), iter 57 (§9.5/§9.6/§9.7/§9.11 extension), iter 72 (§9.9/§9.10 extension). Покрыты 7 из 11 секций Part 9 (4 reference/overview секции без меток by design).
...
**Подсказка следующему агенту:** Перед стартом iter 73+ прочитай `STATUS.md` (iter 72 — KI#49 ✅ CLOSED, Scenario labels §9.9/§9.10 extension done, проект STABLE), `worklog.md` (iter 72 record — самый подробный), этот файл (§8 OP-1 iter milestones + iter 73+ roadmap), `docs/canon/_README.md` (§5 migration status, §3.9 callout labels policy). **Приоритет iter 73+:** LOW/P2 опционально — Recon (LOW), `src/master/VERSION` orphan deletion (KI#46 follow-up, LOW, risk-free), Component extracts regeneration (LOW, опционально), canonical-location-маркер (P2 MEDIUM, ~150 правок, отложен — требует контент-ревью), Dependabot merges (GitHub-level), Glossary double-render (by design), Paragraph drift tuning (170 drifts, false positives нет). Annotation blocks §10.2-10.4 и scenario labels Part 9 — DONE (iter 57 + iter 72). Если новых багов нет — проект STABLE. **Invariants (iter 72+):** (1) `viz > dry text` (iter 8+). (2) VS scroll-animation — `python3 scripts/audit_vs_embeds.py` (KI#44 ✅ CLOSED iter 68, работает без symlink workaround). (3) Component extracts drift (iter 42+). (4) CSS scoping (iter 34+). (5) YAML front-matter (iter 37+). (6) Guide's role as example takes priority over character canon (iter 39+). (7) OCEAN labeling consistency (iter 40+). (8) Bible ↔ canon cross-ref symmetry (iter 41+). (9) **Anchor navigation (iter 51+, KI#36 ✅ CLOSED):** все `<section data-section="X">` имеют `id="X"`. (10) **Canon → master HTML sync (iter 43+, 96/96 PASS):** `docs/canon/*.md` = source of truth. `src/master/*.html` = production HTML. (11) **Drift detector (iter 48+, v1.3):** `audit_canon_master_drift.py` — informational, exit 0. (12) **Build hash `4074bac5` (unchanged iter 72 — content additions в master HTML не влияют на hash, hash computed only from `src/shell/index.html`)**. (13) **Callout class policy (iter 45+):** разрешены `.callout.rule/.rec/.ex` и plain `.callout`. (14) **Recap-чек-листы в `<details>` (iter 55+ invariant).** (15) **Cat B prose mentions (iter 67+):** «Russian (English)» форма, не «English (Russian)». (16) **Version sync (iter 71+):** все 9.1.0 references устранены — `src/VERSION`, `src/master/VERSION`, `package.json`, `data/*.json`, `src/shell/*`, `scripts/*`, `docs/*.md`, `src/master/*.html` top comments все говорят 9.2.0. (17) **Progressive disclosure (iter 71+):** 96/96 секций покрыты `<!-- difficulty: BASIC | INTERMEDIATE | EXPERT -->` маркерами. (18) **Section count (iter 72+):** 96 секций в `src/master/` (verified: `grep -cE '<section[^>]*data-section' src/master/*.html` = 96) — AGENT_NAVIGATION.md:11 синхронизировано. (19) **Scenario labels Part 9 (iter 55/57/72):** 7/11 секций покрыты `<strong>Сценарий:</strong>` маркером (§9.3, §9.5, §9.6, §9.7, §9.9, §9.10, §9.11). (20) **Annotation blocks §10.2-10.4 (iter 57):** Walter, Omnis, Vysherblenny — все 3 карточки имеют `<strong>Annotation:</strong>` блок с `<ul>` списком Demonstrates-ссылок. Если найден новый баг — сначала документируй в `STATUS.md` как KI#N, потом фиксий.
