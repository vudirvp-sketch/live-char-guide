...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 4 Appendix + Part 0 (concept) ✅ MIGRATED/ADDED (iter 18 + iter 38) + iter 34-53 + **iter 54 — исследовательская итерация: разбор сводного аудита гайда. Создан `docs/AUDIT_REVIEW_ITER54.md` (~440 строк, 12 секций) с декомпозицией аудита и вердиктами по 22 утверждениям (0% верны, 59% частично верны, 18% неверны, 23% субъективны). Найдены 3 реальных LOW-бага (KI#37/38/39, 🟡 OPEN). Никаких правок гайда не производилось. Все validation gates PASS (build hash `69d9b813` unchanged, contentHash UNCHANGED — только docs + STATUS/worklog/AGENT_NAVIGATION изменены); audit_canon_master_sync.py 96/96 PASS; drift detector v1.2: 0 master-only, 3 canon-only by design, 88 paragraph drifts (informational); English leaks 20 baseline; 10 unmerged dependabot branches (informational)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 0 (concept) → Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, **аудит канона (iter 33+) — в `docs/AUDIT_VERIFICATION.md` (P0+P1+P2+P3 ✅ CLOSED iter 35-38, KI#21 ✅ CLOSED; 57/57 master sync iter 44+45+46+47 + 1 new section iter 50 + 98 id attrs iter 51)**, **разбор сводного аудита (iter 54) — в `docs/AUDIT_REVIEW_ITER54.md` (22 утверждения, 3 новых KI#37-39 OPEN)**, техническая архитектура — в `docs/architecture.md`.
...
**iter 54+ — что осталось (LOW priority only, проект STABLE):**
...
- **LOW priority — Paragraph drift tuning (опционально, recon iter 53):** iter 53 recon подтвердил: 88 drifts — real semantic differences, false positives нет. Threshold 0.3 / MIN_PARAGRAPH_LENGTH 30 остаются без изменений. Если в будущем появятся новые drifts после canon/master edits — можно повторить recon с `--json` и проверить category breakdown (новые `plain_text` drifts — candidates для investigation).
...
**Подсказка следующему агенту:** Перед стартом iter 54+ прочитай `STATUS.md` (iter 53 — drift categorization added, все HIGH/MEDIUM priority KI закрыты, проект STABLE), `worklog.md` (iter 53 record — самый подробный; iter 52 one-liner), этот файл (§8 OP-1 iter 52+53 row + iter 54+ roadmap — только LOW priority задачи), `docs/canon/_README.md` (§5 migration status, §3.9 callout labels policy). **Приоритет iter 54+: только LOW priority** — Glossary double-render (by design), Component extracts regeneration (опционально, no business value), Dependabot merges (GitHub-level), Paragraph drift tuning (опционально, recon iter 53 — false positives нет). Если новых багов нет — проект STABLE. **Invariants (iter 53+):** (1) `viz > dry text` (iter 8+). (2) VS scroll-animation — `python3 scripts/audit_vs_embeds.py` (symlink workaround). (3) Component extracts drift (iter 42+). (4) CSS scoping (iter 34+). (5) YAML front-matter (iter 37+). (6) Guide's role as example takes priority over character canon (iter 39+). (7) OCEAN labeling consistency (iter 40+). (8) Bible ↔ canon cross-ref symmetry (iter 41+). (9) **Anchor navigation (iter 51+, KI#36 ✅ CLOSED):** все `<section data-section="X">` имеют `id="X"`. `lazy-loader.js` селектор `$$('section[data-section]')`. `hashchange` listener. Glossary panel auto-close on anchor click. Regression test `audit_canon_master_sync.py` 96/96 PASS (+4 KI#36 id checks). `verify_anchors.py` — 96/96 anchors resolve. (10) **Canon → master HTML sync (iter 43+ invariant, iter 50 ✅ COMPLETE 58/58, iter 51 ✅ +98 id attrs):** `docs/canon/*.md` = source of truth. `src/master/*.html` = production HTML. Regression test `audit_canon_master_sync.py` (96/96 PASS — 78 positive + 18 negative). (11) **Drift detector (iter 48+ invariant, iter 52 ✅ paragraph-level, iter 53 ✅ categories):** `audit_canon_master_drift.py` v1.2 — informational, exit 0. iter 53: 0 master-only, 3 canon-only by design, 88 paragraph drifts (15 vs_embed_ref + 14 cross_ref + 4 callout_label + 2 no_master_match + 53 plain_text). (12) **Build hash vs contentHash (iter 44+):** Shell hash `69d9b813` = SHA-256 of `src/shell/index.html` ONLY (unchanged iter 53). contentHash = SHA-256 of combined `src/master/*.html`: iter 51 = 6th change, iter 52+53 UNCHANGED. **Shell hash unchanged ≠ master HTML unchanged.** (13) **Callout class policy (iter 45+):** разрешены `.callout.rule/.rec/.ex` и plain `.callout`. Запрещены `.callout.note/.info/.warn/.tip/.box/.sidebar/.custom/.important`. Если найден новый баг — сначала документируй в `STATUS.md` как KI#N, потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 с sub-ID (A11, B7) и P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID).
