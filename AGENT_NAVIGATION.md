...
- **iter 25 (optional, low priority):** Phase 4 SVG integration — Part 7B Greeting algorithm infographic → new VS element E18 (requires `visual-system/elements/` prototyping first). Создание нового VS element — отдельная задача, не блокирует ничего.
...
**Подсказка следующему агенту:** Перед стартом iter 25+ прочитай `STATUS.md` (iter 24 COMPLETE, KI#13 ✅ CLOSED 123/123 = 100%, все KI#1..#17 ✅ CLOSED, active KI нет), `worklog.md` (iter 24 record), этот файл (AGENT_NAVIGATION §6 pitfall #36 KI#13 ✅ CLOSED, §8 iter 25+ roadmap — только optional Phase 4 SVG), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 25 optional + §8 Phase 4 analysis). iter 25+ (optional, low priority): Phase 4 SVG integration — Part 7B Greeting algorithm → new VS element E18 (requires `visual-system/elements/` prototyping first). Принцип «viz > dry text» (iter 8) — сохраняется. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue, потом фиксий.
