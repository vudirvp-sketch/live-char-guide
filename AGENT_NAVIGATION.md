...
**iter 42+ — что осталось:**
- **KI#30/#31 ✅ CLOSED (iter 41).** OCEAN labeling leftover + Part 10/7A ↔ bible cross-ref устранены.
...
**Подсказка следующему агенту:** Перед стартом iter 42+ прочитай `STATUS.md` (iter 41 — OCEAN LABELING LEFTOVER + BIBLE CROSS-REF ✅ COMPLETE, KI#30/#31 ✅ CLOSED, build hash `69d9b813`; iter 40 one-paragraph — KI#28/#29 ✅ CLOSED; iter 39 one-paragraph — KI#25/#26/#27 ✅ CLOSED; iter 38 one-paragraph — KI#21 P3 ✅ CLOSED полностью; iter 34-37 one-liners — CSS/CSP + KI#21 P0/P1/P2; все previous KI#1..#29 ✅ CLOSED/VERIFIED), `worklog.md` (iter 41 record — самый подробный; iter 40 one-liner), этот файл (AGENT_NAVIGATION §6 pitfall #40 KI#21 ✅ CLOSED полностью, §6 pitfall #41 doc drift invariant iter 39+, §8 OP-1 iter 41 row + iter 42+ roadmap — только minor задачи: Glossary double-render LOW, Component extracts sync MEDIUM), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 4 Appendix + Part 0 concept ✅, Canon COMPLETE; §3.9 callout labels policy — English by design + 2 метки Demonstrates/Annotation iter 38). **Invariants (iter 41+):** (1) `viz > dry text` (iter 8+) — сохраняется. (2) VS scroll-animation — `python3 scripts/audit_vs_embeds.py` (0 regressions expected). (3) CSS scoping (iter 34+) — VS-EMBED selectors scoped к element-specific parent. (4) YAML front-matter (iter 37+) — все canon-файлы используют YAML front-matter. (5) **Guide's role as example takes priority over character canon (iter 39+):** при рассинхроне bible vs canon Part 10 — правится bible, не Part 10. (6) **OCEAN labeling consistency (iter 40+, расширен iter 41):** extreme = строго `<30` или `>70` per Part 5 §5.1 RULE; cautious zone = `30–40` / `60–70`. Label-only fixes допустимы для internal canon consistency — values примера не трогаются. Все canon locations с OCEAN labels проверены iter 41 (part_07a L415, part_10 L51/L148/L254/L408, appendix_character_map). (7) **Bible ↔ canon cross-ref symmetry (iter 41+):** bible Note → Part 10 §10.4 + Part 7A §7A.9 (iter 39 KI#26); reverse Cross-ref Note → bible (iter 41 KI#31). (8) Build hash baseline `69d9b813` (unchanged после iter 34 — KI#23 fix; canon + doc файлы НЕ входят в hash computation — только `src/master/`, `src/shell/`, `src/assets/`, `data/`, `parts/` root fallbacks). Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 таблицу с sub-ID (A11, B7, и т.д.) и пометкой P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID, аудит переоценил, examples в пределах лимита 120 токенов).
