...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 3 Appendix ✅ MIGRATED (iter 18) + KI#13 ✅ **CLOSED** (iter 24, 123/123 = 100%) + Phase 4 SVG integration: E18 ✅ INTEGRATED (iter 25) + Deployed Guide Audit (DGA) ✅ **COMPLETE** (iter 26-31, KI#18 ✅ CLOSED 9/9) + **KI#20 ✅ CLOSED (iter 32 — Visual System Scroll-Animation Bug: vs-scroll-observer.js selector extended for 8 animation classes, 5 VS-EMBED elements fixed)** + KI#14/#16/#17/#19 ✅ CLOSED. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, техническая архитектура — в `docs/architecture.md`.
...
- **Все Known Issues (KI#1..KI#20) ✅ CLOSED.** iter 33+ roadmap: **none planned**.
...
**Подсказка следующему агенту:** Перед стартом iter 33+ прочитай `STATUS.md` (iter 32 — KI#20 Visual System Scroll-Animation Bug ✅ CLOSED: 5/5 sub-items fixed single-file edit'ом `vs-scroll-observer.js`; все previous KI#1..#19 ✅ CLOSED; iter 33+ roadmap — none planned, all KIs CLOSED), `worklog.md` (iter 32 record — самый подробный; iter 31 one-liner; iter 30 one-liner; iter 26 one-liner — DGA Phase 1), этот файл (AGENT_NAVIGATION §6 pitfall #39 KI#20 ✅ CLOSED iter 32, §8 iter 33+ roadmap — none planned, all KIs CLOSED), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 32 → KI#20 CLOSED, §8 iter 32 stop point). Принцип `viz > dry text` (iter 8+) — сохраняется. VS scroll-animation invariant — `python3 scripts/audit_vs_embeds.py` (0 regressions expected). Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий.
