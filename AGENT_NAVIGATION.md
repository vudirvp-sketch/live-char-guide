...
**iter 39+ — что осталось:**
- **KI#21 ✅ CLOSED полностью (57/57 правок — P0+P1+P2+P3).** Все противоречия в каноне устранены. iter 39+ roadmap — none planned.
- **Потенциальные minor задачи (не критично, не запланировано):** Glossary double-render inefficiency, Component extracts sync (visual-system/integration/component-extracts/) — documentation drift.
...
**Подсказка следующему агенту:** Перед стартом iter 39+ прочитай `STATUS.md` (iter 38 — KI#21 ✅ CLOSED полностью 57/57, +2 новых файла, build hash `69d9b813`; iter 37 one-paragraph — canon P2; iter 36 one-paragraph — canon P1; iter 35 one-paragraph — canon P0; iter 34 one-paragraph — CSS/CSP fix; iter 33 one-liner — audit verified; все previous KI#1..#24 ✅ CLOSED/VERIFIED), `worklog.md` (iter 38 record — самый подробный; iter 37 one-liner), этот файл (AGENT_NAVIGATION §6 pitfall #40 KI#21 ✅ CLOSED полностью, §8 OP-1 iter 38 row + iter 39+ roadmap — none planned, только minor задачи), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 4 Appendix + Part 0 concept ✅, Canon COMPLETE; §3.9 callout labels policy — English by design + 2 новые метки Demonstrates/Annotation iter 38). Принцип `viz > dry text` (iter 8+) — сохраняется. VS scroll-animation invariant — `python3 scripts/audit_vs_embeds.py` (0 regressions expected). CSS scoping invariant (iter 34+) — VS-EMBED selectors scoped к element-specific parent (не generic). YAML front-matter (iter 37+) — все canon-файлы используют YAML front-matter, markdown quote-block deprecated. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 таблицу с sub-ID (A11, B7, и т.д.) и пометкой P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID, аудит переоценил, examples в пределах лимита 120 токенов).
