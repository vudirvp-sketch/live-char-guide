...
> **Entry document.** Read this first. Текущая версия: **9.2.0** (docs) / **9.1.0** (code, см. KI#45) + все 10 Parts + 4 Appendix + Part 0 ✅ MIGRATED + **iter 68: Recon + KI#44 fix (audit_vs_embeds.py path bug) + cleanup _DELETED_FILES.txt. iter 60–68 plan COMPLETE. 96 sections unchanged. All validation gates PASS (96/96 sync, 24 English leaks baseline, terms ✅, duplicates ✅, audit_vs_embeds ✅ no symlink workaround needed)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, Canon (источник правды) — в `docs/canon/`.
