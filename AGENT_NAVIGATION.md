...
> **Entry document.** Read this first. Текущая версия: **9.2.0** (docs + code + source-side fully synced) + все 10 Parts + 4 Appendix + Part 0 ✅ MIGRATED + **iter 73: Research — консолидация `guide_analysis.md` (1774 строк, 7 анализов) в `docs/research/guide_analysis_consolidated.md` (12 разделов, 15 противоречий C1–C15, 20 дублей D1–D20, 9 assertions V1–V9 для верификации в iter 74) + план `docs/research/research_plan.md` (Фазы 1–5: Recon → P1 критичные → P2 структура → P3 опционально → отложенные). Никаких правок контента гайда в iter 73 — чисто исследовательская, подготовительная. Все validation gates PASS (без изменений — iter 73 не трогал src/ или data/), build hash `4074bac5` unchanged. Предыдущие: iter 72 KI#49 ✅ CLOSED + Scenario labels §9.9/§9.10 extension; iter 71 KI#48 + progressive disclosure 96/96; iter 70 KI#46/KI#47 + docs version bump.** Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, Canon (источник правды) — в `docs/canon/`, Research — в `docs/research/`.
...
| `docs/research/guide_analysis_consolidated.md` | Консолидированный анализ гайда из внешних источников (iter 73). 12 разделов: противоречия, дубли, чек-листы, приоритизированные предложения. |
| `docs/research/research_plan.md` | План верификации и правок на основе `guide_analysis_consolidated.md` (iter 74+). Фазы 1–5. |
...
| 72 | KI#49 (`AGENT_NAVIGATION.md:11` "97 секций" → "96 секций" off-by-one после iter 62) + Scenario labels §9.9/§9.10 extension (iter 55/57 pattern → §9.9 test_requirements + §9.10 12b_issues) | KI#49 ✅ |
| 73 | Research: консолидация `guide_analysis.md` (1774 строк, 7 анализов) → `docs/research/guide_analysis_consolidated.md` (12 разделов, 15 противоречий C1–C15, 20 дублей D1–D20, 9 assertions V1–V9 для верификации) + план `docs/research/research_plan.md` (Фазы 1–5). Никаких правок контента гайда. | — |

**iter 74+ — что осталось (после iter 73 Research):**
- **iter 74 — Recon & Verification (HIGH priority):** проверить 9 утверждений V1–V9 из `docs/research/guide_analysis_consolidated.md` (см. STATUS.md §Pending Verification). По результатам — открыть KI#50+ если баги подтверждены.
- **iter 75–82 — P1 критичные исправления (MEDIUM):** устранение мёртвых ссылок, fix конфликтов стресса, уточнение CoT Tier 0 / OCEAN-in-Description / Anchors placement / Voice Isolation / Price в примерах. План в `docs/research/research_plan.md` §Фаза 2.
- **iter 83–90 — P2 улучшение структуры (HIGH):** унификация терминологии Anchors, merge Voice разделов, Token Budget single source of truth, устранение дублей Anti-godmoding/Price, добавить Assembly Pipeline Checklist + Master Checklist. План в `docs/research/research_plan.md` §Фаза 3.
- **iter 91+ — P3 опциональные (LOW–MEDIUM):** карта зависимостей, схемы Mermaid, замена процентов, SysPrompt, API blocks → Appendix. План в `docs/research/research_plan.md` §Фаза 4.
- **LOW priority — Glossary double-render inefficiency:** structural, by design.
- **LOW priority — Component extracts regeneration (опционально):** regenerate 54 файла from master.
...
- **Опционально — `src/master/VERSION` orphan file deletion:** KI#46 follow-up, risk-free.
- **DONE (iter 57) — Annotation blocks §10.2-§10.4:** Walter, Omnis, Vysherblenny.
- **DONE (iter 55/57/72) — Scenario labels Part 9:** 7 из 11 секций покрыты.
- **DONE (iter 73) — Research: консолидация `guide_analysis.md`:** `docs/research/guide_analysis_consolidated.md` + `docs/research/research_plan.md`.
...
**Подсказка следующему агенту:** Перед стартом iter 74+ прочитай `STATUS.md` (iter 73 — Research complete, 9 assertions V1–V9 для верификации в §Pending Verification), `worklog.md`, этот файл (§8 OP-1 iter milestones + iter 74+ roadmap), `docs/research/guide_analysis_consolidated.md` (12 разделов анализа), `docs/research/research_plan.md` (Фазы 1–5), `docs/canon/_README.md` (§5 migration status, §3.9 callout labels policy). **Приоритет iter 74:** Recon & Verification — проверить 9 утверждений V1–V9 (см. STATUS.md §Pending Verification, ~30 мин). По результатам — открыть KI#50+ если баги подтверждены. Только потом переходить к iter 75+ (критичные исправления P1). **Принцип:** «Лучше недоделать, чем сломать». Одна правка = одна итерация. Если найден новый баг — сначала документируй в `STATUS.md` как KI#N, потом фиксий. **Invariants (iter 73+):** (1) `viz > dry text` (iter 8+). (2) VS scroll-animation — `python3 scripts/audit_vs_embeds.py` (KI#44 ✅ CLOSED iter 68). (3) Component extracts drift (iter 42+). (4) CSS scoping (iter 34+). (5) YAML front-matter (iter 37+). (6) Guide's role as example takes priority over character canon (iter 39+). (7) OCEAN labeling consistency (iter 40+). (8) Bible ↔ canon cross-ref symmetry (iter 41+). (9) **Anchor navigation (iter 51+, KI#36 ✅ CLOSED):** все `<section data-section="X">` имеют `id="X"`. (10) **Canon → master HTML sync (iter 43+, 96/96 PASS):** `docs/canon/*.md` = source of truth. `src/master/*.html` = production HTML. (11) **Drift detector (iter 48+, v1.3):** `audit_canon_master_drift.py` — informational, exit 0. (12) **Build hash `4074bac5` (unchanged iter 73 — content additions в master HTML не влияют на hash, hash computed only from `src/shell/index.html`)**. (13) **Callout class policy (iter 45+):** разрешены `.callout.rule/.rec/.ex` и plain `.callout`. (14) **Recap-чек-листы в `<details>` (iter 55+ invariant).** (15) **Cat B prose mentions (iter 67+):** «Russian (English)» форма. (16) **Version sync (iter 71+):** все 9.1.0 references устранены. (17) **Progressive disclosure (iter 71+):** 96/96 секций покрыты. (18) **Section count (iter 72+):** 96 секций. (19) **Scenario labels Part 9 (iter 55/57/72):** 7/11 секций покрыты. (20) **Annotation blocks §10.2-10.4 (iter 57):** все 3 карточки имеют Annotation-блок. (21) **Research docs (iter 73+):** `docs/research/guide_analysis_consolidated.md` + `docs/research/research_plan.md` — источник правды для iter 74+ плана верификации и правок.
