...
| `docs/research/editorial_matrix.md` | Editorial matrix (ed-matrix Phases A+B — full guide coverage): block-level audit of Parts 0–10 + 4 appendices — 99 canon-declared section IDs, 500 rows with machine-checkable `<data-section>::<NN>` IDs, repeat registry R01–R29 (§24 classes). Evidence base for ed-1…ed-8 + dupes-1; KI#71/KI#72 recorded from it. The F2 `presentation` column (DEC-24) is seeded per Part/appendix at its build slice (Parts 1–10 + Appendix A: iters 147–158). |
