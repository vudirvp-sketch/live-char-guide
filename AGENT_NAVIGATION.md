...
**iter 48+ — что осталось (LOW priority):**
- **KI#33 ✅ CLOSED (iter 47)** — canon→master HTML sync complete. Minor задачи remain:
  - **General-purpose drift detector** — расширить `scripts/audit_canon_master_sync.py` до сравнения canon §X.Y vs master HTML `<section data-section>` semantic content. Текущий regression test — focused substring checks (89 checks = 71 positive + 18 negative). General-purpose detector требует semantic parsing.
...
**Подсказка следующему агенту:** Перед стартом iter 48+ прочитай `STATUS.md` (iter 47 — KI#33 ✅ CLOSED 57/57 fixes, contentHash `84d69ecf` 4th change since iter 34; iter 46 one-paragraph; iter 34-45 one-liners; все previous KI#1..#33 ✅ CLOSED/VERIFIED), `worklog.md` (iter 47 record — самый подробный; iter 46 one-liner), этот файл (AGENT_NAVIGATION §2a Deployment Pipeline iter 43+, §8 OP-1 iter 47 row + iter 48+ roadmap — minor задачи LOW priority), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 4 Appendix + Part 0 concept ✅, Canon COMPLETE; §3.9 callout labels policy). **Приоритет iter 48+: minor задачи LOW priority** — general-purpose drift detector (расширить `audit_canon_master_sync.py` до semantic parsing), Glossary double-render (LOW — structural), Component extracts regeneration (LOW — опциональный). Если новых багов/противоречий нет — проект STABLE, можно закрывать. **Invariants (iter 47+):** (1) `viz > dry text` (iter 8+). (2) VS scroll-animation — `python3 scripts/audit_vs_embeds.py` (0 regressions expected). (3) Component extracts drift (iter 42+). (4) CSS scoping (iter 34+). (5) YAML front-matter (iter 37+). (6) **Guide's role as example takes priority over character canon (iter 39+).** (7) **OCEAN labeling consistency (iter 40+, расширен iter 41; master sync ✅ iter 44).** (8) **Bible ↔ canon cross-ref symmetry (iter 41+; master sync ✅ iter 44).** (9) **Canon → master HTML sync (iter 43+ invariant, iter 47 ✅ COMPLETE 57/57):** `docs/canon/*.md` = source of truth для content. `src/master/*.html` = production HTML, деплоится на сайт. Regression test `python3 scripts/audit_canon_master_sync.py` (89/89 PASS — 11 iter 44 + 23 iter 45 + 23 iter 46 + 14 positive iter 47 + 18 negative iter 47). General-purpose drift detector — iter 48+ LOW priority. (10) **Build hash vs contentHash (iter 44+):** Shell hash `69d9b813` = SHA-256 of `src/shell/index.html` ONLY. contentHash = SHA-256 of combined `src/master/*.html`: `58f4daa85c05e070` → `34c34a7d9839c11d` (iter 44) → `665cede798c34fc0` (iter 45) → `d2fdafeaf093dd80` (iter 46) → `84d69ecffca28cbf` (iter 47). **Shell hash unchanged ≠ master HTML unchanged.** (11) **Callout class policy (iter 45+ invariant):** разрешены только `.callout.rule/.rec/.ex` и plain `.callout`. Запрещены `.callout.note/.info/.warn/.tip/.box/.sidebar/.custom/.important` (см. `scripts/validate-master.mjs` L452-463). Для «Примечание»/«Методология»/«Annotation»/«Demonstrates»/«Synthesis»/etc — используй plain `<p><strong>Label:</strong>...</p>` без callout wrapper (см. precedent `part_07a.html` L754 для «Примечание», `part_10.html` для «Annotation»/«Demonstrates»). Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 таблицу с sub-ID (A11, B7, и т.д.) и пометкой P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID, аудит переоценил, examples в пределах лимита 120 токенов).
