...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 3 Appendix ✅ MIGRATED (iter 18) + KI#13 ✅ **CLOSED** (iter 24, 123/123 = 100%) + Phase 4 SVG integration: E18 Greeting Algorithm ✅ INTEGRATED (iter 25) + Deployed Guide Audit (DGA) Phase 1 — STARTED (iter 26, KI#18 🟡 ACTIVE: 1/8 fixed, 7 pending iter 28+) + iter 27 status check (no code changes) + KI#14/#16/#17 ✅ CLOSED. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, техническая архитектура — в `docs/architecture.md`.
...
**iter 26 (Deployed Guide Audit Phase 1 — STARTED, 1/8 fixed):** Новый цикл работы по запросу user: пошаговая проверка собранного при деплое гайда на дублирование смысловой и функциональной нагрузки между текстом и визуализациями. Принцип: `viz > dry text` (iter 8+). Полный аудит 14 master HTML файлов выполнен — выявлено 8 duplication/inconsistency кейсов (KI#18 sub-items A–H). Применён 1 safe fix: **KI#18-A** (Part 9 Quality Scale) — убран дублирующий столбец «Признаки» (повторял E14 zone criteria), добавлен недостающий tier «Отличный» (85–100%), исправлена naming inconsistency «Плохой» → «Слабый» (соответствие E14 viz tier label), добавлен intro paragraph linking to E14. `src/master/part_09.html`: 582 → 583 строк (+1). Canon `docs/canon/part_09.md` updated (§9.1 table + front-matter + migration history row 4). `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged. **KI#18 🟡 ACTIVE — 1/8 fixed (A), 7 pending iter 28+ (B–H).**

**iter 27 (STATUS CHECK — без правок кода):** User запросил краткий отчёт о стадии работ. Никаких изменений в master HTML / canon / build artifacts / widgets / data. Только актуализация документации: дата → 2026-07-08, iter 27 запись добавлена в STATUS/worklog/CHANGELOG/PLAN/AGENT_NAVIGATION. Все «iter 27+» → «iter 28+» в KI#18 sub-items (7 pending B–H) и roadmap. Build hash fd3d96d3 unchanged (validation gates не запускались — нет source изменений).

**iter 28+ — что осталось (DGA Phase 2):**
- **KI#18 🟡 ACTIVE — 7 pending sub-items (B–H).** iter 28 priority: **B** (Part 1 p1_card_overview table — drop duplicate «Функция» col) + **C** (Part 2 p2_basic_anchors table — drop duplicate «Описание» col) — lowest risk. Then **E** (Part 5 OCEAN — needs careful rule alignment: strict <30/>70 vs broad <40/>60 definition; viz «1 экстремум» vs text «3 экстремальных полюса») + **F** (Part 6 p6_cot_tiers table — drop «Формат» col). **G** (Part 8 per-AP sections — by design, document rationale) + **H** (Part 10 E15 callouts — intentional annotation, document rationale) likely keep-by-design.
...
**Подсказка следующему агенту:** Перед стартом iter 28+ прочитай `STATUS.md` (iter 27 status check COMPLETE — без правок кода; iter 26 COMPLETE — DGA Phase 1 STARTED, KI#18 🟡 ACTIVE 1/8 fixed A, 7 pending B–H, все previous KI#1..#17 ✅ CLOSED), `worklog.md` (iter 27 record — status check; iter 26 record — DGA Phase 1), этот файл (AGENT_NAVIGATION §6 pitfall #37 KI#18 🟡 ACTIVE, §8 iter 28+ roadmap — DGA Phase 2: fix KI#18-B Part 1 + KI#18-C Part 2 first, then E + F, G+H likely keep-by-design), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 26 → DGA Phase 1 STARTED, §8 iter 26 stop point). Принцип `viz > dry text` (iter 8+) — сохраняется. Unique контент не удаляется даже при дублировании. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий.
