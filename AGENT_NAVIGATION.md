...
18. **Drift detector (iter 48+, iter 52 ✅ paragraph-level added)** — `python3 scripts/audit_canon_master_drift.py` v1.1 — informational, exit 0. Проверяет: section presence drift + heading text mismatch + content hash diff + paragraph-level Jaccard similarity (iter 52+). **88 paragraph drifts expected** (VS-EMBEDs replace text). CLI: `--no-paragraphs`, `--paragraph-threshold FLOAT`, `--json PATH`.
...
**Статус:** iter 1-52 завершены. Полный анализ в `PLAN.md`. Compressed iter 52 — оставлены только milestone iters.
...
**iter 53+ — что осталось (LOW priority only, проект STABLE):**
