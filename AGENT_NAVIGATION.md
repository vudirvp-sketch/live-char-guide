...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 4 Appendix + Part 0 (concept) ✅ MIGRATED/ADDED (iter 18 + iter 38) + KI#13/#18/#20/#21/#22/#23/#32 ✅ CLOSED + iter 34-45 + **iter 46 — KI#33 🔵 PARTIAL (canon→master HTML sync Phase 3: 8 fixes applied; cumulatively 41/57; contentHash `d2fdafea` 3rd change since iter 34; regression test extended to 57 checks)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 0 (concept) → Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, **аудит канона + fix plan (iter 33+) — в `docs/AUDIT_VERIFICATION.md` (P0+P1+P2+P3 ✅ CLOSED iter 35-38, KI#21 ✅ CLOSED canon; 41/57 master sync iter 44+45+46)**, техническая архитектура — в `docs/architecture.md`.
...
**iter 47+ — что осталось:**
- **KI#33 🔵 PARTIAL (iter 46) — canon→master HTML sync Phase 4 (iter 47+, MEDIUM/HIGH risk).** 16 audit правок remain. Категории:
  - **MEDIUM risk:** P2-3 (C5) — multiple files Bridge paragraphs cleanup (~8 delete, 2 keep). Risk: narrative flow. P2-7 (E4) — multiple files «Что вы теперь умеете» resume sections removal. Risk: structural delete. P2-18 (F10) — `part_10` Elena inline annotations → Annotation callout. Risk: structural transformation.
  - **HIGH risk:** P3-2 (D5) — `part_10` HTML comments `<!-- Demonstrates: -->` → visible callouts (5 cards). Risk: structural, добавить 5 новых `<div class="callout rule">` или `<div class="callout rec">` блоков перед карточками. Тестировать визуально.
  - **SKIP (metadata, canon-only):** P2-2 (callout labels policy), P2-4 (YAML front-matter), P2-6 (`[ref: ...]` notation), P2-15 (`<br/>` → em-dash) — canon-only, no master equivalent. P3-7 covered by P0-2. P3-8/11 — part_00/appendix_character_map (no master equivalent).
  - После каждого Part: `pnpm run build` + `validate:master` + `audit_canon_master_sync.py`. Расширить `audit_canon_master_sync.py` до general-purpose drift detector (сравнение canon §X.Y vs master HTML semantic content).
...
**Подсказка следующему агенту:** Перед стартом iter 47+ прочитай `STATUS.md` (iter 46 — KI#33 🔵 PARTIAL 41/57 fixes, contentHash `d2fdafea` 3rd change since iter 34; iter 45 one-paragraph; iter 34-44 one-liners; все previous KI#1..#32 ✅ CLOSED/VERIFIED), `worklog.md` (iter 46 record — самый подробный; iter 45 one-liner), этот файл (AGENT_NAVIGATION §2a Deployment Pipeline iter 43+, §8 OP-1 iter 46 row + iter 47+ roadmap — KI#33 fix Phase 4 MEDIUM/HIGH risk priority + minor задачи LOW), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 4 Appendix + Part 0 concept ✅, Canon COMPLETE; §3.9 callout labels policy). **Приоритет iter 47: KI#33 fix Phase 4 — canon→master HTML sync (16 fixes remain, MEDIUM/HIGH risk).** Начать с MEDIUM risk fixes (P2-3 Bridge paragraphs, P2-7 resume sections, P2-18 Elena annotations). HIGH risk P3-2 (HTML comments → visible callouts, 5 cards в part_10) — отдельно с visual testing. **Invariants (iter 46+):** (1) `viz > dry text` (iter 8+). (2) VS scroll-animation — `python3 scripts/audit_vs_embeds.py` (0 regressions expected). (3) Component extracts drift (iter 42+). (4) CSS scoping (iter 34+). (5) YAML front-matter (iter 37+). (6) **Guide's role as example takes priority over character canon (iter 39+).** (7) **OCEAN labeling consistency (iter 40+, расширен iter 41; master sync ✅ iter 44).** (8) **Bible ↔ canon cross-ref symmetry (iter 41+; master sync ✅ iter 44).** (9) **Canon → master HTML sync (iter 43+ invariant, iter 46 PARTIAL 41/57):** `docs/canon/*.md` = source of truth для content. `src/master/*.html` = production HTML, деплоится на сайт. Regression test `python3 scripts/audit_canon_master_sync.py` (57/57 PASS — 11 iter 44 + 23 iter 45 + 23 iter 46). Расширить до general-purpose drift detector — iter 47+. (10) **Build hash vs contentHash (iter 44+):** Shell hash `69d9b813` = SHA-256 of `src/shell/index.html` ONLY. contentHash = SHA-256 of combined `src/master/*.html`: `58f4daa85c05e070` → `34c34a7d9839c11d` (iter 44) → `665cede798c34fc0` (iter 45) → `d2fdafeaf093dd80` (iter 46). **Shell hash unchanged ≠ master HTML unchanged.** (11) **Callout class policy (iter 45+ invariant):** разрешены только `.callout.rule/.rec/.ex` и plain `.callout`. Запрещены `.callout.note/.info/.warn/.tip/.box/.sidebar/.custom/.important` (см. `scripts/validate-master.mjs` L452-463). Для «Примечание»/«Методология»/etc — используй plain `<p><strong>Примечание:</strong>...</p>` без callout wrapper (см. precedent `part_07a.html` L754). Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 таблицу с sub-ID (A11, B7, и т.д.) и пометкой P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID, аудит переоценил, examples в пределах лимита 120 токенов).
