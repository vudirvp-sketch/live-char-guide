...
**iter 21 (KI#13 Part 3+4 + Phase 4 SVG integration analysis):** (a) KI#13 Part 3: 2 inline styles в `part_03.html` → 1 CSS selector (`.vs-ki13-inset-text strong` — descendant selector для 2 `<strong>` в E07 Storage vs Influence inset). (b) KI#13 Part 4: 21 inline styles в `part_04.html` → 17 CSS selectors: (i) E05 SPINE causal chain panel — 4 селектора (`vs-ki13-spine-chain-panel/heading/text` + descendant `strong`) для 8 inline styles (1 panel + 1 heading + 1 text + 5 strongs); (ii) E06 GHOST Layers ring animation delays — 10 селекторов (`vs-ki13-ring-delay-{0,200,400,500,600,650,700,750,800,850}`) для 10 SVG `transition-delay` inline styles (4 circles + 6 texts); (iii) E06 GHOST Layers ring label titles — 3 селектора (`vs-ki13-ring-title--g3/g2/g1`) для 3 HTML `<div>` color inline styles. Total: 80/123 inline styles fixed (65%). `src/assets/vs-styles.css` +49 строк (SECTION 6, 18 новых селекторов). (c) Phase 4 SVG integration analysis: Canon migration (iter 7–18) уже удалил все major textual duplicates VS-EMBEDs; 3 retained infographics (2 part_04 + 1 part_07b) — unique, intentional retention (iter 8 principle «viz > dry text»); Part 7B (0 VS-EMBED) — candidate для нового VS element (E18+) в iter 22+, low priority. `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged. 10 docs/files updated.

**iter 22+ — что осталось:**
- **iter 22:** KI#13 Part 5+6 (1+5=6 inline styles → CSS classes) + Phase 4 SVG integration (Part 7B new VS element, optional, low priority).
- **iter 23+:** KI#13 Part 7A (19) + Part 9 (6) + Part 10 (12) = 37 remaining inline styles.
...
**Подсказка следующему агенту:** Перед стартом iter 22+ прочитай `STATUS.md` (iter 21 COMPLETE, KI#13 80/123 fixed, 43 remaining), `worklog.md` (iter 21 record), этот файл (AGENT_NAVIGATION §6 pitfall #36 KI#13 ACTIVE, §8 iter 22+ roadmap), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 22 priorities + §8 Phase 4 analysis). iter 22+ priorities: KI#13 Part 5+6 (6 inline styles → CSS classes) + Phase 4 SVG integration (Part 7B new VS element, optional, low priority). Принцип «viz > dry text» (iter 8) — сохраняется. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue, потом фиксий.
