...
**iter 41+ — что осталось:**
- **KI#28/#29 ✅ CLOSED (iter 40).** README section counts + OCEAN labeling устранены.
...
- **OCEAN labeling consistency (iter 40+ invariant):** extreme = строго `<30` или `>70` per Part 5 §5.1 RULE; cautious zone = `30–40` / `60–70`. Label-only fixes допустимы для internal canon consistency (Part 5 RULE vs Part 10/appendix label) — values примера не трогаются.
- **Потенциальные minor задачи (не критично, не запланировано):**
  ...
  - **Part 10 §10.4 moderate vs bible extreme values cross-ref** — Note в bible объясняет различие, но формальной cross-ref между Part 10 §10.4 и bible нет. LOW — cosmetic.
...
**Подсказка следующему агенту:** Перед стартом iter 41+ прочитай `STATUS.md` (iter 40 — README + OCEAN LABELING FIX ✅ COMPLETE, KI#28/#29 ✅ CLOSED, build hash `69d9b813`; iter 39 one-paragraph — KI#25/#26/#27 ✅ CLOSED; iter 38 one-paragraph — KI#21 P3 ✅ CLOSED полностью; iter 34-37 one-liners — CSS/CSP + KI#21 P0/P1/P2; все previous KI#1..#27 ✅ CLOSED/VERIFIED), `worklog.md` (iter 40 record — самый подробный; iter 39 one-liner), этот файл (AGENT_NAVIGATION §6 pitfall #40 KI#21 ✅ CLOSED полностью, §6 pitfall #41 doc drift invariant iter 39+, §8 OP-1 iter 40 row + iter 41+ roadmap — none planned, только minor задачи: Glossary double-render, Component extracts sync, Part 10 moderate vs bible cross-ref), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 4 Appendix + Part 0 concept ✅, Canon COMPLETE; §3.9 callout labels policy — English by design + 2 метки Demonstrates/Annotation iter 38). **Invariants (iter 40+):** (1) `viz > dry text` (iter 8+) — сохраняется. (2) VS scroll-animation — `python3 scripts/audit_vs_embeds.py` (0 regressions expected). (3) CSS scoping (iter 34+) — VS-EMBED selectors scoped к element-specific parent. (4) YAML front-matter (iter 37+) — все canon-файлы используют YAML front-matter. (5) **Guide's role as example takes priority over character canon (iter 39+):** при рассинхроне bible vs canon Part 10 — правится bible, не Part 10. (6) **OCEAN labeling consistency (iter 40+):** extreme = строго `<30` или `>70` per Part 5 §5.1 RULE; cautious zone = `30–40` / `60–70`. Label-only fixes допустимы для internal canon consistency — values примера не трогаются. (7) Build hash baseline `69d9b813` (unchanged после iter 34 — KI#23 fix; canon + doc файлы НЕ входят в hash computation — только `src/master/`, `src/shell/`, `src/assets/`, `data/`, `parts/` root fallbacks). Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 таблицу с sub-ID (A11, B7, и т.д.) и пометкой P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID, аудит переоценил, examples в пределах лимита 120 токенов).
