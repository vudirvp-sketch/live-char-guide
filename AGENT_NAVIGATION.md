...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 4 Appendix + Part 0 (concept) ✅ MIGRATED/ADDED (iter 18 + iter 38) + KI#13/#18/#20/#21/#22/#23/#32/#33 ✅ CLOSED + iter 34-48 + **iter 49 — RECONNAISSANCE ONLY (no code/master HTML/canon/data changes); все validation gates PASS (build hash `69d9b813` + contentHash `84d69ecffca28cbf` UNCHANGED); KI#34/KI#35 confirmed still open (fix deferred to iter 50+); `DELETES.txt` устаревший маркер удалён (trivial doc-cleanup); 10 unmerged dependabot branches (informational)**. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 0 (concept) → Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, **аудит канона + fix plan (iter 33+) — в `docs/AUDIT_VERIFICATION.md` (P0+P1+P2+P3 ✅ CLOSED iter 35-38, KI#21 ✅ CLOSED canon; 57/57 master sync iter 44+45+46+47)**, техническая архитектура — в `docs/architecture.md`.
...
**iter 50+ — что осталось:**
- **MEDIUM priority — KI#34 fix:** add `<section data-section="p1_prebuild_checklist">` block в `src/master/part_01.html` (content из canon `docs/canon/part_01.md` L128-145 — таблица 6 вопросов + RECOMMENDATION + Cross-ref). MEDIUM risk — careful HTML edit + visual verification. После fix: contentHash изменится (5th change since iter 34). Regression test `audit_canon_master_sync.py` расширить с positive check для p1_prebuild_checklist.
- **LOW priority — KI#35 fix (optional, trivial):** add `` `data-section: p4_spine_overview` `` line в `docs/canon/part_04.md` после `## 4.1 SPINE Overview`. Cosmetic canon metadata fix.
...
**Подсказка следующему агенту:** Перед стартом iter 50+ прочитай `STATUS.md` (iter 49 — RECONNAISSANCE ONLY, KI#34/KI#35 confirmed still open; iter 48 one-paragraph — drift detector added; iter 47 one-paragraph — KI#33 ✅ CLOSED 57/57; iter 34-46 one-liners; все previous KI#1..#33 ✅ CLOSED/VERIFIED), `worklog.md` (iter 49 record — самый подробный; iter 48 one-liner), этот файл (AGENT_NAVIGATION §2a Deployment Pipeline iter 43+, §8 OP-1 iter 49 row + iter 50+ roadmap — KI#34 fix MEDIUM priority + LOW priority minor задачи), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 4 Appendix + Part 0 concept ✅, Canon COMPLETE; §3.9 callout labels policy). **Приоритет iter 50+: MEDIUM priority — fix KI#34** (add `<section data-section="p1_prebuild_checklist">` block в `src/master/part_01.html` с контентом из canon `docs/canon/part_01.md` L128-145). LOW priority: KI#35 fix (trivial canon metadata), semantic paragraph-level drift detection, Glossary double-render, Component extracts regeneration, Dependabot merges (GitHub-level). Если новых багов/противоречий нет — проект STABLE. **Invariants (iter 49+):** (1) `viz > dry text` (iter 8+). (2) VS scroll-animation — `python3 scripts/audit_vs_embeds.py` (0 regressions expected; pre-existing path issue — symlink workaround `ln -sfn /path/to/repo /home/z/my-project/work/live-char-guide`). (3) Component extracts drift (iter 42+). (4) CSS scoping (iter 34+). (5) YAML front-matter (iter 37+). (6) **Guide's role as example takes priority over character canon (iter 39+).** (7) **OCEAN labeling consistency (iter 40+, расширен iter 41; master sync ✅ iter 44).** (8) **Bible ↔ canon cross-ref symmetry (iter 41+; master sync ✅ iter 44).** (9) **Canon → master HTML sync (iter 43+ invariant, iter 47 ✅ COMPLETE 57/57):** `docs/canon/*.md` = source of truth для content. `src/master/*.html` = production HTML, деплоится на сайт. Regression test `python3 scripts/audit_canon_master_sync.py` (89/89 PASS — 71 positive + 18 negative). (10) **Drift detector (iter 48+ invariant):** `python3 scripts/audit_canon_master_drift.py` — informational structural comparison canon ↔ master HTML (section IDs + headings + content hashes). Exit 0 всегда. JSON baseline: `build/drift-report-iter48.json`. Actionable: canon-only sections (KI candidates) + master-only sections (canon metadata drift). Heading mismatches (14, by design) + content hash diffs (96, informational) — expected. (11) **Build hash vs contentHash (iter 44+):** Shell hash `69d9b813` = SHA-256 of `src/shell/index.html` ONLY. contentHash = SHA-256 of combined `src/master/*.html`: `58f4daa85c05e070` → `34c34a7d9839c11d` (iter 44) → `665cede798c34fc0` (iter 45) → `d2fdafeaf093dd80` (iter 46) → `84d69ecffca28cbf` (iter 47, iter 48+49 UNCHANGED). **Shell hash unchanged ≠ master HTML unchanged.** (12) **Callout class policy (iter 45+ invariant):** разрешены только `.callout.rule/.rec/.ex` и plain `.callout`. Запрещены `.callout.note/.info/.warn/.tip/.box/.sidebar/.custom/.important` (см. `scripts/validate-master.mjs` L452-463). Для «Примечание»/«Методология»/«Annotation»/«Demonstrates»/«Synthesis»/etc — используй plain `<p><strong>Label:</strong>...</p>` без callout wrapper (см. precedent `part_07a.html` L754 для «Примечание», `part_10.html` для «Annotation»/«Demonstrates»). Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий. Если найдено новое противоречие в каноне — добавляй в `docs/AUDIT_VERIFICATION.md` §2 таблицу с sub-ID (A11, B7, и т.д.) и пометкой P0-P3. **B3 — НЕ пытаться «укоротить Examples Омнис-Зета»** (KI#21-B3 INVALID, аудит переоценил, examples в пределах лимита 120 токенов).
