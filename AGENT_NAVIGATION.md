...
> **Entry document.** Read this first. Текущая версия: **9.1.0** + все 10 Parts + 3 Appendix ✅ MIGRATED (iter 18) + KI#13 ✅ **CLOSED** (iter 24, 123/123 = 100%) + Phase 4 SVG integration: E18 Greeting Algorithm ✅ INTEGRATED (iter 25) + Deployed Guide Audit (DGA) Phase 1 — STARTED (iter 26, KI#18-A ✅) + DGA Phase 2 (iter 28, KI#18-B ✅ + KI#18-C ✅, KI#18-I NEW; iter 29, KI#18-I ✅ + KI#18-F ✅ partial; iter 30, KI#18-D ✅ + KI#18-E ✅ + KI#19 ✅ incidental) + KI#14/#16/#17 ✅ CLOSED. Live-char-guide — инженерный пайплайн для RP-карточек персонажей (от SPINE до деплоя, для моделей 12B–32B+). Единый линейный гайд без слоёв: весь контент читается последовательно Part 1 → Part 10. Актуальный статус — в `STATUS.md`, история итераций — в `worklog.md`, полный план docs-restructure — в `PLAN.md`, **план переработки контента (iter 6+) — в `docs/CONTENT_RESTRUCTURE_PLAN.md`**, **Canon (источник правды для контента, iter 7+) — в `docs/canon/` (см. `_README.md`)**, техническая архитектура — в `docs/architecture.md`.
...
**iter 31+ — что осталось (DGA Phase 2 final):**
- **KI#18 🟡 ACTIVE — 2 pending sub-items (both likely keep-by-design).** **G** (Part 8 per-AP sections repeat E12 cards structure — by design catalog vs detail, document rationale). **H** (Part 10 E15 callouts token budgets duplicate E01 — intentional annotation, document rationale). После G + H — KI#18 ✅ CLOSED.
- **Все previous KI (KI#1..KI#17, KI#19) ✅ CLOSED.** Новые баги — сначала документировать в `STATUS.md` как Known Issue (KI#N), потом фиксить.
- **Принцип `viz > dry text` (iter 8+):** сохраняется. Unique контент не удаляется даже при дублировании.
...
**Подсказка следующему агенту:** Перед стартом iter 31+ прочитай `STATUS.md` (iter 30 DGA Phase 2 continued COMPLETE — KI#18-D ✅ FIXED + KI#18-E ✅ FIXED + KI#19 ✅ FIXED incidental; KI#18 🟡 ACTIVE 7/9 fixed A+B+C+D+E+I+F, 2 pending G+H likely keep-by-design, все previous KI#1..#17 + KI#19 ✅ CLOSED), `worklog.md` (iter 30 record — DGA Phase 2 continued; iter 29 one-liner; iter 28 one-liner; iter 26 record — DGA Phase 1), этот файл (AGENT_NAVIGATION §6 pitfall #37 KI#18 🟡 ACTIVE 7/9 fixed, §6 pitfall #38 KI#19 ✅ CLOSED, §8 iter 31+ roadmap — DGA Phase 2 final: G+H likely keep-by-design), `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE), `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 30 → DGA Phase 2 continued, §8 iter 30 stop point + KI#18 sub-items table). Принцип `viz > dry text` (iter 8+) — сохраняется. Unique контент не удаляется даже при дублировании. Если найден новый баг — сначала документируй в `STATUS.md` как Known Issue (KI#N), потом фиксий.
