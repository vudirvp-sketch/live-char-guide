...
| `visual-system/` | Visual system prototypes: `PLAN.md`, `DESIGN-TOKENS.css`, `shared/`, `elements/` (E02–E06, E09, E11–E14, E16–E18 — E01/E08/E10/E15 → `REMOVED_WITH_REASON` iter 138, E07 → iter 142, DEC-19), `hero/` → `REMOVED_WITH_REASON` iter 140 (DEC-19 §6.4 disposal; archive = git history), `integration/` (component-extracts). `tokens.json` feeds `qa:contrast`. | **Frozen design artifacts (DEC-19, iter 138):** master embed (L2) = the canonical visual markup; archive = git history; remaining families ride the freeze at their slice time. |
...
| `docs/research/owner_gates_iter144.md` | Owner-gate recommendation package (iter 144, doc-only): verified evidence + recommended calls for the two remaining owner gates — G1 KI#72 sampling values (12-row live-location inventory; S-a…S-d, §7A.6 canonical recommended; Options B/C rejected) + G2 v2 architecture phase opening (P-a/P-b/P-c first-slice options, P-a consolidated spec recommended; order G1 → sampling slice → phase) — agent-prepared input; both gates stay owner-called (nothing DECIDED). |
