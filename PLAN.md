...
**Iter 28 ✅ DONE (DGA Phase 2 — 2 fixes applied, 1 new bug documented):** Применены 2 safe fix из 7 pending sub-items KI#18: **KI#18-B** (Part 1 p1_card_overview table) — drop duplicate «Функция» column (повторял E01 `.block-content`), add intro paragraph linking to E01 viz (pattern KI#18-A). Table: 4 cols → 3 cols (Блок / Влияние на модель / Доля бюджета — both unique). part_01: 365 → 367 (+1). **KI#18-C** (Part 2 p2_basic_anchors table) — drop duplicate «Описание» column (повторял E03 `flow-node__desc`), expand intro paragraph linking to E03. Table: 4 cols → 3 cols (# / Этап / Пример — examples unique). part_02: 415 → 415 (0 net). Canon `docs/canon/part_01.md` + `docs/canon/part_02.md` updated. **KI#18-I NEW** (Part 2 p2_embodiment table «Описание» col duplicates E04 viz — same pattern as C) — found during KI#18-C analysis, documented in STATUS.md §Known Issues, NOT fixed (pending iter 29+). `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash fd3d96d3 unchanged. **KI#18 🟡 ACTIVE — 3/9 fixed (A+B+C), 6 pending (D, E, F, G, H, I) iter 29+.**
...
**Активный KI:** KI#18 🟡 ACTIVE (3/9 fixed A+B+C, 6 pending D+E+F+G+H+I iter 29+). Все previous KI (KI#1..KI#17) ✅ CLOSED.
