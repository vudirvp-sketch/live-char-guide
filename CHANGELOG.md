...
### iter 159 — v2-build-appendix-b: Appendix B (Model Capability Table) v2 build slice — F2 matrix seeding (4 rows, all `open`, + the iter-145 НАБЛЮДЕНИЕ audit-coverage row added — living total 500→504 attributed); canon zero edits (byte-stable); master: two mirror completions (the «Ключевые следствия» block restored + the Script Tax cell) + the v7-era `part-resume` disposed REMOVED_WITH_REASON (the iter-158 «B/C slices co-decide» sub-decision executed; the stale MODEL_NOTE bullet removed); drift 142→141

- **Scope (STATUS Next-step row 1, pinned iter 158):** the Appendix B (Model Capability Table) v2 build slice — PLAN row v2-build-appendix-b, **not owner-gated** (the appendix slices = the pinned track continuation; the owner's session go-ahead «продолжай работы по планам!»). Executed per the ratified spec §7 shape (map §5.20, P-B-1..P-B-5). Bounded slice: 1 section / 4 matrix rows (3 re-derived + the НАБЛЮДЕНИЕ row added); Appendix B hosts no cluster owner; no infrastructure work (the KI#70 WIRE live since iter 158).
- **Evidence (P-B-1 — F2 seeding, thirteenth application):** the matrix appendix_model_table table re-headed to 9 columns; 4 rows dispositioned — **all `open`** (the Reference-layer verdict holds: intro → the reference table → the OBSERVATION strength framing → the apply-layer следствия; zero compression candidates). **The НАБЛЮДЕНИЕ row added** (::03, METADATA — the block post-dated the Phase-A/B audit: added iter 145 as DEC-20's first OBSERVATION application, no matrix row at the time; the §1.1 document-order re-derivation; ex-::03 следствия → ::04 with L35–39 — zero external references to the old IDs). Matrix §1.2 scope note extended to Parts 1–10 + Appendices A/B; **the audit-total header disclosed** (500 → **504** — the script-examined count: the iter-126 audit's 500 + 4 post-audit row additions (iter 139/142/147's rows, each disclosed at its own slice but never folded into the header, + the НАБЛЮДЕНИЕ row added this slice)); **the ::01 stale marker inventory refreshed** (was «markers ×2 in Part 6» — canon `[Model: see Appendix B]` ×4 (Part 6 ×2 + Part 7A ×2) + master `model-note` spans ×5 (Part 4 ×1 + Part 6 ×2 + Part 7A ×2), all live at runtime since the iter-158 WIRE); the appendices verdict extended with the B-sliced line. Count script-verified: 4 `open`.
- **Canon (P-B-2):** `appendix_model_table.md` — **zero edits, byte-stable** (git-diff-verified). F5 verified F5-approximate — the Reference-layer shape is the target form (Concept → reference payload → strength framing → apply summary; the zero-restructure precedent chain); section ID `appendix_model_table` preserved; the OBSERVATION anchor intact (the sampling gate's check-8 canon probe).
- **Master (P-B-3):** `appendix_model_table.html` — **two mirror completions** (fence #10 canon wins, the iter-155/158 v9-slimming-gap precedent): (a) the entire «Ключевые следствия для сборки карточки» block restored (h3 + 3 `<li>` + the `#p7a_core_directives` link on «см. §7A.2 ПРАВИЛО» — the part_07a §7A.12-row link pattern; the v9 Phase-6 slimming had dropped the block whole); (b) the Script Tax API-column word order restored to the canon form. **The v7-era `part-resume` block disposed REMOVED_WITH_REASON** — the iter-158-recorded «B/C slices co-decide» sub-decision executed at B: its bullet 2 («Учитывать MODEL_NOTE при выборе техник…») directed readers at markers that no longer exist anywhere (grep-verified — the v9 Phase 3.3 consolidation replaced them with this very table), a live misdirection since the WIRE made the appendix render; 5-point check + zero tool deps + zero back-links documented in map §5.20 P-B-3; the family precedent = the iter-47 P2-7 cleanup (the Parts cleaned, the appendices explicitly out of that KI's scope); A's benign instance remains recorded (P-app-4a), C's slice follows the B precedent.
- **Observations (P-B-4 — recorded, deferred):** the remaining appendix `part-resume` co-travelers (A/C); the heading-mismatch informational family (canon «B.1 …» vs the non-numbered master h2 — 15 drift WARNs, informational); the canon front-matter staleness (the repo-wide iter-152 family); the master part_04 `model-note` span (P4-4 enrichment family); the comment-noise drift entry (informational).
- **Registries/state:** map — the §5 preamble living-count (500→504) + NEW §5.20 + the §7 iteration log; matrix — the presentation column + the НАБЛЮДЕНИЕ row + the header disclosure + the stale-note refreshes + the verdict; STATUS (iter 159; the v2-architecture invariant extended to Appendices A/B; KI#84 row deleted per lifecycle — 2 iterations elapsed; the KI-notes block re-sorted; Next step → the Appendix C slice); PLAN (v2-build-appendix-b → COMPLETE; NEW row v2-build-appendix-c); worklog (iter-158 → one-liner; iter-149 dropped — ≤10 cap); CHANGELOG (this entry; iter-157 collapsed); NAV §7 de-staled (the appendix slices: A/B).
- **Verification (full battery, executed; pnpm absent — the underlying node/python commands run directly, zero external deps):** build ✓ (hash `2ab607d6` unchanged — `src/shell/index.html` untouched; root fallback `parts/appendix_model_table.html` regenerated — grep-verified src+root; root `index.html` timestamp-only) · validate ✓ · validate:master ✓ · version:check 9.2.6 ✓ · tests **64/64** · qa:csp/bundle/contrast PASS · qa:doc-versions 1 pre-existing warning · qa:english **18** (baseline exact) · qa:syntax **238** (baseline exact) · sync **97/97 PASS** (no appendix_model_table probes — untouched) · drift actionable **142→141** (expected: the следствия mirror completion cleared the appendix_model_table plain_text entry — pre-verified by simulation with the drift tool's own tokenizer, best-Jaccard 0.660 ≥ 0.3; JSON pre/post diff; the remaining entry = comment-noise, informational; zero new rows) · parity **×7 PASS** (sampling PASS — the OBSERVATION framing intact, check 8 green) · `git diff --check` clean. Lint not run (eslint requires node_modules; no JS infrastructure files touched — disclosed).

### iter 158 — v2-build-appendix-a + KI#70: Appendix A (MBTI Reference) v2 build slice — F2 matrix seeding (6 rows, all `open` — the Reference-layer verdict holds); canon zero edits (byte-stable); master: three mirror completions + the stale v9-era transition disposed; **the KI#70 WIRE EXECUTED per DEC-24 Q7** (appendices render at runtime, 93→96 sections; KI#70 CLOSED); drift 143→142

- **Scope (STATUS Next-step row 1, pinned iter 157):** the Appendix A (MBTI Reference) v2 build slice — PLAN row v2-build-appendix-a, **not owner-gated** (DEC-24 Q7 decided WIRE; the appendix slices = the pinned track continuation; the owner's session go-ahead «продолжай работы по планам!») — **carrying the KI#70 WIRE implementation**. The owner's KI#84 directive quoted in the same message was verified ALREADY EXECUTED in iter 157 (the repo-wins conflict rule: §9.7 sole Walter reference + both moved tool anchors confirmed in-file). Executed per the ratified spec §7 shape (map §5.19, P-app-1..P-app-6). Bounded slice: 1 section / 6 matrix rows; Appendix A hosts no cluster owner.
- **Evidence (P-app-1 — F2 seeding, twelfth application):** the matrix appendix_mbti table re-headed to 9 columns; 6 rows dispositioned — **all `open`** (the Reference-layer verdict holds: intro → 2 reference tables → the disambiguation note → the interactive tool → the РЕКОМЕНДАЦИЯ decision statement; zero compression candidates). Matrix §1.2 scope note extended to Parts 1–10 + Appendix A; the §14 header note de-staled (the appendices render at runtime since the WIRE); **two stale row notes refreshed** (::04 — the pre-iter-123 «EN label „NOTE"» + pre-iter-127 «`§7A.X` vague» states, pickaxe-verified; ::05 — the KI#70 dependency RESOLVED) + the appendices verdict refreshed (the pre-iter-127 «18 vague glossary refs» line corrected — the glossary per-row notes stay stale, deferred to that appendix's own slice). Count script-verified: 6 `open`.
- **Canon (P-app-2):** `appendix_mbti.md` — **zero edits, byte-stable** (git-diff-verified). F5 verified F5-approximate — the Reference-layer shape is the target form (the Parts 5/6/7B/8 zero-restructure precedent); section ID `appendix_mbti` preserved; the canon refs verified full (`[ref: part_07a.md §7A.1 — System Prompt]` — the iter-127 R18 repair, pickaxe `8a365553`).
- **Master (P-app-3):** `appendix_mbti.html` — **three mirror completions** (fence #10 canon wins, the iter-155 v9-slimming-gap precedent): (a) the S·P note restored to the canon form («Примечание:» label + sentence 2 — the `S·P`-dot convention — + the `#p7a_system_prompt` link; was em-parenthetical sentence 1 only); (b) the temperament table header «Цвет» → «Цвет (в виджете)»; (c) the РЕКОМЕНДАЦИЯ gloss «(поведенческие якоря)» restored. **The stale v9-era transition disposed REMOVED_WITH_REASON** («Следующие секции: Enneagram Wings, OCEAN×Enneagram Matrix» — the v9 Part-5-move leftover pointing back into Part 5 from a post-Part-10 appendix; 5-point check in map §5.19 P-app-3; grep-verified zero tool probes + zero back-link registry entries; the iter-148 §2.3 precedent). The master-only `part-resume` block recorded as an observation (P-app-4 — the A/B/C co-traveler family), not disposed.
- **KI#70 (P-app-5 — the DEC-24 Q7 WIRE):** `src/shell/lazy-loader.js` — the load list appends `manifest.appendices` after `manifest.parts`, re-ordered by the new `APPENDIX_GUIDE_ORDER` map (guide lettering A→B→C per the canon appendix files' A.1/B.1/C.1 numbering; the manifest array is an alphabetical `build-unified.mjs` artifact; unknown files sort last, stable) — the appendices inherit the KI#69 retry/placeholder machinery for free; **the TOC-grouping sub-decision executed**: `generateTOC()` gains an «Приложения» group after the Part groups (the auto-injected part-01 TOC's own convention; entry text = each appendix's own h2; accordion binding inherited; Part-number active highlighting intentionally Part-scoped). **Runtime-verified headless** (the iter-119-suite extension, pre/post probes against dist/): pre-WIRE — 93 sections, the `#appendix_mbti` TOC link present-but-dead, no `#mbti-embed`; post-WIRE — **96 sections (93→96)**, appendix order A→B→C verified, the `#appendix_mbti` link target rendered **and anchor navigation scrolls into view** (scrollY 142112 → top 80), the `#mbti-embed` container present, **the mbti-composer widget initializes with zero page errors / zero console errors** (the never-exercised path, now exercised), the sidebar TOC carrying the 3 entries in guide order. 11/11 probe checks PASS. **KI#70 CLOSED.**
- **Observations (P-app-4 — recorded, deferred):** the master-only `part-resume` v7-era family (A/B/C co-travelers); the canon container-note drift entry (informational); the front-matter staleness (the repo-wide iter-152 family); the manifest `appendices` order = alphabetical build artifact (the lazy-loader re-orders; a manifest-order refinement = a supersedeable future DEC); the appendix_glossary + character_map stale matrix notes (their own slices); the part_05 §5.5 MBTI text-vs-link variance.
- **Registries/state:** map — NEW §5.19 (P-app-1..P-app-6 + accounting + deferred + drift attribution) + the §7 iteration log; matrix — the presentation column + the §14 de-stale + the stale-note refreshes + the verdict; STATUS (iter 158; the v2-architecture invariant extended — the appendix-slice stage OPEN, the WIRE EXECUTED, 96 sections; KI#70 CLOSED; iter-158 KI-note; Next step → the Appendix B slice); PLAN (v2-build-appendix-a → COMPLETE; NEW row v2-build-appendix-b; the ki-70 gated row marked RESOLVED); worklog (iter-157 → one-liner; iter-148 dropped — ≤10 cap); CHANGELOG (this entry; iter-156 collapsed); NAV §7 de-staled (the appendix slices).
- **Verification (full battery, executed):** build ✓ (hash `2ab607d6` unchanged — `src/shell/index.html` untouched; root fallbacks `assets/lazy-loader.js` + `parts/appendix_mbti.html` regenerated — grep-verified src+root; root `index.html` timestamp-only) · validate ✓ · validate:master ✓ · version:check 9.2.6 ✓ · tests **64/64** · lint ✓ · qa:csp/bundle/contrast PASS · qa:doc-versions 1 pre-existing warning · qa:english **18** (baseline exact) · qa:syntax **238** (baseline exact — content delta 0) · sync **97/97 PASS** (no appendix_mbti probes — untouched) · drift actionable **143→142** (expected: the S·P-note mirror completion cleared the appendix_mbti plain_text entry; verified by the JSON pre/post diff; the two remaining entries = comment-noise + container-note metadata; zero new rows) · parity **×7 PASS** · the post-WIRE runtime probe **11/11 PASS** · `git diff --check` clean.
