...
### iter 155 — v2-build-part8: Part 8 (Anti-patterns) v2 build slice — F2 matrix seeding (40 rows, all `open` — the Debug-mode catalog Part); canon zero edits (byte-stable); master: sixteen mirror completions (the v9 Phase-6 slimming gaps restored + the §8.1 «Ссылка» re-point); drift 154→148

- **Scope (STATUS Next-step row 1, pinned iter 154):** the Part 8 (Anti-patterns) v2 build slice — PLAN row v2-build-part-8, **not owner-gated** (the DEC-24 ratification gate cleared; per-Part slices = the pinned track). Executed per the ratified spec §7 shape on the iter-147…154 precedent (maps §5.8–§5.15 → this slice's map §5.16, P8-1..P8-5). Bounded slice: 16 sections / 40 matrix rows — the second-largest Part (after Part 7A's 94); executed whole. Part 8 hosts no cluster owner (the seven owners sit outside — no byte-stability lock beyond fence #10).
- **Evidence (P8-1 — F2 seeding, ninth application):** the sixteen matrix Part 8 tables (§12) re-headed to 9 columns; 40 rows dispositioned — **40 `open`** (the Debug-mode catalog Part: the per-AP recognition cards + the before/after demos = the recognition core; zero qualifying Collapsible blocks per the registered criteria — every candidate block is the teaching payload itself) + zero `collapsible` + zero `canonical-link` + zero `—` (the Part verdict «deliberate two-layer design, zero compression candidates» held). Matrix §1.2 scope note extended to Parts 1–8; the Part 8 verdict appended with the iter-155 slice note; **eleven stale row notes + the verdict defect-load line refreshed** (the vague-ref + unbalanced-paren defects repaired iter 127, ed-8/R18 — pickaxe `8a365553`; the R17-lite phrase stripped iter 129; the KI#72 conditional superseded by DEC-22 iter 145 — the notes described pre-fix states). Count script-verified: 40 = 40.
- **Canon (P8-2):** `part_08.md` verified F5-approximate per section (the AP-card pattern Concept → Rule → Core example → Reference). **Zero content edits — byte-unchanged** (sha256 `8f4a4e02…` pre = post; the iter-148/149/151/152/154 zero-canon-edit precedent).
- **Master (P8-3 — sixteen mirror completions, 16 insertions + 11 deletions):** the v9 «Phase 6: Reduced content duplication» slimming gaps — every dropped canon clause / sentence / «Ссылка» row / link restored per canon (fence #10 — canon wins): the §8.1 ПРАВИЛО tail («выявляемая на этапе валидации»); the §8.1 label re-point «Примечание»→«Ссылка» **with the sync P1-3 probe substring updated in step** (2 lines in `scripts/audit_canon_master_sync.py` — the expected substring + the description note; the iter-148 probe-extension precedent, disclosed); the §8.2 bullet imperative + the ::03 «Ссылка» row; the §8.3 WHY-clause; the §8.4 mechanism sentence; the §8.5 missing §7A.1 link; the §8.6 suppression clause; the §8.7 rationale sentence; the §8.8 explanation clause; the §8.9::02 «Ссылка» row; the §8.11::04 «Ссылка» row; the §8.13 Format Lock sentence + the ::04 «Ссылка» row; the §8.15 §7A.12 link; the §8.16 «Причина» line. 5-point check per edit (map §5.16 P8-3 — four new anchors navigating: `#p10_elena`, `#p4_spine_mapping`, `#p6_cot_anchors`, `#p7a_xml_tags` + `#p7a_format_lock` + `#p7a_token_budget`); the E12 embed + the four other part_08 probes verified untouched.
- **Observations (P8-4 — recorded, deferred):** seven items — the four master-only inter-section transitions (the P7B-4(c) family); the stray/vacant illustration chips (§8.5/§8.7/§8.13/§8.14); the §8.7 До/После table→paragraph variance; the EN-terminology variances (canon «дефект»/«призрак» vs master «FLAW»/«GHOST»; the §8.9 gloss drop); the §8.1 parenthetical glosses + punctuation churn; the §8.16 structural quirks; the §8.8 29-char paragraph below the drift tool's MIN_PARAGRAPH_LENGTH (KI#79-adjacent tool-boundary artifact, pre-existing, exit 0). Recorded as OBSERVATIONS in map §5.16 (ride the canonical-audit stage). No KI (registry rows, not defects).
- **Registries/state:** map — NEW §5.16 (P8-1..P8-5 + accounting + deferred + drift attribution) + §7 iteration log; matrix — the presentation column + the slice note + the stale-note refreshes; STATUS (iter 155; the v2-architecture invariant + Next-step preamble extended to Parts 1–8 / iters 147–155 / maps §5.8–§5.16; iter-155 KI-note; Next step → the Part 9 slice); PLAN (v2-build-part-8 → COMPLETE; NEW row v2-build-part-9); worklog (iter-154 → one-liner; iter-145 dropped — ≤10 cap); CHANGELOG (this entry; iter-154 collapsed to one-liner — the Document-caps trim); NAV §7 de-staled (Parts 1–8). One in-slice tooling alignment (the sync P1-3 probe substring — disclosed; the iter-148 precedent).
- **Verification (full battery, executed):** build ✓ (hash `2ab607d6` unchanged; root fallback `parts/part_08.html` regenerated carrying the sixteen mirrors — grep-verified src+root; root `index.html` timestamp-only) · validate ✓ · validate:master ✓ · version:check 9.2.6 ✓ · tests **64/64** (sandbox `PUPPETEER_EXECUTABLE_PATH` workaround — env-only) · qa:csp/bundle/contrast PASS · qa:doc-versions 1 pre-existing warning · qa:english **18** (baseline exact) · qa:syntax **238** (baseline exact — content delta 0) · sync **97/97** · drift actionable **154→148** (six canon-side matches gained: §8.2::03 + §8.6 + §8.7 + §8.8 + §8.9::02 + §8.16 left the actionable lists — verified by the JSON pre/post diff; zero new rows) · parity **×7 PASS** · `git diff --check` clean.
