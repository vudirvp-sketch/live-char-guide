...
- **Scope (non-gated matrix-output execution under the Editorial Policy DEC-15; owner chat directive «продолжай работу последней итерации, что там дальше следует логично»):** PLAN row `dupes-2` second batch — the R18 Phase A remainder (STATUS Next-step row 5) + R17 canon-side strips #14/#15/#17/#18. Canon-only; master untouched by design (verified: master resolves every affected ref to a proper anchor; all 4 strips are canon-only/non-rendering).
- **R18 Phase A refs repaired (16 — grep recount; the matrix ~13 estimate was undercounted again, L64/L134 in part_09 were matrix-unflagged):** part_09 ×12 — §9.3 L64 `§4.X`→§4.1 (SPINE tags) · L65 `§3.X — Voice Isolation`→`§3.3 — Правила Examples` (label aligned: master resolves the row to `#p3_examples_rules`) · L66 `§7B.X — Lorebook/Greeting`→`§7B.2 — Greeting` (label trimmed: row checks are Greeting-only) · §9.4 L86 `§1.X`→§1.7 (Top-3) · §9.5 L103→§3.1 / L104→§4.8 / L105→§3.3 / L106→§2.2 / L113→§6.2 · §9.6 L134→§5.3 (OCEAN Warning) / L137→§6.2 · §9.10 L227→§6.3 (CoT Tiers; canon-only line, target from label) · part_07a ×3 — L109 `§2.X`→§2.4 (Embodiment Protocol) · L340 `§3.X`→§3.1 · L699 `§6.X`→§6.2 + unbalanced paren closed · part_03 L165 `§7B.X`→§7B.2 (Greeting Message). Every target verified against the existing master anchor (id↔§ cross-check).
- **R17 strips #14/#15/#17/#18 (non-rendering, canon-only):** part_00 L45 «английские метки отменены iter 123, DEC-16» history clause dropped (label inventory kept) · part_00 L58 `(DEC-16, docs/canon/_README.md §3.9)` repo pointers dropped (substance kept) · part_06 L83 «(iter 29, KI#18-F partial fix)» label dropped (accessibility rationale kept) · character_map L29 «(фикс iter 40 — KI#29)» label dropped (labeling statement kept).
- **Verification (executed):** build PASS hash `2ab607d6` unchanged · validate ✓ · validate:master 12 checks ✓ · version:check sync 9.2.6 · tests 64/64 · canon sync **97/97 PASS** · drift exit 0 · qa:csp/bundle/contrast/doc-versions PASS (1 pre-existing warning = obs-3) · qa:english **18** / qa:syntax **247** (baselines exact) · `git diff --check` clean · index.html timestamp churn restored (no deliverables — no src/ changes). 6 canon + 5 state/evidence docs vs `BASE_COMMIT 8a365553`; no deletions. Left as-is (out-of-scope): `_README.md` L103 (ref-notation format example) · character_map L41 «Part 10 §10.X» (generic form, matrix-unflagged) · R17 [B] remainder = part_00 L13 preamble + optional part_08 L27 «v9 restructure».

### iter 127 — ed-8/R18: canon vague-ref repair batch (Phase B mechanical family)

- **Scope (non-gated matrix-output execution under the Editorial Policy DEC-15; owner chat directive «продолжай работу последней итерации, что там дальше следует логично»):** PLAN row `dupes-2` first batch — the largest mechanical family from the iter-126 editorial matrix: R18 canon-only vague `§X.X` placeholder refs (Phase B set) + part_08 unbalanced parens (×6) + KI#77-a…d canon ref repair. Canon-first; master needs no link changes (verified: master resolves every affected ref to a proper anchor — the only master edit is the part_05 L21 mirror).
- **R18 Phase B refs repaired (33):** part_08 ×7 (`§3.X`→§3.1 ×2, `§2.X`→§2.2 ×2, `§6.X`→§6.6, `§3.X`→§3.8, `§7B.X`→§7B.3 — targets verified against existing master anchors `#p3_voice_isolation`/`#p2_anchor_rules`/`#p6_cot_anchors`/`#p3_multi_char`/`#p7b_lorebook_basics`) · glossary ×18 (5× `§7A.X`→§7A.1/§7A.2/§7A.12, 6× `§4.X`→§4.2/§4.3/§4.4/§4.5/§4.6/§4.11, 4× `§3.X`→§3.1/§3.6/§3.8, 2× `§7B.X`→§7B.2/§7B.3, 1× `§2.X`→§2.4) · part_10 ×4 (Token Budget → §7A.12) · part_05 ×2 (L89 Assembly → §7A.13; L21 «см. §5.3 контекстные лимиты» → «см. таблицу «Контекстные лимиты полюсов» ниже» — the limits table lives in §5.1, R25 finding) · appendix_mbti ×1 (→ §7A.1) · character_map «Part 6 §6.X» → §6.5–§6.6 (R23 teach+summary sites).
- **part_08 unbalanced parens ×6 closed** (L44/66/78/98/112/150 — `(см. `[ref: …]`.` → `…`).`); master renders these lines as clean «Исправление → <a>» links — no mirror needed.
- **KI#77 (a)–(d) repaired ((e) remains open):** (a) part_04 L281 `[ref: §7A.6 — Consequence Driven]` → §7A.2 (directive #6 home; master link was already correct) · (b) character_map Omnis stale refs («Part 5 §5.2 (OCEAN extreme)» — no Омнис in part_05, byte-verified; «Part 8 §8.X (AP-15 OCEAN Overload)» — concept moved to Part 5 §5.3 per §8.1's own pointer, AP-15 now Nested Anchors) → «Part 5 §5.1/§5.3 (концепты полюсов и перегрузки OCEAN)» · (c) Выщербленный «Part 5 (эннеаграмма)» dropped — 5w4 lives in §10.4 (already listed) + Эннеаграмма column carries 5w4 · (d) «Part 6 §6.X» → §6.5–§6.6. **(e) budget ranges (R27) deferred — needs the ed-5 framing decision.**
- **Mirror + audit:** `src/master/part_05.html` L38 mirror of the L21 change; sync-audit P0-16 expected substring updated to the new wording (established pattern: iters 94/105/107/123 — canon wins, the guard follows).
- **Verification (executed):** build PASS hash `2ab607d6` unchanged · validate ✓ · validate:master 12 checks ✓ · version:check sync 9.2.6 · tests 64/64 · canon sync **97/97 PASS** · drift exit 0 · qa:csp/bundle/contrast/doc-versions PASS (1 pre-existing warning = obs-3) · qa:english **18** / qa:syntax **247** (baselines exact) · `git diff --check` clean · root fallback `parts/part_05.html` regenerated (real build — index.html timestamp churn kept, iter-121 precedent). 7 canon + 1 master + 1 script + 2 build outputs + 5 state/evidence docs vs `BASE_COMMIT 4a6c9a5e`; no deletions. Discovered out-of-scope: Phase A R18 remainder ~13 refs (part_09 ×9, part_07a ×3 incl. §7A.13 L699 unbalanced paren, part_03 §3.5 ×1 — matrix Phase A count 8 was undercounted) → next ed-8 batch; character_map L41 «Part 10 §10.X» generic form (matrix-unflagged, left as-is).
