...
### iter 145 — sampling-cluster-build: LAST bounded area executed (DEC-22: §7A.6 canonical, S-a…S-d); E17 re-pointed to canonical values; KI#72 closed; semantic extraction COMPLETE (7/7); first OBSERVATION application; phase v2 opened (DEC-23)

- **Scope (STATUS Next-step row 1, pinned iter 144):** the Sampling values cluster build slice (Registry B row 2 — the LAST bounded area; PLAN row sampling-cluster) per the iter-144 package §2. Owner call received: chat 2026-09-15 «я согласен с рекомендациями, сделай как лучше и качественнее» — the go-ahead on the package (single reply decides both gates — the iter-137 → DEC-19+20 precedent) → **DEC-22** (S-a…S-d) + **DEC-23** (v2 architecture phase OPEN; first slice P-a).
- **Canon (S-a/S-b/S-c/S-d):** §7A.6 — canonical «Базовые параметры» table + PP rules + «Модель-специфичные» point recs byte-unchanged (the S-a owner); `[VS: E17]` marker upgraded to the full `_README.md` §3.3 format (SHARED_REFERENCE; 12B–32B middle tier = **declared omission**; audit named). §7A.7 — the 3 contradicting param rows dropped (Temperature **0.85–1.1** / RepPen **1.0–1.05** — the KI#72 side), header → «Возможность», defer line to §7A.6 added; 5 capability rows kept (canon catches up to the deployed master shape). Hints: §9.3 12B-qualified, §9.4 model-range defer, §9.10 12B-qualified + `[ref:]`; `[VS: E12]` marker declares the numeric-fix defer (AP-5 → §7A.6 via §8.6).
- **Appendix B = the first OBSERVATION-strength application (DEC-20):** «НАБЛЮДЕНИЕ» callout on the model-capability percentages — empirical observations, not normative thresholds. Representation: base `.callout` (registered component — fence #7 clean; no new class); RULE↔ПРАВИЛО / GUIDELINE↔РЕКОМЕНДАЦИЯ mappings untouched; EXPERIMENTAL stays pending its first consuming slice.
- **Master:** E17 embed re-pointed — 32B+ column shows the canonical «32B+/API» values (Temperature **0.7–1.0**, RepPen **1.05–1.10**); the dual-side «базовая \| чеклист» splits and the orphan asterisk **removed**; re-point comment (iter 145 / DEC-22 / declared omission / audit). E12 AP-5 fix card re-framed: «Держите RepPen в диапазоне своей модели — §7A.6 (никогда > 1.10). Используйте MinP вместо повышения RepPen.» §7A.7 mirror intro = capability defer + link; §9.3/§9.4/§9.10 mirrors; Appendix B callout mirror. Root fallbacks regenerated (`parts/part_07a/08/09/appendix_model_table.html`; root `index.html` = timestamp only).
- **New MUST-PASS gate `scripts/audit_sampling_parity.py` PASS** (12 check groups, `audit_voice_parity.py` pattern): canonical record (6×3 exact) · S-b defer · master mirrors (voice-slice anchors intact) · E17 column-block checks (canonical values; sub-range/asterisk/базовая/чеклист banned in the 32B+ block; PP 0.0 ×3) · E12 re-frame · hint qualifications (every Part 9 «1.0–1.05» line carries «12B») · OBSERVATION anchors · glossary defers (registry + generated JSON) · **no competing ranges** («0.85–1.1» / «0.7–1.1» / «1.0–1.10» / «1.00–1.10» = 0 across canon + master + parts + widgets + data) · root fallbacks · map/matrix/STATUS/AGENTS parity + disposal. Wired into `AGENTS.md` canon-audits (parity **×7**).
- **KI#72 CLOSED + disposal:** the reader-visible contradiction (open since iter 120) resolved — §7A.6 is the single value owner everywhere. **8 E17/E12 prototype/extract files REMOVED_WITH_REASON** (DEC-19 at-slice-time: `elements/{E17,E12}*.html` + `component-extracts/{E17,E12}-{script,styles,visual}`; archive = git history). KI#81 row deleted from STATUS (closed iter-140 >2 iterations — lifecycle rule).
- **Registries/state:** map — NEW §5.7 (SP-1..SP-12 + accounting + deferred notes + the S-a consequence statement: the «0.85–1.1» side disappears from the live site, canonical 32B+ Temperature top = 1.0; AP-5 ≤ 1.10 framing unaffected); Registry A E17/E12 + Registry B sampling row → DECIDED (DEC-22) + EXECUTED; §6 preamble (sixth gate + phase opening); §7 log. Matrix — 8 back-pointers + R12 → EXECUTED + candidates #7 closed. DECISIONS — **DEC-22 + DEC-23**. STATUS (+2 invariants; Next step re-pinned: v2 architecture phase OPEN — P-a spec slice). PLAN (sampling-cluster → COMPLETE; NEW row v2-arch-spec). NAV §1 (E12/E17 disposal). 22 modified + 1 created + 8 deleted vs `BASE_COMMIT 28367020`.
- **Verification (executed — full battery):** build ✓ (hash `2ab607d6` unchanged) · validate ✓ · validate:master 12/12 (pre-existing warnings) · version:check 9.2.6 ✓ · tests **64/64** · qa:csp/bundle/contrast PASS · qa:doc-versions 1 pre-existing warning · qa:english **18** / qa:syntax **245** (≤ baselines; −2 — removed sub-range splits carried Markdown pipes) · sync **97/97** · drift actionable **159** (baseline exact) · parity **×7 PASS** · `git diff --check` clean.
