...
- **Scope:** agent operational layer only — no product/content/build code touched. Imported canonsim **principles** (not mechanics): authority order, preflight, anti-loop, scope discipline, Definition of Done, reproducible delivery, information ownership. Diagnosed first: iteration number duplicated across 4 files, roadmap table duplicated (STATUS + NAV §8), iter-115 detail triplicated, PLAN.md carrying stale state (KI#64 "OPEN" — closed iter-107), README stuck at iter 101, NAV carrying history.
- **`AGENTS.md` rewritten as operating law:** authority order (repository > repo state > task/plan > navigation > bootstrap prompt > agent inference — the repository wins over the prompt) · 7-step preflight (BASE_COMMIT, never assume a clean worktree, preserve pre-existing changes) · anti-loop rules (same failed approach twice → KI + stop; third blind retry forbidden; doc-loop alarm) · scope discipline (discovering a problem does not grant permission to fix it: observe → record → defer → continue) · task-type reading gradient (content/widget/CSS/build/data/refactor/meta) · honest QA semantics (aggregate `pnpm run qa` exits 1 by design — english=19, syntax=247 are fixed baselines) · Definition of Done (5 conditions — green tests alone ≠ Done) · delivery discipline vs BASE_COMMIT · doc caps.
- **`AGENT_NAVIGATION.md` → current-state map:** "describes what IS, not what happened" header law; iteration number removed; §2 build-hash wording fixed (scope verified in code: hash = sha256 of `src/shell/index.html` only); §8 roadmap history table (iter 81–115) + §9 "v9.1 Restructure Changes" narrative REMOVED (history → worklog/CHANGELOG/git); NEW §10 Information Ownership (anti-drift map — one fact, one owner); NEW pitfall #27 (display artifacts vs byte-level verification).
- **`STATUS.md` → state snapshot:** compact Current State (detail → worklog); NEW authoritative **Next step** (owner-gated rows are not auto-candidates); roadmap removed (→ PLAN.md). **False positive withdrawn in-iteration:** workflow branch filters initially looked corrupted (`branches: ain]` in terminal output) — byte-level verification proved them intact (`branches: `[main]``, counts 2/2/1); the artifact: terminal output eats the `[m` sequence. No KI opened; lesson = pitfall #27.
- **`PLAN.md` repurposed:** dead docs-restructure plan → live backlog; every row now carries Task · Scope · Acceptance criteria · Required verification · Owner gate. **`DECISIONS.md` created** (append-only, `DEC-NN`): 13 seed decisions previously scattered. **`README.md`:** stale "iter 101" status line → STATUS.md pointer.
- **Files (8, all doc-only):** `AGENTS.md` · `AGENT_NAVIGATION.md` · `STATUS.md` · `PLAN.md` · `DECISIONS.md` (NEW) · `README.md` · `CHANGELOG.md` · `worklog.md` (iter-113 collapsed to one-liner). No `src/`, `data/`, `scripts/`, `tests/`, workflows touched.
- **Validation:** baseline re-verified BEFORE edits at BASE_COMMIT `c212a024` (build PASS hash `2ab607d6` · tests 64/64 · version:check PASS · canon sync 97/97 · drift exit 0 · qa:csp/bundle/contrast/doc-versions PASS · english 19 · syntax 247). After edits: doc-only — `version:check` PASS · `qa:doc-versions` PASS · hash unchanged · root fallbacks untouched (exploratory build's timestamp churn in `index.html` restored) · committed with `SKIP_ARTIFACT_BUILD=1` per doc-only convention.
