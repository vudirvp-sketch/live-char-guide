...
### iter 138 — owner-gates-ratified + §6.1 disposal: all four map-§6 gates DECIDED (DEC-19/DEC-20); visual-markup ownership locked; 16 stale prototype/extract files removed

- **Scope (STATUS Next-step row 1, pinned iter 137):** the four open owner decision points (map §6). Owner chat 2026-09-14 «начинай работу по планам» = the go-ahead on the iter-137 recommended package — recorded with verbatim provenance (DEC-18 chat-directive precedent; every gate called, none auto-passed).
- **Decisions recorded:** **DEC-19** (§6.1+§6.4, compound per the ~20-row cap): master embed (L2) = the canonical visual markup; `visual-system/` L4/L5 = frozen design artifacts (archive = git history + recorded reasons); per-family stale copies → `REMOVED_WITH_REASON` at slice time; hero = remove/archive (disposal slice pending; KI#81 cleanup rides it). **DEC-20** (§6.2+§6.5): E13 = `TEXTUAL_CANONICAL` — the unique symptom→check→branch→AP/E mappings canonicalize in §9.6 (canon §9.6 L139 «визуальная версия дерева» claim becomes true), E13 = visual presentation / decision aid; RULE/GUIDELINE/OBSERVATION/EXPERIMENTAL ratified + UNKNOWN = temporary only, incremental application (ПРАВИЛО/РЕКОМЕНДАЦИЯ ↔ RULE/GUIDELINE; OBSERVATION/EXPERIMENTAL markup = first consuming slice, fence #7).
- **§6.1 disposal slice executed:** 16 files REMOVED_WITH_REASON — `visual-system/elements/` E01/E08/E10/E15 prototypes + `integration/component-extracts/` ×12 (stale second copies of parity-locked embeds; archive = git history). Pre-deletion dependency audit: build/test path untouched (grep-verified — no build/validate/test consumer of `visual-system/`; `qa:contrast` reads only `tokens.json`, kept); `audit_component_extracts{,_css}.py` degrade to MISSING (pre-existing `exists()` guards, verified post-deletion); production `Source: elements/E##-*.html` provenance comments resolve to git history (frozen v1 — untouched).
- **Registry + tooling truth-up:** map §2 L4/L5 → frozen; §6 rows 1/2/4/5 → DECIDED with DEC pointers; Registry A (E01/E08/E10/E15/hero/E13) + Registry B (rows 1/4/6) → executed/decided; §7 iteration log. Three family parity audits' deferred-layer notes → executed (E08 / E01+E15 / E10 — the recommendation's "four parity audits" = four families on three scripts; glossary audit carries no prototype notes). NAV §1 + architecture.md visual-system rows → frozen; STATUS +2 invariants (visual-markup ownership; rule-strength) + KI#80 deleted (lifecycle rule: closed iter 134 > 2 iterations); PLAN: mig-3 gate → CLEARED (DEC-20) + hero-disposal row added; STATUS Next step re-pinned (mig-3 unblocked = next).
- **Verification (executed):** four cluster parity audits PASS · canon sync **97/97 PASS** · drift informational (actionable 160 = 160 unchanged) · extracts audits run clean (MISSING, no crash) · `git diff --check` clean · `git status --short` = intended files only. No node battery — no `src/`/`data/`/build-path file touched (iter-131 doc-only precedent; `SKIP_ARTIFACT_BUILD=1` applicable per DEC-11). 11 modified + 16 deleted vs `BASE_COMMIT 5de8fc9c`.

### iter 137 — owner-gates-recommendation: decision package for the four open map-§6 gates prepared, verified, recorded (doc-only)

- **Scope (owner chat directive):** analyze the four open owner decision points (map §6: §6.1 visual-markup ownership, §6.2 E13, §6.4 hero, §6.5 rule-strength) and the owner's drafted decision package; refine with verified evidence; record. Doc-only — no gate passed, nothing marked DECIDED, no DEC-NN assigned (DECISIONS.md records decisions only after the owner answers).
- **Fact check — all four calls CONFIRMED against the repo:** §6.1 (93% prototype↔embed identity = 494/532, foundation §4.2; no L4/L5 sync tooling; E10 three-way drift + E01/E05/E06 russification lag; foundation INFERENCE: master embeds = de-facto production canonical) · §6.2 (E13 carries unique symptom→check→branch→AP/E mappings for 3 symptoms absent from §9.5/§9.6, foundation §4.4; P/I 40/40) · §6.4 (never integrated — no hero section in the shell index, INTEGRATION-MAP row unexecuted) · §6.5 (vocabulary already owner-directed, foundation §8.4; research §19 four-class table).
- **Refinements recorded:** §6.1 disposal mechanics in the repo's vocabulary (L4/L5 frozen design artifacts; stale copies → `REMOVED_WITH_REASON` at slice time; archive = git history; runnable consequence = mechanical disposal slice for E08+E01+E15+E10) · §6.2 (canon §9.6 L139 already claims E13 is its visual version — TEXTUAL_CANONICAL makes the existing claim true; mig-3 rides the established slice pattern) · §6.5 (ПРАВИЛО/РЕКОМЕНДАЦИЯ ↔ RULE/GUIDELINE continuity; OBSERVATION/EXPERIMENTAL new — markup representation = first consuming slice; **sampling cluster additionally needs the KI#72 value decision**) · call order **§6.1 → §6.2 → §6.5 → §6.4** (unblock value).
- **New evidence (byte-verified):** hero runs on Three.js via `cdn.jsdelivr.net` importmap (the dependency class iter-113 removed); dead `.vs-hero-placeholder` CSS stub in `src/shell/styles.css` (5 rules, zero consumers; Phase-4 commit `0addf38d`) → **KI#81 opened**, cleanup deferred to the §6.4 disposal slice.
- **Recorded:** new `docs/research/owner_gates_iter137.md` (full package) · map §6 preamble + per-item recommendation lines + Registry A hero/E13 notes + §7 log · STATUS (iter 137, KI#81, Next step re-pin) · NAV §7 row · worklog. 6 files vs `BASE_COMMIT 9b8d3138`; no deletions. Doc-only verification: `git diff --check` clean; no battery (no production file touched, iter-131 precedent).

### iter 136 — mig-5-enneagram-data: fourth bounded area executed — single canonical value owner locked (§5.4 ↔ generated data layer ↔ E10)

- **Scope (STATUS Next-step row 1, pinned iter 135):** execute the Enneagram data v2 build slice on the mig-1 generator pattern (Registry B row 6; foundation §5.6; PLAN row mig-5, LOW-MEDIUM gate — derivation direction pre-set by DEC-17/DEC-18 map §2 L6; LIE fold mechanical). §5.4 stays the canonical record (9-type table, 7 columns — **unchanged**, canon + master table untouched).
- **Design decision (in-slice, PLAN-sanctioned):** no parallel canon-side registry — §5.4 already IS the canonical record; the new generator `scripts/generate_enneagram.mjs` (wired into `pnpm run build` after the glossary stage) owns the machine-layer supplement (SPINE WANT/NEED/GHOST templates, stress/growth directions, wings, ocean_correlation, ocean_defaults, flaw_anchors, mbti_suggestions) — the single hand-edited source for every non-canonical field.
- **`data/enneagram.json` becomes GENERATED (schema 2.1.0):** canonical fields (name/name_alt/core_fear/core_desire/lie_template/flaw_pattern) = §5.4 verbatim — the v1 extended variants («, дефектным», «Работоголизм, поверхностность…», «=»→«равна» rewrites) dropped as non-canonical drift per the map disposition «derived from §5.4 — single internal copy of each value»; **LIE folded to ONE internal copy** (`types[].lie_template`; `spine_templates.LIE` removed — the v1 second copy carried divergent wording); `spine_templates[].FLAW` emitted from the same §5.4 cell as `flaw_pattern` (single parsed source, E15 derived-value lock pattern); `spine_templates[].NEED` == `types[].need_direction` (single supplement source); deterministic output (2-space + trailing newline).
- **Widget re-points:** `enneagram-builder.js` (SPINE tab L3 field) + `persona-synthesis.js` (generateNarrative) read the LIE from `types[].lie_template`; headers declare the v2.1.0 generated contract. `vs-e10-enneagram.js` **unchanged** (PLAN: consumes the generated file unchanged — field shape preserved).
- **E10 mini-cards → SHARED_REFERENCE:** master `part_05.html` static defaults aligned to §5.4 type-4 canonical values (fear «Быть обычным, незначимым», desire «Быть уникальным, аутентичным», flaw «Отстранённость»; `parts/part_05.html` regenerated); embed comment declares the mig-5 §5.4 derivation (hexad graph = VISUAL_CANONICAL); canon `[VS: E10]` marker extended with the ownership declaration (E01/E15 marker precedent). §5.2 Elena 6w5 directions (6→3 / 6→9) parity-locked against the generated layer.
- **New acceptance gate `scripts/audit_enneagram_parity.py` PASS (11 checks + deferred-layer notes):** canon §5.4 == hardcoded acceptance invariants (9×6 fields); generated layer currency + shape (spine keys exactly WANT/NEED/FLAW/GHOST; FLAW == §5.4 cell; NEED == need_direction; version; deterministic formatting); single internal LIE copy (9×1 occurrences); generator + package.json wiring; E10 embed defaults + in-embed declaration; canon marker tokens; widget read-paths (no `template.LIE` reads remain; simulated read fills every consumed field 9/9 types — the «widget works (data shape verified)» acceptance); root fallback current; map §5.4 EN-1…EN-8 + Registry A/B statuses; matrix back-pointers ×3; §5.2 ↔ type-6 directions. Deferred: prototype/extract E10 (§6.1-blocked); hexad geometry (VISUAL_CANONICAL); unconsumed machine fields (wings/wing_pairs/need_direction/stress_direction/growth_direction — informational, EN-8).
- **Map + matrix:** §5.4 Enneagram data slice seeded + executed (EN-1…EN-8); Registry A E10 + Registry B row 6 + §2 L6 → executed status; §7 iteration log. Matrix: back-pointers `→ migration_map_v2 EN-1/EN-2/EN-8` on `p5_enneagram_basics::04/::02` + `p5_elena_profile::04` (duplication now DERIVED, not copied).
- **Verification (executed):** enneagram parity audit **PASS** · glossary + core-directives + token-budget parity PASS · full build OK (hash `2ab607d6` unchanged — shell untouched) · validate ✓ · validate:master 12/12 · version:check 9.2.6 ✓ · tests 64/64 · qa:csp/bundle/contrast/doc-versions PASS · qa:english **18** / qa:syntax **247** (baselines exact) · canon sync **97/97 PASS** · drift informational (actionable 160 = 160 unchanged) · `git diff --check` clean. pnpm absent in the sandbox — scripts invoked via `node`/`python3` exactly as package.json defines. 20 files vs `BASE_COMMIT 27de84b2`; no deletions.
