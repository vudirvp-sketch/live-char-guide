...
### iter 123 — ru-policy: two-layer guide language policy (owner request, full russification)

- **Scope (owner request in chat 2026-09-14: «все русифицировать что можно»):** replace the pre-iter-123 "English semantic anchors" language rule with a two-layer policy and russify all russifiable guide prose. New law: `AGENTS.md` **fence #12** (two-layer, DEC-16) · `AGENT_NAVIGATION.md` pitfall #4 · `docs/canon/_README.md` §3.8/§3.9 · `docs/terminology_dictionary.md` (§1/§6 + Category table) · `DECISIONS.md` **DEC-16**.
- **Layer 1 (russified):** guide prose, headings, callout labels — `RULE → ПРАВИЛО`, `RECOMMENDATION → РЕКОМЕНДАЦИЯ`, `EXAMPLE → ПРИМЕР`, `TEMPLATE → ШАБЛОН`, `Bridge → Переход`, `Synthesis → Синтез`, `Demonstrates → Демонстрирует`; ordinary terminology — `token → токен`, `lie → ложь`, `GHOST Layers → слои призрака` (gloss at first mention), `CoT Bridge/Basics/Tiers CoT → русские заголовки`, `No Anti-godmoding → Отсутствие анти-годмодинга`, `SPINE Framework (h2) → Обзор SPINE`, `System Prompt: Structure and Assembly (h2) → русская форма`, `### Bridge (part_09) → ### Переход`, `Nested-Anchor labels → вложенный/атомарные якоря`, `canonical location → каноническое место`, `cross-ref → кросс-ссылка`.
- **Layer 2 (preserved English):** executable/recommended prompt content (SP blocks, `<CORE_DIRECTIVES>` canonical wording, Tone Frame strings, card XML, copy-paste templates — each with adjacent Russian explanation); technical identifiers (section IDs, XML tags, class names, card field names, parameter names); proper names; `<!-- canonical: ... -->` metadata comments; VS-EMBED internals; glossary dictionary headwords («**GHOST Layers (слои призрака)**» bilingual entry format kept).
- **Coverage:** 17 canon files (all `docs/canon/*.md` content + `_README.md` policy sections) · 14 `src/master/*.html` (mirrored canon; `audit_canon_master_sync.py` expectations updated for 4 russified strings — policy encoded in validation) · 15 root fallbacks (`parts/*.html` + `parts/manifest.json`, regenerated via `pnpm run build`; `index.html` timestamp churn restored per DEC-11). No deletions; no `data/`/`src/shell/`/widget changes.
- **English leak baseline re-baselined 19 → 18:** one baseline leak was ordinary prose (translated legitimately); the remaining 18 are executable-content leaks by design. Baseline updated in `AGENTS.md` (fence #12 + verification cheat-sheet), `AGENT_NAVIGATION.md` pitfall #4, `docs/architecture.md`, `DECISIONS.md` DEC-16; `PLAN.md` ed-1 row criterion `qa:english ≤ 19` → `≤ 18`.
- **Validation (after edits):** build PASS hash `2ab607d6` unchanged · validate 5/5 + SHELL-* · validate:master 12/12, 0 errors, 17 pre-existing warnings (identical to BASE) · version:check 9.2.6 · tests 64/64 · canon sync **97/97** · qa:csp/bundle/contrast PASS · qa:english **18** (≤ re-baselined baseline) / qa:syntax 247 (unchanged) · qa:doc-versions 8/8 parsed, 1 pre-existing warning · lint 0 errors (1 pre-existing warning in untouched `vs-e10-enneagram.js`, verified identical at BASE) · `git diff --check` clean · drift (informational) 133 → 159 actionable — paragraph-similarity artifacts of the mass translation, exit 0, MUST-gate sync unaffected; vs `BASE_COMMIT 49db7eab`, 56 intended files, no deletions.

### iter 122 — ed-policy: Editorial Policy adopted as content-editing law (doc-only, owner decision)

- **Scope (owner-gated row executed — owner decision received in chat 2026-09-14: policy accepted with amendments):** adopt the editorial rule set as content-editing law. Guide content untouched (policy text only). Files: `AGENTS.md` · `docs/canon/_README.md` · `DECISIONS.md` · `PLAN.md` · `STATUS.md` · `worklog.md` · `CHANGELOG.md`.
- **Law (verbatim owner wording):** `AGENTS.md` → **Editorial Policy (content-editing law)** section + **fence #13** (compact normative pointer: compress redundant presentation, never unique capability · 5-point functional-load check — capability/coverage/retrieval/context/dependency · UNCLEAR ≠ delete · success ≠ word count). The reading gradient's Content/canon-change row now routes through the policy. Source lineage: research §27 + §20/§22/§24/§32; the owner's amended text supersedes §27 as the operative wording.
- **Canon-side application:** `docs/canon/_README.md` §4.4 (canon-first: checks pass on canon before master mirrors; compression = replacement per DEC-03; UNCLEAR → PLAN/owner, not deletion) + anti-pattern #8 + validation-checklist item.
- **Conflicts vs existing law — resolved explicitly by the owner's amendments, recorded in DEC-15:** functional repetition preserved when serving a distinct function (subsumes the useful-repetition whitelist: Price / SPINE causality / Show Never Tell / Embodiment); compression-as-replacement consistent with `viz > dry text` (DEC-03); cross-ref replacement rules consistent with IMP-48 (fence #11).
- **Acceptance:** rule set cited once in owner doc (full text only in `AGENTS.md`; `_README.md` cites without restating — one fact, one owner) · content iterations reference it (`PLAN.md` preamble binds `ed-1`…`ed-8`, `dupes-1`, `ki-72`) · conflicts resolved + recorded (DEC-15).
- **Validation (doc-only):** `version:check` PASS (9.2.6) · `qa:doc-versions` exit 0 (1 pre-existing obs-3 warning, unchanged) · `git diff --check` clean · 7 intended files vs `BASE_COMMIT b39b194c`, no deletions, no `src/`/`data/`/build outputs → commit with `SKIP_ARTIFACT_BUILD=1`.
