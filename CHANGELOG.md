...
### iter 121 — ed-6: reader/repo-metadata separation + KI#71 (content, canon-first)

- **Scope (non-gated backlog continuation, owner directive):** `STATUS.md` Next step #5 — **ed-6** (strip repo-internal references from reader-facing prose) executed as the natural carrier for the **KI#71** one-line fix and the stale §7A.12 canon note (dupes-1 second half). Owner-gated rows untouched.
- **Edits (canon-first, mirrors only where mirrors exist):** `docs/canon/part_07a.md` — KI#71 duplicate identity-name RULE line deleted (master carried it once, untouched by the fix); §7A.1 stale `[ANCHORS]`/KI#58 drift note rewritten to the 1-sentence reader-useful canonical-format note (Anchors = `<anchors>` XML с категориями — substance kept per acceptance criterion; Part 2 defines placement, not the XML format, so §7A.1 remains the format's canonical location); §7A.2 `(D4, iter 93)` convention marker stripped; §7A.12 stale iter-11 migration note deleted (described plain-copy block no longer exists). `docs/canon/part_01.md` — §1.2 Mermaid/iter-14 history + `AUTO_TOC_PLACEHOLDER`/build-скрипт internals stripped, reader function kept («Оглавление — единственный навигационный артефакт»). `docs/canon/part_08.md` — §8.1 «убрана в iter 55 … (KI#38 ✅ CLOSED)» history sentence removed. Master mirrors: `src/master/part_07a.html` (§7A.1 + §7A.2), `src/master/part_08.html` (§8.1).
- **Acceptance (ed-6 criterion):** rendered guide prose carries **zero `KI#…` / `iter NN` references** — sweep over `src/master/` + `parts/` + `dist/` returns a single hit inside the E18 VS-EMBED provenance **HTML comment** (`part_07b.html:39`, byte-verified non-rendering). Non-rendering repo-meta left per scope discipline, recorded for Phase B: `part_06.md` L83, `part_00.md` L13, `appendix_character_map.md` L29.
- **Files (5 content + 3 regenerated + 5 state):** `docs/canon/part_01.md` · `docs/canon/part_07a.md` · `docs/canon/part_08.md` · `src/master/part_07a.html` · `src/master/part_08.html` · auto-regenerated: `parts/part_07a.html` + `parts/part_08.html` + `index.html` (build timestamp) · `STATUS.md` (iter 121, KI#71 CLOSED, KI#69 deleted per lifecycle) · `worklog.md` (iter-111 dropped per cap) · `CHANGELOG.md` (iter-118 collapsed) · `PLAN.md` (ed-6 + ki-71 → Completed; dupes-1 re-scoped; ed-matrix Phase B note) · `docs/research/editorial_matrix.md` (R17 registry updated).
- **Validation:** baselines BEFORE edits at `BASE_COMMIT 6fe532b6` (drift actionable 134, canon-only sections 3, sync 97/97). After edits: build PASS hash `2ab607d6` unchanged · validate 5/5 + SHELL-* · validate:master 12/12 · version:check 9.2.6 · tests 64/64 · canon sync 97/97 · drift exit 0 (actionable 134→133 — repo-meta removal reduces drift) · qa:csp/bundle/contrast PASS · qa:english 19 / qa:syntax 247 (baselines unchanged) · qa:doc-versions 8/8, 1 pre-existing warning · lint 0 errors · `git diff --check` clean.
