...
### iter 153 — v2-build-part7a: Part 7A (System Prompt & Assembly) v2 build slice — the largest so far (13 sections / 94 rows, executed whole): F2 matrix seeding (86 `open` + 4 `canonical-link` + 2 `collapsible` + 2 `—`); canon: the R01 #8 §7A.11 fold (the three cluster-owner records byte-stable); master: the fold mirror; drift 155 unchanged

- **Scope (STATUS Next-step row 1, pinned iter 152):** the Part 7A (System Prompt & Assembly) v2 build slice — PLAN row v2-build-part-7a, **not owner-gated** (the DEC-24 ratification gate cleared; per-Part slices = the pinned track). Executed per the ratified spec §7 shape on the iter-147…152 precedent (maps §5.8–§5.13 → this slice's map §5.14, P7A-1..P7A-5). The largest slice so far — 13 sections / 94 matrix rows — **executed whole** (the sanctioned sub-slicing option not needed: the canon/master surface proved bounded — one fold + one mirror; decision recorded in worklog).
- **Evidence (P7A-1 — F2 seeding, seventh application):** the thirteen matrix Part 7A tables (§4) re-headed to 9 columns; 94 rows dispositioned — **86 `open`** (canonical definitions, RULE/REC callouts, templates, canonical tables, the §7A.13 walkthrough = Build+Learn required reading; the §7A.13::10 optional extensions considered for `collapsible` and rejected: the text list = the accessible-text counterpart of the E02 embed's optional-branch section) + **4 `canonical-link`** (§7A.1::06 — the iter-125 dupes-1 fold; §7A.9::01 — the compliant R01 short+refs intro; §7A.13::05 — the iter-134 R02/mig-2 shorthand — all three re-verified at HEAD; + §7A.11::05 — the fold executed this slice) + **2 `collapsible`** (both pre-existing deployed F6 usages documented: §7A.6::06 the model-specific recommendations table + §7A.13::11 the recap checklist) + **2 `—`** (blocks disposed pre-slice by executed decisions — the KI#71 duplicate line + the §7A.12 migration note, both ed-6 iter 121; the `—` historical-row rule documented in §1.2). Matrix §1.2 scope note extended to Parts 1–7A; the Part 7A verdict appended with the iter-153 slice note; **seven stale notes refreshed** (the KI#71/ed-6/iter-125/iter-128 pre-fix states). Count script-verified: 94 = 86+4+2+2.
- **Canon (P7A-2 — the R01 #8 fold, the slice's only content edit):** `part_07a.md` §7A.11 — the Anchors-placement conceptual sentence («Концептуально якоря — отдельный блок в Examples-зоне (см. правило §1.4 и правило §7A.1)») folded into the inline pointer «— концепция и полное правило размещения: `[ref: part_01.md §1.4 — правило]`»; the 4K substance kept verbatim (list form allowed; the 8K+ `<anchors>`-tag tail). 5-point check passed (documented in map §5.14 P7A-2); the Part-4 R21-#16 iter-150 precedent. **Byte-stability locks honored:** `git diff -U0` = exactly 1 line changed; the §7A.2 (CORE DIRECTIVES, mig-2) / §7A.6 (sampling, DEC-22) / §7A.12 (token budget, mig-4) cluster-owner records + the E08/E17 presentation chains untouched — the three cluster parity audits PASS (the iter-149 §3.2 / iter-151 §5.4 precedents). The R01 family now COMPLETE on the canon side (§7A.11 was the last full-form non-owner occurrence).
- **Master (P7A-3 — the fold mirror):** `part_07a.html` §7A.11 callout — the same fold; `[ref: …]` rendered as `<a href="#p1_core_rules">§1.4 правило</a>` (the master §7A.1::06 anchor convention). 1 line changed.
- **Observations (P7A-4 — recorded, deferred):** five items — the §7A.13 master-only intro sentences; the §7A.7 family variances (master by-model-tier `<ul>` vs the canon `Возможность` table + CoT-tier drift «Tier 0–1 / Tier 1–2» vs «✗ / Tier 1 / Tier 2–3» + the E17 checklist values incl. Anti-godmoding API «2 строки» vs canon «1 строка» — pre-existing, ungated; needs a reconciliation owner call); the recap-checklist label variance («Recap-» vs «Сводный»); the §7A.5 render variance (table vs `<ul>`); the E02/E16 canon-declaration gaps (Registry A PROPOSED). Recorded as OBSERVATIONS in map §5.14 (ride the canonical-audit stage). No KI (registry rows, not defects).
- **Registries/state:** map — NEW §5.14 (P7A-1..P7A-5 + the F2 presentation map + accounting + deferred + drift attribution) + §7 iteration log; matrix — the presentation column + the slice note + the stale-note refreshes; STATUS (iter 153; the v2-architecture invariant + Next-step preamble extended to Parts 1–7A / iters 147–153 / maps §5.8–§5.14; iter-153 KI-note; Next step → the Part 7B slice); PLAN (v2-build-part-7a → COMPLETE; NEW row v2-build-part-7b); worklog (iter-152 → one-liner; iter-143 dropped — ≤10 cap); CHANGELOG (this entry; iter-152 collapsed to one-liner — the Document-caps trim); NAV §7 de-staled (Parts 1–7A). Zero infrastructure touches (no script/tool changes).
- **Verification (full battery, executed):** build ✓ (hash `2ab607d6` unchanged; root fallback `parts/part_07a.html` regenerated carrying the fold — grep-verified src+root) · validate ✓ · validate:master ✓ · version:check 9.2.6 ✓ · tests **64/64** (sandbox `PUPPETEER_EXECUTABLE_PATH` workaround — env-only) · qa:csp/bundle/contrast PASS · qa:doc-versions 1 pre-existing warning · qa:english **18** (baseline exact) · qa:syntax **238** (baseline exact — the content delta contributes 0) · sync **97/97** · drift actionable **155** (unchanged — the folded paragraph keeps its canon↔master match; verified by the JSON pre/post diff) · parity **×7 PASS** · `git diff --check` clean.
