...
(function() {
  ...
  function isSpineComplete() {
    // Simplified: For Track B+, always consider SPINE as "started" to show GHOST teaser
    // This removes the prerequisite barrier while keeping the layered disclosure pattern
    const track = window.NavigationState?.getTrack() || 'B';
    if (track === 'A') return false;  // Track A never shows GHOST
    return true;  // Track B+ always shows at least teaser
  }
