...
**iter 136 — mig-5-enneagram-data (fourth bounded area executed, Registry B row 6).** The Enneagram data cluster now has one canonical value owner end-to-end: §5.4 = the canonical record (9-type table, 7 columns — **unchanged**, canon + master table untouched); `data/enneagram.json` = **GENERATED** by `scripts/generate_enneagram.mjs` (wired into `pnpm run build` after the glossary stage; schema 2.1.0) — canonical fields (name/name_alt/fear/desire/LIE/flaw) derive from §5.4 verbatim (v1 extended variants dropped as non-canonical drift), machine-layer supplement (SPINE WANT/NEED/GHOST, directions, wings, ocean_correlation/defaults, flaw_anchors, mbti_suggestions) lives in the generator — the in-slice design decision, no parallel canon registry created; **LIE folded to ONE internal copy** (`types[].lie_template`; `spine_templates.LIE` removed — the v1 copy carried divergent wording); `enneagram-builder.js` + `persona-synthesis.js` re-pointed to `types[].lie_template`; `vs-e10-enneagram.js` unchanged (shape preserved); E10 mini-cards = `SHARED_REFERENCE` (master static defaults → §5.4 type-4 values, embed comment + canon `[VS: E10]` marker declare the §5.4 ownership + generated layer; hexad graph = VISUAL_CANONICAL); §5.2 Elena 6w5 directions parity-locked. Acceptance gate `scripts/audit_enneagram_parity.py` PASS (11 checks). Prototype/extract E10 stays §6.1-gated. Verification (executed): enneagram parity PASS + full battery — build hash `2ab607d6` unchanged (shell untouched) + validate + validate:master 12/12 + version:check + tests 64/64 + qa gates (english 18 / syntax 247 — baselines exact; csp/bundle/contrast/doc-versions PASS) + canon sync 97/97 + actionable drift 160 = 160 + all four cluster audits PASS + `git diff --check` clean. pnpm absent in the sandbox — scripts invoked via `node`/`python3` directly (identical commands); the owner's pre-commit hook re-runs the battery.
...
1. **Owner decision points remaining (map §6, recommended order) — the migration track now waits on owner calls:** (a) visual-markup ownership (§6.1 — gates the prototype/extract disposals of **four executed families**: E08 + E01 + E15 + E10, all §6.1-deferred with parity locks in place); (b) E13 diagnostic mappings (§6.2 — gates mig-3, the Diagnostics cluster / Debug mode — the last unexecuted Registry B bounded area with a ready plan); (c) hero prototype disposition (§6.4); (d) rule-strength convention (§6.5 — needed before Part 7A/8 v2 slices; the sampling-cluster presentation waits on it + the KI#72 value decision).
2. **mig-3 — Diagnostics cluster (Debug mode):** blocked on the E13 decision (§6.2) — not an auto-candidate until the owner calls it.
3. **Superseded (per owner directive — do not auto-resume):** `Fork D 2/3` (sampling widget), `ki-70` (standalone; v1 wire/drop superseded by the v2 Reference design — the glossary slice already serves the Reference mode via panel/no-JS), `Fork D 3/3`, `ki-72` (as content decision — the KI#72 *value* decision resurfaces inside the migration's sampling-cluster disposition; 4 live locations after iter 133), `dupes-2` and equivalent sequential cleanup batches. `ki-79`, `cache-bust-1`, `vs-e09-hex`, obs-3/4/5/6: deferred unless a migration slice requires them.

A new agent: run the preflight in `AGENTS.md`, confirm this Next step against the worktree (`git log` vs iteration above), then start. iter-136 (mig-5 Enneagram data build slice) is complete and verified — four bounded areas now live (glossary Reference chain + CORE DIRECTIVES single presentation + Token budget single value owner + Enneagram single value owner). Four cluster audits MUST PASS: `python3 scripts/audit_glossary_parity.py`, `python3 scripts/audit_core_directives_parity.py`, `python3 scripts/audit_token_budget_parity.py`, `python3 scripts/audit_enneagram_parity.py` (all wired into `AGENTS.md` canon-audits). `data/glossary.json` is **generated** (`pnpm run build:glossary`) and `data/enneagram.json` is **generated** (`pnpm run build:enneagram` / inside `pnpm run build`) — never hand-edit; term changes go through `docs/canon/glossary_registry.md`, enneagram canonical values through `docs/canon/part_05.md` §5.4, the enneagram machine-layer supplement through `scripts/generate_enneagram.mjs`. v1 remains frozen as migration source — do not edit `docs/canon/part_*.md`/`appendix_*.md` or `src/master/` content without an explicit migration-slice scope.
