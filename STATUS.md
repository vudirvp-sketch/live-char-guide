...
**iter 129 — ed-7-family editorial batch + R17 [B] fold (PLAN row `dupes-2` third batch, non-gated under DEC-15; owner chat directive «продолжай работу последней итерации»).** Executed matrix candidates **#4 + #5** (Part 3 §3.1, canon-first + master mirrors): **#4 R05** — Методология disclaimer compressed to 1-sentence pointer to §1.1 (`[ref: part_01.md §1.1]`, master → `#p1_value_proposition`) + the unique §3.2 note kept locally; empirical detail (~50 cards / model list / 128K caveat) remains canonical in §1.1. **#5 R03** — Pattern Matcher re-teach compressed to 1 sentence + ref §1.4 (`#p1_core_rules`); applied outcome contrast kept («директива будет проигнорирована, паттерн — скопирован»). **R29 terminology ×4 fixed:** glossary G-GHOST «G2=Отрочество»→«G2=Юность» (canon+master) · glossary C-CoT «Tier 0 (basic Anchor)»→«Tier 0 (без отдельного CoT-блока)» (canon; master already carried the aligned long form) · §7B.5 checklist «призрак Anchors»→«якоря призрака (GHOST-якоря)» aligned to the L257 RULE (canon+master) · appendix_mbti «NOTE:»→«Примечание:» (canon-only, DEC-16). **R17 [B] remainder executed (family complete):** part_00 L13 «Создан: iter 38 — KI#21» preamble deleted (non-rendering) · part_08 L27 «в v9 restructure» repo-meta phrase stripped, redirect substance kept (canon+master). Sync-audit substrings P3-6 + P1-3 updated to the new wordings (established pattern: iters 94/105/107/123/127). **KI#78 opened** (master part_07b.html L407 «поведенческие якоря (поведенческие якоря)» paren dupe — discovered during R29 checklist work, out of matrix scope → deferred). **Verification (executed):** build SUCCESS hash `2ab607d6` unchanged · canon sync **97/97 PASS** · actionable drift set unchanged (159 = 159, hash-only shifts on pre-existing drift blocks) · validate + validate:master ✓ · version:check sync · tests **64/64** · qa:csp/bundle/contrast/doc-versions PASS (1 warning = pre-existing obs-3) · qa:english **18** / qa:syntax **247** (baselines exact) · `git diff --check` clean · root fallbacks `parts/part_03|07b|08|appendix_glossary.html` regenerated + index.html timestamp churn kept (real build — iter-121/127 precedent).
...
1. **Owner's choice (see `PLAN.md` for scope/acceptance/verification):** dupes-1 — COMPLETE iter 125; ed-matrix Phase B — COMPLETE iter 126; ed-8/R18 Phase B ref-repair batch — COMPLETE iter 127 (33 refs + 6 parens + KI#77-a–d); **ed-8/R18 Phase A remainder + R17 strips #14/#15/#17/#18 — COMPLETE iter 128** (16 refs + 1 paren + 4 strips); **ed-7-family (R05 #4, R03 #5, R29 ×4) + R17 [B] remainder — COMPLETE iter 129**. Remaining owner choice narrows to **Fork D 2/3** (sampling widget; executes with infrastructure approval) or a new row.
...
5. **Non-gated, ready under the Editorial Policy (matrix output): remaining 9 open candidates (#6, #8, #10, #11, #13, #16, #19, #20 + the R04 pair).** Small batches: ed-4-family (R06 #6), R01 #8 (§7A.11 fold), R21 #16 (§4.7 fold), R15 #11 (ed-3 fold), #13 (§9.7 compress); KI#77-e (character_map budget ranges — needs the ed-5 framing decision); **KI#78** (1-line master fix, fold into any master-touching iteration). Sequence by owner choice or as dupes-N-style batches.
...
A new agent: run the preflight in `AGENTS.md`, confirm this Next step against the worktree (`git log` vs iteration above), then start. iter-129 (ed-7-family + R17 [B] fold) is complete and verified; the owner-gated rows above are unchanged.
