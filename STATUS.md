...
**iter 137 — owner-gates-recommendation (doc-only).** The four open owner gates (map §6) now carry a prepared recommendation package: `docs/research/owner_gates_iter137.md` (agent-prepared, **NOT decisions** — every gate stays owner-called). Recommended calls: §6.1 master embed = canonical visual markup / L4-L5 frozen (four executed families' stale copies → `REMOVED_WITH_REASON` at slice time) · §6.2 `TEXTUAL_CANONICAL` (E13 mappings → §9.6; unblocks mig-3) · §6.5 ratify RULE/GUIDELINE/OBSERVATION/EXPERIMENTAL + UNKNOWN-as-temporary-only, incremental application (no mass annotation) · §6.4 hero remove/archive. Recommended call order: §6.1 → §6.2 → §6.5 → §6.4 (unblock value; §6.4 gates nothing). New verified evidence: hero runs on Three.js via `cdn.jsdelivr.net` importmap (the dependency class iter-113 removed); dead `.vs-hero-placeholder` CSS stub in `src/shell/styles.css` (5 rules, zero consumers) — **KI#81 opened**, cleanup deferred to the §6.4 disposal slice. Map §6 preamble + per-item recommendation lines + Registry A hero/E13 notes + §7 iteration log updated. Doc-only iteration: no production file touched, no battery (iter-131 precedent); `git diff --check` clean.
...
1. **Owner decision points remaining (map §6) — the migration track now waits on owner calls; a recommendation package is prepared (iter 137, `docs/research/owner_gates_iter137.md` — NOT decisions):** (a) visual-markup ownership (§6.1 — gates the prototype/extract disposals of **four executed families**: E08 + E01 + E15 + E10, all §6.1-deferred with parity locks in place; recommended: master embed canonical / L4-L5 frozen); (b) E13 diagnostic mappings (§6.2 — gates mig-3, the Diagnostics cluster / Debug mode — the last unexecuted Registry B bounded area with a ready plan; recommended: `TEXTUAL_CANONICAL` → §9.6); (c) hero prototype disposition (§6.4 — gates nothing; recommended: remove/archive; KI#81 CSS cleanup rides it); (d) rule-strength convention (§6.5 — needed before Part 7A/8 v2 slices; the sampling-cluster presentation waits on it + the KI#72 value decision; recommended: ratify 4 classes + UNKNOWN-as-temporary, incremental application). Recommended call order: **§6.1 → §6.2 → §6.5 → §6.4**.
