...
**iter 125 — dupes-1: self-admitted dupes cleanup (Editorial Policy DEC-15; matrix candidates #2 + #12; owner chat directive: «начни работу по Research & Guide Refactor», Russian-maximal + visual-system/elements usage).** **R16 (§9.11):** quick-check `<details>` dupe (self-described «дубликат §9.3 в формате ожидаемый результат») deleted canon+master — all 5 items + expected results preserved in §9.3 checklist table and E14 «5 быстрых проверок» embed (matrix-verified, runtime-verified); intro rewritten as pointer (базовая версия → §9.3, визуальная → E14 §9.1); 14-item list «Пункты 1–5» re-pointed from the deleted block + its old self-link `#p9_pre_deploy` to §9.3 + E14 (dangling-ref repair). **R01 (§7A.1):** Anchors-placement full copy (4th family occurrence; canonical owner §1.4) compressed to 1 sentence + refs — unique local payload «не часть SP» and placement kept; full rule → §1.4, anchor rules → §2.2; viz pointer VS-EMBED E01 (§1.1) added per `viz > dry text` + owner session directive. Inbound short+refs («см. правило §7A.1», §7A.9/§7A.11/§7A.13) still land on a valid local statement. IMP-48 pairs unchanged. **Verification (executed):** build PASS (hash `2ab607d6` unchanged) · validate 5/5 · validate:master 12/12 (17 warnings = BASE) · version:check 9.2.6 · tests 64/64 · canon sync 97/97 · drift actionable 159 = BASE (stash-compare) · qa:csp/bundle/contrast PASS · qa:english 18 / qa:syntax 247 (unchanged) · qa:doc-versions 1 pre-existing warning (obs-3) · lint 0/0 · `git diff --check` clean · headless-Chrome runtime smoke **12/12** (§9.11 single `<details>`, links resolve, E14 = 5 items, §7A.1 compressed rule present / full copy gone, zero page errors). Root fallbacks rebuilt & staged together with `src/` (DEC-11). **Out-of-scope discovered (deferred → PLAN obs-6):** «Выщебленного» misspelling ×3 (canon part_10 L395, master part_10 L521, master part_09 §9.11 h4; pre-existing at BASE, byte-verified).
...
1. **Owner's choice (see `PLAN.md` for scope/acceptance/verification):** Fork D part 2/3 — sampling widget · dupes-1 — **COMPLETE iter 125** (R16 §9.11 dupe deleted + dangling-ref repair; R01 §7A.1 full-copy → 1-sentence + refs + E01 viz pointer; battery green). Remaining owner choice narrows to Fork D 2/3 (or a new row). Fork D 2/3 executes with infrastructure approval (new widget).
...
6. obs-4 — stale version headers in remaining docs (`components.md`, `terminology_dictionary.md` — each needs its own content-verification pass before refresh). Note: after the ki-67 fix the gate parses their date headers (both currently drift 0d); obs-4 is about version staleness, not dates. obs-3 — `CONTENT_RESTRUCTURE_PLAN.md` header/status (now also the gate's 1 warning). obs-6 — «Выщебленного» misspelling ×3 (recorded iter 125).
...
A new agent: run the preflight in `AGENTS.md`, confirm this Next step against the worktree (`git log` vs iteration above), then start. iter-125 (dupes-1) is complete and verified; the owner-gated rows above are unchanged.
