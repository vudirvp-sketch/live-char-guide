...
- ✅ Правила (ПРАВИЛО:, РЕКОМЕНДАЦИЯ:) — как `**ПРАВИЛО:**` выделение
...
### 3.9 Языковая политика Canon (двухслойная, DEC-16)

Русский — основной язык гайда: вся объяснительная, нормативная, инструктивная и описательная проза, включая метки callouts и заголовки секций. Обычная английская терминология переводится, если существует естественный русский эквивалент (`token → токен`, `lie → ложь`, `rule → правило`).

**Английский сохраняется только в трёх случаях:**

1. **Исполняемый промпт-контент** — текст, который гайд рекомендует писать на английском (SP-блоки, формулировки `<CORE_DIRECTIVES>`, строки Tone Frame, карточки-примеры, copy-paste шаблоны). Каждый такой английский блок обязан иметь русское пояснение/перевод рядом — русский слой для читателя, английский — форма для модели.
2. **Технические идентификаторы** — `data-section` ID, XML-теги, `{{плейсхолдеры}}`, имена полей карточки (System Prompt, Description, Examples, Greeting), имена параметров, имена файлов, машинно-читаемые значения.
3. **Имена собственные** — названия моделей, фронтендов, фреймворков (SPINE, OCEAN, MBTI, Lorebook).

**Метки callouts — на русском** (это проза гайда, не парсинг-якоря; прежнее правило «English semantic anchors» отменено iter 123):

- `**ПРАВИЛО:**` — каноническое правило (нарушение = регрессия качества)
- `**РЕКОМЕНДАЦИЯ:**` — мягкая рекомендация
- `**ПРИМЕР:**` — пример выполнения
- `**ИЛЛЮСТРАЦИЯ:**` — визуальная демонстрация (diff-view, multi-char scene)
- `**ШАБЛОН:**` — шаблон для копирования
- `**Переход:**` — нарративный переход между Parts (только 1-2 на гайд, не во всех Parts)
- `**Синтез:**` — 1-2 предложения итога в конце Part (только для Parts с major conceptual shift)
- `**Ссылка:**` — указатель на связанную секцию в другом Part (нотация `[ref: …]` остаётся машинно-читаемой)
- `**Демонстрирует:**` — список принципов, которые карточка демонстрирует (iter 38+)
- `**Примечание:**` — локальное уточнение (русское, как и раньше)

Тело callouts — на русском. Английский текст внутри копируемых промпт-контейнеров не переводится механически — рядом с контейнером даётся русское пояснение.

**CORE DIRECTIVES:** английская формулировка канонична внутри исполняемого SP-блока; в прозе гайда используется «основные директивы» / «директива». Каждая директива имеет русское имя и пояснение в §7A.2 (русское имя — семантический слой для читателя; английская формулировка — рекомендуемая исполняемая форма).
...
- [ ] Языковая политика (DEC-16): проза/метки/заголовки русские; английский — только исполняемые промпт-контейнеры (с русским пояснением), идентификаторы, имена собственные
