...
**Методология:** проценты отклонения — эмпирические оценки авторов гайда на основе тестирования ~50 карточек на 12B–32B моделях 2024–2025 годов (Llama 2, Qwen 1.x, ранний Mistral). Для современных моделей со словарём от 128K токенов разрывы могут быть меньше указанных. Не точные измерения; воспринимайте как качественные ориентиры (стабилен / дрейфует / сломан). Аналогичные проценты в §3.2 (Иерархия влияния на голос) — той же природы.

**ПРАВИЛО:** Модель — копировщик паттернов, не исполнитель инструкций. Если вы напишете в Description «говорит саркастично» — модель это проигнорирует. Но если в Examples персонаж говорит саркастично — модель скопирует стиль.
...
**Пояснение:** «говорит с сарказмом» / «использует слово 'чувак'» — лингвистический паттерн, его место в Examples. «тихий, чуть хрипловатый тембр» — физическая характеристика голоса, она уживается в Description как часть Embodiment (сенсорный слой «Звук»).
...
**Приветствие (Greeting Message)** — начальное сообщение персонажа. Задаёт место, время и тон. Полный алгоритм написания Greeting (сенсорный якорь → тело FLAW → реплика → крючок), пример Елены и правила длины → `[ref: part_07b.md §7B.X — Greeting Message]`.
