...
**Примечание:** S·P = темперамент Keirsey **S**ensing-**P**erceiving, не **SP** (System Prompt). В виджете и таблицах используется `S·P` (с точкой) для устранения коллизии с аббревиатурой SP → `[ref: part_07a.md §7A.1 — System Prompt]`.
...
**РЕКОМЕНДАЦИЯ:** Для создания карточки используйте OCEAN (полюса → Anchors (поведенческие якоря)) и Enneagram (тип → SPINE). MBTI дополняет понимание, но не имеет прямого маппинга на SPINE.
