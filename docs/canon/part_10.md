...
- **ANCHORS — Базовые:** At-rest Anchors (WANT-направленные) — см. Part 2 §2.1 и Part 4 §4.8.
- **ANCHORS — FLAW-linked:** SPINE-derived Anchors (GHOST-триггеры → FLAW-поведение → Price) — см. Part 4 §4.8 (mapping table).
...
- **ANCHORS — Базовые + FLAW-linked + GHOST-linked:** 3 типа Anchors с T→A→P форматом; Базовые + FLAW-linked Anchors показывают bodily/mechanical Price (сервоприводы, термальные клапаны, аудиовывод, манипуляторы, теплообменник); GHOST-linked Anchors показывают Price (раскрытие уязвимости, рассеивание внимания) — см. Part 2 §2.X и Part 4 §4.8.
