...
[VS: E15 — Annotated Blueprint (Elena Card Walkthrough). Чертёж карточки Елены с 4 слоями аннотаций (структура / якоря / SPINE / директивы). Аннотации структуры несут диапазоны бюджета «мин–макс» по блокам — значения общие с канонической таблицей §7A.12 (SHARED_REFERENCE). Итог Examples (~80–400) — производное значение: итог = бюджет на один пример × количество примеров (исходные значения — §7A.12). Замещает текстовый чертёж-разбор структуры карточки.]
...
[DESCRIPTION]
<identity>
Елена — журналистка-расследователь. Циничная, недоверчивая.
</identity>
...
**Бюджет токенов:** ~540 токенов (базовые) / ~1000 токенов (с SPINE и якорями FLAW-linked). Канонический бюджет по блокам → `[ref: part_07a.md §7A.12 — Token Budget]`.
...
**Бюджет токенов:** ~1100 токенов. Канонический бюджет по блокам → `[ref: part_07a.md §7A.12 — Token Budget]`.
...
**Бюджет токенов:** ~2150 токенов. Канонический бюджет по блокам → `[ref: part_07a.md §7A.12 — Token Budget]`.
...
**бюджет токенов:** ~1500+ токенов. Канонический бюджет по блокам → `[ref: part_07a.md §7A.12 — Token Budget]`.
