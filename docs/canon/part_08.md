...
**Решение:** Формат якоря (Anchor Format): Триггер → Действие → Цена. Каждый якорь обязан иметь Цену — без неё модель не показывает уязвимость (см. `[ref: part_02.md §2.X — Behavioral Anchors]`.
...
**Ссылка:** `[ref: part_06.md §6.X — CoT-якоря]`.
...
**Решение:** Разбить на атомарные якоря: один якорь = одна связь T→A→P. Принцип атомарности. Модель не может надёжно исполнить цепочку из 3+ вложенных условий на ≤14B. См. `[ref: part_02.md §2.X — поведенческие якоря]`.
