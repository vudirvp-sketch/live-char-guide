...
[VS: E12 — Antipattern Catalog. Сетка 5×3 с 15 AP-картами. Каждая карта: AP-ID, severity dot (high/medium), симптом, причина, fix. Замещает текстовое перечисление 15 AP в обзорной секции и даёт визуальный каталог для быстрого сканирования. **Canonical location** для сводного обзора анти-паттернов. Fix-императивы с числовыми значениями — SHARED_REFERENCE на канонических владельцев (AP-5 RepPen → §7A.6 через §8.6; паритет фиксирует `scripts/audit_sampling_parity.py`).]

**ПРАВИЛО:** Проверяйте карточку по этому списку перед тестом. Каждый анти-паттерн — частая ошибка, выявляемая на этапе валидации.
...
**Решение:**
- Удалите дубликаты.
- Оставьте только факты и якоря.
- Голос → Examples (`[ref: part_03.md §3.1 — Voice Isolation]`).

**Пример:** Description 1200 токенов → модель игнорирует половину → персонаж inconsistent.

### До/После: раздувание токенов — Елена
...
**Ссылка:** полная карточка Елены — Part 10 (`p10_elena`).
...
**Симптом:** якорь (Anchor) без физической цены.

**Решение:** Формат якоря (Anchor Format): Триггер → Действие → Цена. Каждый якорь обязан иметь Цену — без неё модель не показывает уязвимость (см. `[ref: part_02.md §2.2 — Behavioral Anchors]`).
...
**Решение:** Description = только факты. Голос → Examples/Greeting. Модель считывает характер из примеров диалога, а не из описания (см. `[ref: part_03.md §3.1 — Voice Isolation]`).

### До/После: голос в Description — Елена
...
**Симптом:** Психологический анализ (SPINE, призрак) помещён в System Prompt.

**Причина:** Попытка "инструктировать" модель о характере.

**Решение:** SPINE (WANT/NEED/дефект)/призрак = только в Description. SP = только инструкции и запреты (см. `[ref: part_07a.md §7A.1 — System Prompt structure]`).

**ПРАВИЛО:** Модель не следует инструкциям о психологии. Психология должна быть в контексте (Description) — как структурированные теги (`<spine>`, `<ocean>`, `<enneagram>`), не нарративом — не в инструкциях (SP).
...
## 8.7 AP-6: Отсутствие анти-годмодинга (No Anti-godmoding)
...
## 8.9 AP-8: Призрак без якорей

`data-section: p8_ap8_ghost_no_anchors`

<!-- difficulty: INTERMEDIATE -->

**Симптом:** призрак описан, но нет якорей, связанных с ним.

**Причина:** призрак как описание, а не поведенческий драйвер.

**Решение:** Каждый GHOST = минимум 1 якорь-триггер. Призрак — корень причинной цепочки SPINE, должен выражаться в поведении через якорь.

**Пример:** Елена — призрак «предательство редактора» → якорь «Когда вопрос о лояльности → напрягается».

**Ссылка:** `[ref: part_04.md §4.8 — SPINE → карта якорей]` (FLAW-linked якоря обязательны).
...
**Симптом:** SPINE-элементы не связаны логически — желание совместимо с потребностью (нет конфликта), дефект существует без объяснения.

**Причина:** желание, потребность, дефект, ложь, призрак написаны независимо, без причинно-следственной связи.
...
желание совместимо с потребностью — нет конфликта. Дефект блокирует обе потребности — но откуда дефект? Без объяснения через ложь/призрак цепь разорвана.

**Критерий сломанной SPINE (broken SPINE)** (сохраняется как диагностика): «желание совместимо с потребностью — нет конфликта» + дефект не объяснён через ложь/призрак. Это не то же самое, что «призрак/ложь текстуально отсутствуют» — по Part 4 §4.1, призрак и ложь могут быть **неявными** для простых персонажей. Диагностируйте сломанную SPINE по разрыву каузальной цепи (дефект без корня в лжи/призраке), а не по текстовому отсутствию элементов.
...
**Симптом:** > 3 CoT-якоря в карточке.

**Причина:** Попытка задать весь внутренний процесс.

**Решение:** 2–3 CoT-якоря максимум. Остальное — обычные якоря.

**ПРАВИЛО:** Модель генерирует внутренние монологи вместо действий, персонаж "думает" больше чем "делает".

### ❌ Перегрузка CoT

Карточка содержит 5 CoT-якоря. Модель тратит >50% ответа на внутренний процесс → внешний ответ обеднён.

### ✅ Максимум 2–3 CoT-якоря

Оставьте только ключевые триггеры (GHOST-связь, критический конфликт). Остальные якоря — обычные T→A→P без CoT.

**Ссылка:** `[ref: part_06.md §6.6 — CoT-якоря]`.
...
**Решение:** Character Markers (Маркеры персонажа) — явные маркеры уникальности голоса. Примеры: специфичная лексика, синтаксические паттерны, телесные реакции, catchphrases. См. `[ref: part_03.md §3.8 — Мульти-персонажные примеры]`.
...
**Ссылка:** `[ref: part_07a.md §7A.9 — XML structure]` + `[ref: part_07a.md §7A.4 — Format Lock]`.
...
**Решение:** Проверяйте консистентность всех Lorebook записей с Description. См. `[ref: part_07b.md §7B.3 — Lorebook]`.
...
## 8.16 AP-15: Вложенные якоря (Nested Anchors)

`data-section: p8_ap15_nested_anchors`

<!-- difficulty: EXPERT -->

**Симптом:** якорь содержит вложенные условия: «Если X, а потом Y → делает Z».

**Причина:** Попытка задать сложную поведенческую логику в одном якоре.

**Решение:** Разбить на атомарные якоря: один якорь = одна связь T→A→P. Принцип атомарности. Модель не может надёжно исполнить цепочку из 3+ вложенных условий на ≤14B. См. `[ref: part_02.md §2.2 — поведенческие якоря]`.
...
**Синтез:** 15 анти-паттернов покрывают ~90% ошибок сборки. Симптом → причина → исправление. Наиболее частые: AP-3 (голос в Description), AP-9 (broken SPINE), AP-15 (nested якоря).
