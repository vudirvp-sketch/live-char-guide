...
**Нотация `[ref: ...]`:** cross-reference между canon-файлами. Форматы: `[ref: part_XX.md §X.Y — Topic]` (между файлами) или `[ref: §X.Y]` (внутри файла). Ссылки двунаправленные — см. `AGENT_NAVIGATION.md` §9 Cross-Reference Pairs.
...
- `**Synthesis:**` — 1–2 предложения summary в конце Part (только для Parts с major conceptual shift).
...
**Cross-ref:** Полное оглавление генерируется build-скриптом (`AUTO_TOC_PLACEHOLDER` в master HTML). Section registry — `docs/canon/_README.md` §2. Правила Canon — `docs/canon/_README.md` §3.
...
1. **System Prompt (SP):** «You are [имя], [роль]. Never speak or act for {{user}}.» + Tone Frame + Format Lock. ~50 токенов → `[ref: part_07a.md §7A.X — Assembly Pipeline]`.
2. **Description:** `<identity>` + `<spine>` (WANT/NEED/FLAW/LIE/GHOST — 1 строка на каждый элемент). ~150 токенов → `[ref: part_04.md §4.7 — Полная цепочка SPINE]`.
3. **Examples (2–3 блока `<START>`):** каждый 80–120 токенов, демонстрирует голос + минимум один embodiment (физическая реакция). Голос задаётся ТОЛЬКО здесь → `[ref: part_03.md §3.3 — Правила написания Examples]`.
4. **Greeting:** 50–100 токенов, 4 шага (Sensory Anchor → тело FLAW → реплика → крючок) → `[ref: part_07b.md §7B.2 — Greeting Message]`.
5. **Anchors (3–5):** Trigger → Action → Price (Price = наблюдаемый физический маркер в той же сцене) → `[ref: part_02.md §2.1 — Behavioral Anchors]`.
6. **Pre-deploy check:** 5 быстрых проверок (PP=0, Voice в Examples, Price в Anchors, Format Lock, Anti-godmoding) → `[ref: part_09.md §9.11 — Пред-деплой валидация]`.

**Готовый пример:** карточка Елены — `[ref: part_10.md §10.1]`. Полная карточка с аннотациями (~440 токенов базовые, ~900 с SPINE+FLAW-linked Anchors).

**3 правила, которые нельзя нарушать:**

1. **Anchor = Trigger → Action → Price.** Без Price (наблюдаемой физической реакции в той же сцене) Anchor = инструкция без паттерна.
2. **Голос — только в Examples и Greeting.** Voice в Description = потеря токенов и Voice Leak.
3. **Психология (SPINE, OCEAN) — только в Description.** Никогда в System Prompt.
...
**Cross-ref:** Pre-build checklist (6 вопросов перед стартом) — `[ref: part_01.md §1.8 — Pre-build checklist]`. Карта всех персонажей гайда — `[ref: appendix_character_map.md]`. Pre-deploy валидация (5+14 проверок) — `[ref: part_09.md §9.11]`.
