...
**Примечание:** Канонический формат Anchors — `<anchors>` XML с категориями (Базовые / FLAW-linked / GHOST-linked / etc.).
...
**Convention:** В примерах карточек (Part 10) и шаблонах CORE_DIRECTIVES заменяются на shorthand `{{CORE_DIRECTIVES — канонический шаблон → Part 7A}}` — ссылка на полное определение в §7A.2. Это принято как convention: принцип «одно определение — одно место» запрещает дублирование полного текста директив в нескольких локациях. Shorthand = навигационная ссылка, не самостоятельный контент.
