...
**Приветствие (Greeting Message)** — начальное сообщение персонажа. Задаёт место, время и тон.
...
**Сенсорный якорь (Sensory Anchor)** — приём пред-настройки восприятия: персонаж уже в состоянии до первого слова. Пример: Елена за барной стойкой слышит звон бокалов и чувствует запах виски — прежде чем произнести реплику.

### Разобранный пример: Greeting Елены

**Примечание:** Greeting Елены здесь — учебный пример для разбора 4-шагового алгоритма (бар, ночь). Канонический Greeting для production-карточки Елены (кабинет редакции, 2 часа ночи) — `[ref: part_10.md §10.1 — GREETING]`. Разные сцены = разные сенсорные якоря, тот же персонаж.
...
**РЕКОМЕНДАЦИЯ:** Lorebook используется для сенсорных активаций и контекстных фактов. Активация через GHOST-события описана в `[ref: §7B.4 — Lorebook механики]`.
...
**Пример 1: факт призрака (GHOST, предательство)** — ИЛЛЮСТРАЦИЯ (LOREBOOK, GHOST):
...
**Пример 2: ⚠️ ВНЕ КАНОНА — вторичный призрак (secondary GHOST, пожар)** — ИЛЛЮСТРАЦИЯ (LOREBOOK, SECONDARY GHOST):

> **⚠️ ВНЕ КАНОНА для Елены:** Этот пример показывает сценарий вторичного призрака (secondary GHOST). Каноническая Елена имеет ОДИН призрак (GHOST) — предательство редактора (см. `docs/elena_character_bible.md` и `part_04.md` §4.2). «Пожар» используется только если в карточке автора выбран сценарий вторичного призрака. Не включайте в production-карточку Елены без явного решения автора.
...
**Ссылка:** Поведенческие механики Lorebook (каскад по диапазону, комбинированный триггер, контекстный фильтр) → `[ref: §7B.4 — Lorebook механики]`.
...
**РЕКОМЕНДАЦИЯ (Область применения):** Параметры механик работают вместе с базовой структурой записи. Сначала изучите `[ref: §7B.3 — Структуру Lorebook]`.

**ПРАВИЛО (Ограничение):** Никогда не вставляйте действия или мысли `{{user}}` в поле `content`. Это не техническое ограничение, а принцип Anti-godmoding (анти-годмодинга).
...
Чёткое разделение системных инструкций и реплик персонажа внутри поля `content`. **ИЛЛЮСТРАЦИЯ:**
...
**Ссылка:** Продвинутые механики (эмуляция усталости, мета-лор, ложная память) → `[ref: §7B.5 — Продвинутые механики Lorebook]`.

**РЕКОМЕНДАЦИЯ:** Механики Lorebook особенно полезны для GHOST-связанных триггеров. GHOST-факты (события прошлого) могут активироваться через комбинированный триггер и каскад по диапазону, создавая динамическое поведение.
...
**ПРАВИЛО (Эмуляция усталости — критическое предупреждение):** При использовании этой механики якоря призрака (GHOST-якоря, `[ref: part_04.md §4.2 — GHOST]`) **обязательно** должны быть явно пересказаны внутри `content` этой записи. Без этого поведенческие якоря деградируют вместе с поверхностными поведениями.

**ПРАВИЛО (Ложная память — только для экспертов):** Риск Одиночное использование. Требует предварительного согласия пользователя. Злоупотребление разрушает достоверность лора. Конфликтует с принципом психологического ядра при повторном использовании.
...
- [ ] Эмуляция усталости: якоря призрака (GHOST-якоря) явно пересказаны в content.
