...
| id | lines | block | tag | decision | load | repeat | presentation | notes |
|---|---|---|---|---|---|---|---|---|
| appendix_glossary::01 | L19 | intro (definition + ref pattern) | NAVIGATION | KEEP | YES | — | open | Declares the glossary's own entry pattern; → migration_map_v2 C-1. **iter 160 observation (P-C-4b):** the «всех технических терминов» coverage claim overstates — the appendix carries 25 of the registry's 45 entries (the 20 promoted machine-layer terms absent; count corrected iter 163 — the canonical-audit stage verified 25 canon `###` articles + 25 master `glossary-entry` divs = the registry's 25 merged-canonical + 20 machine-layer split); the appendix↔registry relationship rides D-5's switch-time disposition |
