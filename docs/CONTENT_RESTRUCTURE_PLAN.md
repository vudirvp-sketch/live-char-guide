...
| **31** | DGA Phase 2 final (KI#18-G + KI#18-H ✅ CLOSED keep-by-design → KI#18 ✅ CLOSED) | ✅ DONE — KI#18-G (Part 8 per-AP sections vs E12 Antipattern Catalog viz — catalog vs detail pattern): rationale documented в `docs/canon/part_08.md` (7-dimension comparison table). KI#18-H (Part 10 E15 Annotated Blueprint callouts vs E01 Card Anatomy in Part 1 — annotation layer pattern): rationale documented в `docs/canon/part_10.md` (6-dimension comparison table). **No master HTML edit** — both sub-items keep-by-design. **KI#18 ✅ CLOSED — 9/9 resolved** (7 fixed, 2 keep-by-design). **DGA COMPLETE.** | `docs/canon/part_08.md`, `docs/canon/part_10.md` | LOW |
| **32** | **Visual System Scroll-Animation Bug (KI#20) ✅ CLOSED** | ✅ DONE — Single-file edit `src/shell/widgets/vs-scroll-observer.js`: `SCROLL_ENTER_SELECTOR` extended from 3 → 11 classes (added `.ring-anim, .ring-text-anim, .bar-rect, .anim-group, .center-pulse, .pentagon-anim, .profile-anim, .callout`). 43 animation elements на 5 VS-EMBED (E06/E07/E08/E09/E15) теперь корректно наблюдаются IntersectionObserver. Audit script `scripts/audit_vs_embeds.py` добавлен (regression test). Stale iter-specific READMEs DELETED (README_iter18.md, README_ITER8_MERGE.md, ITER9_PATCH_README.md, MERGE_INSTRUCTIONS.md — 444 строки). Документация актуализирована и почищена. `validate:master`/`build`/`validate`/`test:unit`/`lint`/`qa:csp`/`qa:bundle`/`qa:doc-versions` PASS. Build hash `fd3d96d3` unchanged (JS widget edit only, не master HTML). **KI#20 ✅ CLOSED — 5/5 sub-items fixed.** | `src/shell/widgets/vs-scroll-observer.js`, `widgets/vs-scroll-observer.js`, `scripts/audit_vs_embeds.py` (NEW) | HIGH |
...
**Iter 32 COMPLETE — KI#20 Visual System Scroll-Animation Bug ✅ CLOSED.** 5/5 sub-items fixed single-file edit'ом `src/shell/widgets/vs-scroll-observer.js` (selector extended for 8 animation classes: `.ring-anim, .ring-text-anim, .bar-rect, .anim-group, .center-pulse, .pentagon-anim, .profile-anim, .callout`). 43 animation elements на 5 VS-EMBED (E06/E07/E08/E09/E15) теперь корректно отображаются на собранном сайте. Audit script `scripts/audit_vs_embeds.py` добавлен (regression test). Stale iter-specific READMEs DELETED (README_iter18.md, README_ITER8_MERGE.md, ITER9_PATCH_README.md, MERGE_INSTRUCTIONS.md — 444 строки). Все validation gates PASS, build hash `fd3d96d3` unchanged. Документация актуализирована и почищена (CHANGELOG.md: 201 → 93 строки, AGENT_NAVIGATION.md: -9198 chars / -18%, PLAN.md: 220 → 115 строк, STATUS.md: 93 → 107 строк с переписанным содержимым).

**Iter 33+ — что осталось:**

1. **Все Known Issues (KI#1..KI#20) ✅ CLOSED.** iter 33+ roadmap: **none planned**.
2. **Принцип `viz > dry text` (iter 8+):** сохраняется. Unique контент не удаляется даже при дублировании. **Catalog vs Detail / Annotation Layer patterns** — NOT pure re-explanation, keep-by-design с documented rationale (см. KI#18-G/H).
3. **VS scroll-animation invariant (KI#20, iter 32+):** все animation classes в `src/assets/vs-styles.css` должны быть покрыты либо `SCROLL_ENTER_SELECTOR` в `vs-scroll-observer.js`, либо `scroll-enter` class на каждом элементе. Audit: `python3 scripts/audit_vs_embeds.py` (0 regressions expected). При добавлении нового VS-EMBED с animation classes — добавить классы в `SCROLL_ENTER_SELECTOR` или `scroll-enter` на элементы.
4. **Новые баги:** сначала документировать в `STATUS.md` как KI#N, потом фиксить.

**KI#20 — Visual System Scroll-Animation Bug — sub-items:**
...
**Conclusion:** KI#20 ✅ CLOSED iter 32 — 5/5 sub-items fixed, 43 animation elements покрыты. Все previous KI (KI#1..KI#19) ✅ CLOSED. iter 33+ roadmap: none planned — все KIs CLOSED.

**KI#18 — Deployed Guide Duplication Audit (DGA) — sub-items (✅ CLOSED iter 31):**

| Sub | Part | Описание | Severity | Status |
|-----|------|----------|----------|--------|
| **A** | Part 9 | E14 Quality Scale viz ↔ p9_quality_scale table | MEDIUM | ✅ FIXED iter 26 |
| **B** | Part 1 | E01 Card Anatomy viz ↔ p1_card_overview table «Функция» col | LOW-MEDIUM | ✅ FIXED iter 28 |
| **C** | Part 2 | E03 Behavioral Anchors viz ↔ p2_basic_anchors table «Описание» col | LOW-MEDIUM | ✅ FIXED iter 28 |
| **D** | Part 4 | E05 SPINE viz + panel ↔ p4_spine_overview intro partial re-explanation | LOW | ✅ FIXED iter 30 |
| **E** | Part 5 | E09 OCEAN Context Limits + semantic inconsistency (rule <30/>70 vs <40/>60) | MEDIUM (semantic) | ✅ FIXED iter 30 |
| **F** | Part 6 | E11 CoT viz ↔ p6_cot_tiers table «Формат» col | LOW-MEDIUM | ✅ FIXED iter 29 (partial) |
| **G** | Part 8 | E12 Antipatterns viz ↔ per-AP sections (catalog vs detail) | LOW (by design) | ✅ CLOSED iter 31 (keep-by-design) |
| **H** | Part 10 | E15 Annotated Blueprint callouts ↔ E01 token budgets (annotation layer) | LOW (intentional) | ✅ CLOSED iter 31 (keep-by-design) |
| **I** | Part 2 | E04 Embodiment Protocol viz ↔ p2_embodiment table «Описание» col | LOW-MEDIUM | ✅ FIXED iter 29 |

**Подсказка следующему агенту:**

> Перед стартом iter 33+ прочитай:
> 1. `STATUS.md` (iter 32 — KI#20 ✅ CLOSED 5/5; все previous KI#1..#19 ✅ CLOSED; iter 33+ roadmap — none planned)
> 2. `worklog.md` (iter 32 record — самый подробный)
> 3. `AGENT_NAVIGATION.md` (§6 pitfall #39 KI#20 ✅ CLOSED iter 32, §8 iter 33+ roadmap — none planned)
> 4. `docs/canon/_README.md` (§5 migration status — все 10 Parts + 3 Appendix ✅, Canon COMPLETE)
> 5. `docs/CONTENT_RESTRUCTURE_PLAN.md` (§5.2 iter 32 → KI#20 CLOSED, §8 iter 32 stop point)
>
> VS scroll-animation invariant: `python3 scripts/audit_vs_embeds.py` (0 regressions expected). Принцип `viz > dry text` — сохраняется. Если найден новый баг — сначала документируй в `STATUS.md` как KI#N, потом фиксий.
