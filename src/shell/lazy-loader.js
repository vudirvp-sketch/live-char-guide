...
/* ============================================
   OCEAN INTERACTIVE FUNCTIONALITY (from plan)
   ============================================ */

function initOcean() {
  const TRAITS = ['O', 'C', 'E', 'A', 'N'];

  // Trait data for panel updates
  const traitData = {
    O: {
      name: 'Открытость опыту',
      low: 'Практичный, предпочитает знакомое, консервативный в методах.',
      high: 'Любопытный, творческий, открыт новому опыту и идеям.',
      enneagramHigh: [4, 5, 7],
      enneagramLow: [1, 6, 9]
    },
    C: {
    ...
    E: {
      name: 'Экстраверсия',
      low: 'Интроверт, предпочитает одиночество или малые группы.',
      high: 'Экстраверт, энергичный, общительный.',
      enneagramHigh: [2, 3, 7, 8],
      enneagramLow: [4, 5, 9]
    },
    A: {
      name: 'Доброжелательность',
      low: 'Конкурентный, критичный, эгоистичный.',
      high: 'Доверчивый, альтруистичный, кооперативный.',
      enneagramHigh: [2, 6, 9],
      enneagramLow: [3, 8]
    },
    N: {
      name: 'Нейротизм',
      low: 'Эмоционально стабильный, спокойный.',
      high: 'Тревожный, эмоционально реактивный.',
      enneagramHigh: [4, 6],
      enneagramLow: [3, 9]
    }
  };
  ...
  TRAITS.forEach(trait => {
    ...
    const handler = () => {
      ...
      document.querySelectorAll('#ocean-svg .ocean-node').forEach(n => n.classList.remove('active'));
      // Add active to clicked node
      node.classList.add('active');

      // Open corresponding details
      const details = document.querySelector(`#ocean-detail-${trait}`);
      if (details) {
        details.open = true;
      ...
      if (panel && data) {
        panel.innerHTML = `
          <div class="trait-title" style="color:var(--ocean-${trait});">${trait}: ${data.name}</div>
          <div class="trait-low"><strong>Низкая:</strong> ${data.low}</div>
          <div class="trait-high"><strong>Высокая:</strong> ${data.high}</div>
          <p style="margin-top:12px;font-size:0.9em;color:var(--text-muted);">
            Связь с Enneagram: высокая → типы ${data.enneagramHigh.join(', ')}, низкая → типы ${data.enneagramLow.join(', ')}
          </p>
        `;
      }
    };
...
function initOceanValidator() {
  ...
  // Trait data for Enneagram suggestions
  const traitData = {
    O: { enneagramHigh: [4, 5, 7], enneagramLow: [1, 6, 9] },
    C: { enneagramHigh: [1, 3, 5], enneagramLow: [7, 9] },
    E: { enneagramHigh: [2, 3, 7, 8], enneagramLow: [4, 5, 9] },
    A: { enneagramHigh: [2, 6, 9], enneagramLow: [3, 8] },
    N: { enneagramHigh: [4, 6], enneagramLow: [3, 9] }
  };

  // Check if validator exists
  const validator = document.getElementById('ocean-validator');
  if (!validator) return;

  // 3. Slider handlers with profile polygon update
  TRAITS.forEach(trait => {
    const slider = document.getElementById(`slider-${trait}`);
    const output = document.getElementById(`value-${trait}`);
  ...
  function updateValidator() {
    ...
    TRAITS.forEach(t => {
      ...
      values[t] = slider ? parseInt(slider.value, 10) : 50;
    });

    // Count extremes
    const extremes = TRAITS.filter(t => values[t] < 30 || values[t] > 70);
    const status = document.getElementById('validator-status');
    const detailsList = document.getElementById('extremes-list');
    const enneaSuggestion = document.getElementById('enneagram-suggestion');

    if (extremes.length >= 3) {
      status.className = 'ocean-validator-status validator-red';
      status.textContent = '✗ 3+ экстремума — риск непоследовательности персонажа';
    } else if (extremes.length === 0) {
      status.className = 'ocean-validator-status validator-yellow';
      status.textContent = '⚠ 0 экстремумов — персонаж может быть забываемым';
    } else {
      status.className = 'ocean-validator-status validator-green';
      status.textContent = '✓ 1–2 экстремума — запоминающийся персонаж';
    }

    // Details
    if (extremes.length > 0) {
      const parts = extremes.map(t => {
        const val = values[t];
        const dir = val < 30 ? 'низкая' : 'высокая';
        return `${t}: ${val} (${dir})`;
      });
      detailsList.textContent = 'Экстремумы: ' + parts.join(', ');
    } else {
      detailsList.textContent = 'Все значения в нормальном диапазоне (30–70).';
    }

    // Enneagram suggestion
    const suggestedTypes = new Set();
    extremes.forEach(t => {
      const val = values[t];
      const types = val < 30 ? traitData[t].enneagramLow : traitData[t].enneagramHigh;
      types.forEach(type => suggestedTypes.add(type));
    });
    if (suggestedTypes.size > 0) {
      enneaSuggestion.textContent = 'Рекомендуемые типы Enneagram: ' + [...suggestedTypes].sort().join(', ');
    } else {
      enneaSuggestion.textContent = '';
    }
  }

  // 5. Dynamic profile polygon
  function updateProfilePolygon() {
    ...
    // Pentagon geometry: center (200, 185), radius 145, angles starting at -90°
    const cx = 200;
    const cy = 185;
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      TRAITS.forEach(t => {
        const slider = document.getElementById(`slider-${t}`);
        const output = document.getElementById(`value-${t}`);
        if (slider) {
          slider.value = 50;
  ...
  if (copyBtn) {
    copyBtn.addEventListener('click', () => {
      ...
      navigator.clipboard.writeText(profile).then(() => {
        ...
        alert('Ошибка копирования: ' + profile);
      });
    });
  }
...
// Initialize when DOM ready or after content load
if (document.readyState === 'loading') {
