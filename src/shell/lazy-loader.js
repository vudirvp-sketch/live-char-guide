  ...
  function showOceanPanel(letter, name, color) {
    ...
    const data = {
      'O': {
        ...
        anchor: 'Low O → preference for routine, suspicion of novelty\nHigh O → seeks variety, open to unconventional ideas'
      },
      'C': {
        low: ['Спонтанный', 'Гибкий', 'Менее организованный'],
        high: ['Организованный', 'Дисциплинированный', 'Надёжный'],
        anchor: 'Low C → last-minute decisions, flexible deadlines, chaos-tolerant\nHigh C → plans ahead, structured routines, detail-oriented anchors'
      },
      'E': {
        low: ['Интроверт', 'Предпочитает одиночество', 'Задумчивый'],
        high: ['Экстраверт', 'Энергичный', 'Общительный'],
        anchor: 'Low E → solo activities, internal monologue, small-group preference\nHigh E → initiates conversation, seeks stimulation, group activities'
      },
      'A': {
        low: ['Конкурентный', 'Критичный', 'Скептичный'],
        high: ['Доверчивый', 'Альтруистичный', 'Кооперативный'],
        anchor: 'Low A → competitive edge, challenging others, skepticism as default\nHigh A → helps without being asked, assumes good intent, conflict-avoidant'
      },
      'N': {
        low: ['Эмоционально стабильный', 'Спокойный', 'Устойчивый'],
        high: ['Тревожный', 'Эмоционально реактивный', 'Чувствительный'],
        anchor: 'Low N → crisis-ready, doesn\'t dwell on negatives, steady under pressure\nHigh N → stress reactions visible, overthinks, needs reassurance'
      }
    };
